...
${loopBody}
\t}
\treturn pos, pendingNl, prevText, prevKind, bpText, hasPrev, hasPrev2, parenHead, lastClose, lastBang, templateStack
}
func lex(src string) []Tok {
\tvar out []Tok
\tlexFrom(src, 0, false, "", "", "", false, false, nil, false, false, nil, &out, 0)
\treturn out
}`;
  }
  if (rxOrTpl) {
...
\tfor scanOff < len(newText) {
\t\tbefore := len(scratch)
\t\tscanOff, pendingNl, prevText, prevKind, bpText, hasPrev, hasPrev2, parenHead, lastClose, lastBang, templateStack = lexFrom(newText, scanOff, pendingNl, prevText, prevKind, bpText, hasPrev, hasPrev2, parenHead, lastClose, lastBang, templateStack, &scratch, 1)
\t\tif len(scratch) == before { break }
\t\tt := scratch[len(scratch)-1]
\t\ttxt := newText[t.Off:t.End]
\t\thd := false
...
\tfor pos < len(src) {
\t\tbefore := len(toks)
\t\tpos, pendingNl, prevText, prevKind, bpText, hasPrev, hasPrev2, parenHead, lastClose, lastBang, templateStack = lexFrom(src, pos, pendingNl, prevText, prevKind, bpText, hasPrev, hasPrev2, parenHead, lastClose, lastBang, templateStack, &toks, 1)
\t\tif len(toks) == before { break }
\t\tt := toks[len(toks)-1]
\t\ttxt := src[t.Off:t.End]
\t\thd := false
...
func countLive(id int32) int {
	...
	nd := &nodes[id]
	if !nd.IsLeaf {
		for i := 0; i < nd.KidCount; i++ { n += countLive(kids[nd.KidStart+i]) }
	}
	return n
}
...
${shapeA ? `func tryReuseTop(oldRoot int32, newText string, newMeta []alignMeta, byteDelta, oldN, newN, prefix, suffix int) (int32, int) {
	...
	for i := 0; i < old.KidCount; i++ { oldKids[i] = kids[old.KidStart+i] }
	prefixLen := 0
	for prefixLen < len(oldKids) {
		ext := nodes[oldKids[prefixLen]].Ext
		if ext <= prefix { prefixLen++ } else { break }
	}
	suffixStart := len(oldKids)
	for i := len(oldKids) - 1; i >= prefixLen; i-- {
		if nodes[oldKids[i]].TokStart >= oldN-suffix { suffixStart = i } else { break }
	}
	...
	if prefixLen > 0 { pos = nodes[prefixKids[prefixLen-1]].TokEnd } else { pos = 0 }
	scratch = scratch[:0]
	mid := make([]int32, 0)
	suffixBound := newN - suffix
	maxCand := -1
	for _, id := range suffixCand {
		c := nodes[id].TokStart + tokDelta
	...
	tryHit := func() (int32, int, bool) {
		...
		for i, id := range suffixCand {
			if nodes[id].TokStart+tokDelta == pos { r, n := finishHit(i); return r, n, true }
		}
		return -1, 0, false
	}
...
\tIsLeaf          bool
\tKidStart, KidCount, Offset, End, TokStart, TokEnd, Ext int
}
