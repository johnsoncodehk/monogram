...
import { token, rule, defineGrammar, many, opt, alt, seq, oneOf, noneOf, range, named, star, plus, notFollowedBy } from './src/api.ts';
...
const Name = token(seq(oneOf(range('a', 'z'), range('A', 'Z')), star(oneOf(named('word'), ':', '.', '-'))), { identifier: true });
...
const VoidName = token(seq(oneOf(range('a', 'z'), range('A', 'Z')), star(oneOf(named('word'), ':', '.', '-'))), { scope: 'entity.name.tag' });
...
const UnquotedValue = token(plus(noneOf(named('whitespace'), '"', "'", '<', '>', '=', '`')), { scope: 'string.unquoted.html' });
...
const Comment = token(seq('<!--', star(seq(notFollowedBy('-->'), named('any')), { greedy: false }), '-->'), { scope: 'comment.block.html' });
