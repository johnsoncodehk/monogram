...
**Highlighter correctness, per language — Monogram vs the official grammar.** Auto-generated by [`test/highlight-bench.ts`](test/highlight-bench.ts); both rows are graded by the same neutral `tsc` oracle:
...
Monogram is more correct than the official hand-written grammar in **both** languages — from one shared, conformance-proven core. (On *unambiguous* code the TextMate grammars are neck-and-neck at ~99%; the ledger above is the ambiguity-rich code where a parser-derived highlighter pulls clear. The tree-sitter output, derived from the *same* grammar, leads outright at 95.9% — opt-in via `MONOGRAM_TS_WASM=… node test/highlight-bench.ts --engines`.)
