// GENERATED by src/gen-cst-match.ts — do not edit. Per-arm CST destructurers for "typescriptreact".
/* eslint-disable */
import type { $templateNode, ArrayBindingElementNode, BindingElementNode, BindingNode, BindingPatternNode, BindingPropertyNode, BlockNode, ClassHeritageNode, ClassMemberNode, CstChild, CstLeaf, DeclNode, DecoratorExprNode, EnumMemberNode, ExportSpecifierNode, ExprNode, ForBindingNode, ForHeadNode, ImportClauseNode, ImportSpecifierNode, InterfaceMemberNode, JSXAttrNode, JSXAttrValueNode, JSXChildNode, JSXContainerNode, JSXElementNode, JSXTagNameNode, MemberNameNode, NewTargetNode, ParamNode, ProgramNode, PropNode, StmtNode, SwitchCaseNode, TypeMemberNode, TypeNode, TypeParamNode, TypeParamsNode, TypeofRefNode } from "./typescriptreact.cst-types.ts";

const __lit = (c: readonly CstChild[], i: number, src: string, text: string, tt: string): boolean => {
  const k = c[i] as CstLeaf | undefined;
  return k !== undefined && k.tokenType === tt && k.end - k.offset === text.length && src.startsWith(text, k.offset);
};
const __tok = (c: readonly CstChild[], i: number, name: string): boolean => {
  const k = c[i] as CstLeaf | undefined;
  return k !== undefined && k.tokenType === name;
};
const __nodeOf = (c: readonly CstChild[], i: number, rule: string): boolean => {
  const k = c[i] as { rule?: string } | undefined;
  return k !== undefined && k.rule === rule;
};

export type TypeMatch =
  | { arm: "ident"; ident: CstLeaf; type?: TypeNode; isTok?: CstLeaf }
  | { arm: "led_lt"; left: TypeNode; type: (TypeNode)[] }
  | { arm: "led_bracket"; left: TypeNode }
  | { arm: "led_pipe"; left: TypeNode; type: TypeNode }
  | { arm: "led_amp"; left: TypeNode; type: TypeNode }
  | { arm: "pipe"; type: TypeNode }
  | { arm: "amp"; type: TypeNode }
  | { arm: "keyof"; type: TypeNode }
  | { arm: "typeof_"; typeofRef: TypeofRefNode }
  | { arm: "readonly"; type: TypeNode }
  | { arm: "paren"; type: TypeNode }
  | { arm: "typeParams"; typeParams?: TypeParamsNode; param: (ParamNode)[]; type: TypeNode }
  | { arm: "abstract"; abstractTok?: CstLeaf; typeParams?: TypeParamsNode; param: (ParamNode)[]; type: TypeNode }
  | { arm: "bracket"; ident: (CstLeaf)[]; type: (TypeNode)[] }
  | { arm: "brace"; typeMember: (TypeMemberNode)[]; _Kw: (";" | ",")[] }
  | { arm: "asserts"; ident: CstLeaf; type?: TypeNode; isTok?: CstLeaf }
  | { arm: "led_extends"; left: TypeNode; type: TypeNode; type2: TypeNode; type3: TypeNode }
  | { arm: "infer"; ident: CstLeaf; type?: TypeNode; type2?: TypeNode; type3?: TypeNode; extendsTok?: CstLeaf }
  | { arm: "string"; string: CstLeaf }
  | { arm: "number"; number: CstLeaf }
  | { arm: "hexNumber"; hexNumber: CstLeaf }
  | { arm: "octalNumber"; octalNumber: CstLeaf }
  | { arm: "binaryNumber"; binaryNumber: CstLeaf }
  | { arm: "bigInt"; bigInt: CstLeaf }
  | { arm: "dash"; alt: { branch: "number"; number: CstLeaf } | { branch: "bigInt"; bigInt: CstLeaf } }
  | { arm: "true_" }
  | { arm: "false_" }
  | { arm: "null_" }
  | { arm: "undefined" }
  | { arm: "void_" }
  | { arm: "this_" }
  | { arm: "unique" }
  | { arm: "import_"; type: TypeNode }
  | { arm: "template"; template: CstLeaf | $templateNode }
  | { arm: "led_bracket2"; left: TypeNode; type: TypeNode }
  | { arm: "led_dot"; left: TypeNode; ident: CstLeaf }
  | { arm: "led_dot2"; left: TypeNode; type: (TypeNode)[] }
  | { arm: "question"; type: TypeNode }
  | { arm: "bang"; type: TypeNode }
  | { arm: "question2" }
  | { arm: "star" }
  | { arm: "function_"; param: (ParamNode)[]; type?: TypeNode }
  | { arm: "led_question"; left: TypeNode }
  | { arm: "led_bang"; left: TypeNode };

function _Type$ident(c: readonly CstChild[], src: string): TypeMatch | null {
  let ident: (CstLeaf) | undefined;
  let type: (TypeNode) | undefined;
  let isTok: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "is", "$keyword")) { _t1 = false; break _b2; }
      isTok = c[i] as CstLeaf;
      i++;
      if (!__nodeOf(c, i, "Type")) { _t1 = false; break _b2; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident!, type, isTok };
}

function _Type$led_lt(c: readonly CstChild[], src: string): TypeMatch | null {
  let left: (TypeNode) | undefined;
  const type: (TypeNode)[] = [];
  let i = 0;
  if (!__nodeOf(c, i, "Type")) return null;
  left = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, "<", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "Type")) { _t3 = false; break _b4; }
      type.push(c[i] as TypeNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "Type")) { _t1 = false; break _b2; }
        type.push(c[i] as TypeNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, ">", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_lt", left: left!, type };
}

function _Type$led_bracket(c: readonly CstChild[], src: string): TypeMatch | null {
  let left: (TypeNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Type")) return null;
  left = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, "[", "$punct")) return null;
  i++;
  if (!__lit(c, i, src, "]", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_bracket", left: left! };
}

function _Type$led_pipe(c: readonly CstChild[], src: string): TypeMatch | null {
  let left: (TypeNode) | undefined;
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Type")) return null;
  left = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, "|", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_pipe", left: left!, type: type! };
}

function _Type$led_amp(c: readonly CstChild[], src: string): TypeMatch | null {
  let left: (TypeNode) | undefined;
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Type")) return null;
  left = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, "&", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_amp", left: left!, type: type! };
}

function _Type$pipe(c: readonly CstChild[], src: string): TypeMatch | null {
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "|", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "pipe", type: type! };
}

function _Type$amp(c: readonly CstChild[], src: string): TypeMatch | null {
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "&", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "amp", type: type! };
}

function _Type$keyof(c: readonly CstChild[], src: string): TypeMatch | null {
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "keyof", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "keyof", type: type! };
}

function _Type$typeof_(c: readonly CstChild[], src: string): TypeMatch | null {
  let typeofRef: (TypeofRefNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "typeof", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "TypeofRef")) return null;
  typeofRef = c[i] as TypeofRefNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "typeof_", typeofRef: typeofRef! };
}

function _Type$readonly(c: readonly CstChild[], src: string): TypeMatch | null {
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "readonly", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "readonly", type: type! };
}

function _Type$paren(c: readonly CstChild[], src: string): TypeMatch | null {
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "paren", type: type! };
}

function _Type$typeParams(c: readonly CstChild[], src: string): TypeMatch | null {
  let typeParams: (TypeParamsNode) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "TypeParams")) { _t1 = false; break _b2; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t3 = i; let _t6 = true;
    _b7: {
      if (!__nodeOf(c, i, "Param")) { _t6 = false; break _b7; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t6) { i = _t3; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t32 = i; let _t4 = true;
      _b5: {
        if (!__nodeOf(c, i, "Param")) { _t4 = false; break _b5; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t4) { i = _t32; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (!__lit(c, i, src, "=>", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "typeParams", typeParams, param, type: type! };
}

function _Type$abstract(c: readonly CstChild[], src: string): TypeMatch | null {
  let abstractTok: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "abstract", "$keyword")) { _t1 = false; break _b2; }
      abstractTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (!__lit(c, i, src, "new", "$keyword")) return null;
  i++;
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__nodeOf(c, i, "TypeParams")) { _t4 = false; break _b5; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t6 = i; let _t9 = true;
    _b10: {
      if (!__nodeOf(c, i, "Param")) { _t9 = false; break _b10; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t9) { i = _t6; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t62 = i; let _t7 = true;
      _b8: {
        if (!__nodeOf(c, i, "Param")) { _t7 = false; break _b8; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t7) { i = _t62; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (!__lit(c, i, src, "=>", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "abstract", abstractTok, typeParams, param, type: type! };
}

function _Type$bracket(c: readonly CstChild[], src: string): TypeMatch | null {
  const ident: (CstLeaf)[] = [];
  const type: (TypeNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "[", "$punct")) return null;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      {
        const _t3 = i; let _t4 = true;
        _b5: {
          if (!__lit(c, i, src, "...", "$punct")) { _t4 = false; break _b5; }
          i++;
        }
        if (!_t4) i = _t3;
      }
      {
        const _t6 = i; let _t7 = true;
        _b8: {
          if (!(__tok(c, i, "Ident"))) { _t7 = false; break _b8; }
          ident.push(c[i] as CstLeaf);
          i++;
          {
            const _t9 = i; let _t10 = true;
            _b11: {
              if (!__lit(c, i, src, "?", "$punct")) { _t10 = false; break _b11; }
              i++;
            }
            if (!_t10) i = _t9;
          }
          if (!__lit(c, i, src, ":", "$punct")) { _t7 = false; break _b8; }
          i++;
        }
        if (!_t7) i = _t6;
      }
      {
        const _t12 = i; let _t13 = true;
        _b14: {
          if (!__lit(c, i, src, "...", "$punct")) { _t13 = false; break _b14; }
          i++;
        }
        if (!_t13) i = _t12;
      }
      if (!__nodeOf(c, i, "Type")) { _t1 = false; break _b2; }
      type.push(c[i] as TypeNode);
      i++;
      {
        const _t15 = i; let _t16 = true;
        _b17: {
          if (!__lit(c, i, src, "?", "$punct")) { _t16 = false; break _b17; }
          i++;
        }
        if (!_t16) i = _t15;
      }
      {
        const _t18 = i; let _t19 = true;
        _b20: {
          if (!__lit(c, i, src, ",", "$punct")) { _t19 = false; break _b20; }
          i++;
        }
        if (!_t19) i = _t18;
      }
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, "]", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "bracket", ident, type };
}

function _Type$brace(c: readonly CstChild[], src: string): TypeMatch | null {
  const typeMember: (TypeMemberNode)[] = [];
  const _Kw: (";" | ",")[] = [];
  let i = 0;
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "TypeMember")) { _t1 = false; break _b2; }
      typeMember.push(c[i] as TypeMemberNode);
      i++;
      {
        const _t3 = i; let _t4 = true;
        _b5: {
          if (!(__lit(c, i, src, ";", "$punct") || __lit(c, i, src, ",", "$punct"))) { _t4 = false; break _b5; }
          _Kw.push(src.slice(c[i].offset, c[i].end) as ";" | ",");
          i++;
        }
        if (!_t4) i = _t3;
      }
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "brace", typeMember, _Kw };
}

function _Type$asserts(c: readonly CstChild[], src: string): TypeMatch | null {
  let ident: (CstLeaf) | undefined;
  let type: (TypeNode) | undefined;
  let isTok: (CstLeaf) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "asserts", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "is", "$keyword")) { _t1 = false; break _b2; }
      isTok = c[i] as CstLeaf;
      i++;
      if (!__nodeOf(c, i, "Type")) { _t1 = false; break _b2; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (i !== c.length) return null;
  return { arm: "asserts", ident: ident!, type, isTok };
}

function _Type$led_extends(c: readonly CstChild[], src: string): TypeMatch | null {
  let left: (TypeNode) | undefined;
  let type: (TypeNode) | undefined;
  let type2: (TypeNode) | undefined;
  let type3: (TypeNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Type")) return null;
  left = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, "extends", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, "?", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type2 = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type3 = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_extends", left: left!, type: type!, type2: type2!, type3: type3! };
}

function _Type$infer(c: readonly CstChild[], src: string): TypeMatch | null {
  let ident: (CstLeaf) | undefined;
  let type: (TypeNode) | undefined;
  let type2: (TypeNode) | undefined;
  let type3: (TypeNode) | undefined;
  let extendsTok: (CstLeaf) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "infer", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "extends", "$keyword")) { _t1 = false; break _b2; }
      extendsTok = c[i] as CstLeaf;
      i++;
      if (!__nodeOf(c, i, "Type")) { _t1 = false; break _b2; }
      type = c[i] as TypeNode;
      i++;
      {
        const _t3 = i; let _t4 = true;
        _b5: {
          if (!__lit(c, i, src, "?", "$punct")) { _t4 = false; break _b5; }
          i++;
          if (!__nodeOf(c, i, "Type")) { _t4 = false; break _b5; }
          type2 = c[i] as TypeNode;
          i++;
          if (!__lit(c, i, src, ":", "$punct")) { _t4 = false; break _b5; }
          i++;
          if (!__nodeOf(c, i, "Type")) { _t4 = false; break _b5; }
          type3 = c[i] as TypeNode;
          i++;
        }
        if (!_t4) i = _t3;
      }
    }
    if (!_t1) i = _t0;
  }
  if (i !== c.length) return null;
  return { arm: "infer", ident: ident!, type, type2, type3, extendsTok };
}

function _Type$string(c: readonly CstChild[], src: string): TypeMatch | null {
  let string: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "String"))) return null;
  string = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "string", string: string! };
}

function _Type$number(c: readonly CstChild[], src: string): TypeMatch | null {
  let number: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Number"))) return null;
  number = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "number", number: number! };
}

function _Type$hexNumber(c: readonly CstChild[], src: string): TypeMatch | null {
  let hexNumber: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "HexNumber"))) return null;
  hexNumber = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "hexNumber", hexNumber: hexNumber! };
}

function _Type$octalNumber(c: readonly CstChild[], src: string): TypeMatch | null {
  let octalNumber: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "OctalNumber"))) return null;
  octalNumber = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "octalNumber", octalNumber: octalNumber! };
}

function _Type$binaryNumber(c: readonly CstChild[], src: string): TypeMatch | null {
  let binaryNumber: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "BinaryNumber"))) return null;
  binaryNumber = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "binaryNumber", binaryNumber: binaryNumber! };
}

function _Type$bigInt(c: readonly CstChild[], src: string): TypeMatch | null {
  let bigInt: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "BigInt"))) return null;
  bigInt = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "bigInt", bigInt: bigInt! };
}

function _Type$dash(c: readonly CstChild[], src: string): TypeMatch | null {
  let alt: ({ branch: "number"; number: CstLeaf } | { branch: "bigInt"; bigInt: CstLeaf }) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "-", "$punct")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_number: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "Number"))) { _t2 = false; break _b3; }
        _t4_number = c[i] as CstLeaf;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "number", number: _t4_number! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_bigInt: (CstLeaf) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!(__tok(c, i, "BigInt"))) { _t6 = false; break _b7; }
        _t8_bigInt = c[i] as CstLeaf;
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "bigInt", bigInt: _t8_bigInt! }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) return null;
  }
  if (i !== c.length) return null;
  return { arm: "dash", alt: alt! };
}

function _Type$true_(c: readonly CstChild[], src: string): TypeMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "true", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "true_" };
}

function _Type$false_(c: readonly CstChild[], src: string): TypeMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "false", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "false_" };
}

function _Type$null_(c: readonly CstChild[], src: string): TypeMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "null", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "null_" };
}

function _Type$undefined(c: readonly CstChild[], src: string): TypeMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "undefined", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "undefined" };
}

function _Type$void_(c: readonly CstChild[], src: string): TypeMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "void", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "void_" };
}

function _Type$this_(c: readonly CstChild[], src: string): TypeMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "this", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "this_" };
}

function _Type$unique(c: readonly CstChild[], src: string): TypeMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "unique", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "symbol", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "unique" };
}

function _Type$import_(c: readonly CstChild[], src: string): TypeMatch | null {
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "import", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "import_", type: type! };
}

function _Type$template(c: readonly CstChild[], src: string): TypeMatch | null {
  let template: (CstLeaf | $templateNode) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Template") || __nodeOf(c, i, '$template'))) return null;
  template = c[i] as CstLeaf | $templateNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "template", template: template! };
}

function _Type$led_bracket2(c: readonly CstChild[], src: string): TypeMatch | null {
  let left: (TypeNode) | undefined;
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Type")) return null;
  left = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, "[", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, "]", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_bracket2", left: left!, type: type! };
}

function _Type$led_dot(c: readonly CstChild[], src: string): TypeMatch | null {
  let left: (TypeNode) | undefined;
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Type")) return null;
  left = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, ".", "$punct")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_dot", left: left!, ident: ident! };
}

function _Type$led_dot2(c: readonly CstChild[], src: string): TypeMatch | null {
  let left: (TypeNode) | undefined;
  const type: (TypeNode)[] = [];
  let i = 0;
  if (!__nodeOf(c, i, "Type")) return null;
  left = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, ".", "$punct")) return null;
  i++;
  if (!__lit(c, i, src, "<", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "Type")) { _t3 = false; break _b4; }
      type.push(c[i] as TypeNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "Type")) { _t1 = false; break _b2; }
        type.push(c[i] as TypeNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, ">", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_dot2", left: left!, type };
}

function _Type$question(c: readonly CstChild[], src: string): TypeMatch | null {
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "?", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "question", type: type! };
}

function _Type$bang(c: readonly CstChild[], src: string): TypeMatch | null {
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "!", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "bang", type: type! };
}

function _Type$question2(c: readonly CstChild[], src: string): TypeMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "?", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "question2" };
}

function _Type$star(c: readonly CstChild[], src: string): TypeMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "*", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "star" };
}

function _Type$function_(c: readonly CstChild[], src: string): TypeMatch | null {
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "function", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "Param")) { _t3 = false; break _b4; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "Param")) { _t1 = false; break _b2; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  {
    const _t5 = i; let _t6 = true;
    _b7: {
      if (!__lit(c, i, src, ":", "$punct")) { _t6 = false; break _b7; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t6 = false; break _b7; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t6) i = _t5;
  }
  if (i !== c.length) return null;
  return { arm: "function_", param, type };
}

function _Type$led_question(c: readonly CstChild[], src: string): TypeMatch | null {
  let left: (TypeNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Type")) return null;
  left = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, "?", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_question", left: left! };
}

function _Type$led_bang(c: readonly CstChild[], src: string): TypeMatch | null {
  let left: (TypeNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Type")) return null;
  left = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, "!", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_bang", left: left! };
}

export function matchType(n: TypeNode, src: string): TypeMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "$template": {
        { const m = _Type$template(c, src); if (m !== null) return m; }
        break;
      }
      case "Type": {
        const k1 = c[1] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
        if (k1 === undefined) {
        } else if (k1.tokenType === undefined) {
          switch (k1.rule) {
            default: {
              break;
            }
          }
        } else if (k1.tokenType === '$keyword' || k1.tokenType === '$punct') {
          switch (src.charCodeAt(k1.offset)) {
            case 33: {
            { const m = _Type$led_bang(c, src); if (m !== null) return m; }
              break;
            }
            case 38: {
            { const m = _Type$led_amp(c, src); if (m !== null) return m; }
              break;
            }
            case 46: {
            { const m = _Type$led_dot(c, src); if (m !== null) return m; }
            { const m = _Type$led_dot2(c, src); if (m !== null) return m; }
              break;
            }
            case 60: {
            { const m = _Type$led_lt(c, src); if (m !== null) return m; }
              break;
            }
            case 63: {
            { const m = _Type$led_question(c, src); if (m !== null) return m; }
              break;
            }
            case 91: {
            { const m = _Type$led_bracket(c, src); if (m !== null) return m; }
            { const m = _Type$led_bracket2(c, src); if (m !== null) return m; }
              break;
            }
            case 101: {
            { const m = _Type$led_extends(c, src); if (m !== null) return m; }
              break;
            }
            case 124: {
            { const m = _Type$led_pipe(c, src); if (m !== null) return m; }
              break;
            }
            default: {
              break;
            }
          }
        } else {
          switch (k1.tokenType) {
            default: {
              break;
            }
          }
        }
        break;
      }
      case "TypeParams": {
        { const m = _Type$typeParams(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 33: {
        { const m = _Type$bang(c, src); if (m !== null) return m; }
        break;
      }
      case 38: {
        { const m = _Type$amp(c, src); if (m !== null) return m; }
        break;
      }
      case 40: {
        { const m = _Type$paren(c, src); if (m !== null) return m; }
        { const m = _Type$typeParams(c, src); if (m !== null) return m; }
        break;
      }
      case 42: {
        { const m = _Type$star(c, src); if (m !== null) return m; }
        break;
      }
      case 45: {
        { const m = _Type$dash(c, src); if (m !== null) return m; }
        break;
      }
      case 63: {
        { const m = _Type$question(c, src); if (m !== null) return m; }
        { const m = _Type$question2(c, src); if (m !== null) return m; }
        break;
      }
      case 91: {
        { const m = _Type$bracket(c, src); if (m !== null) return m; }
        break;
      }
      case 97: {
        { const m = _Type$abstract(c, src); if (m !== null) return m; }
        { const m = _Type$asserts(c, src); if (m !== null) return m; }
        break;
      }
      case 102: {
        { const m = _Type$false_(c, src); if (m !== null) return m; }
        { const m = _Type$function_(c, src); if (m !== null) return m; }
        break;
      }
      case 105: {
        { const m = _Type$infer(c, src); if (m !== null) return m; }
        { const m = _Type$import_(c, src); if (m !== null) return m; }
        break;
      }
      case 107: {
        { const m = _Type$keyof(c, src); if (m !== null) return m; }
        break;
      }
      case 110: {
        { const m = _Type$abstract(c, src); if (m !== null) return m; }
        { const m = _Type$null_(c, src); if (m !== null) return m; }
        break;
      }
      case 114: {
        { const m = _Type$readonly(c, src); if (m !== null) return m; }
        break;
      }
      case 116: {
        { const m = _Type$typeof_(c, src); if (m !== null) return m; }
        { const m = _Type$true_(c, src); if (m !== null) return m; }
        { const m = _Type$this_(c, src); if (m !== null) return m; }
        break;
      }
      case 117: {
        { const m = _Type$undefined(c, src); if (m !== null) return m; }
        { const m = _Type$unique(c, src); if (m !== null) return m; }
        break;
      }
      case 118: {
        { const m = _Type$void_(c, src); if (m !== null) return m; }
        break;
      }
      case 123: {
        { const m = _Type$brace(c, src); if (m !== null) return m; }
        break;
      }
      case 124: {
        { const m = _Type$pipe(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "BigInt": {
        { const m = _Type$bigInt(c, src); if (m !== null) return m; }
        break;
      }
      case "BinaryNumber": {
        { const m = _Type$binaryNumber(c, src); if (m !== null) return m; }
        break;
      }
      case "HexNumber": {
        { const m = _Type$hexNumber(c, src); if (m !== null) return m; }
        break;
      }
      case "Ident": {
        { const m = _Type$ident(c, src); if (m !== null) return m; }
        break;
      }
      case "Number": {
        { const m = _Type$number(c, src); if (m !== null) return m; }
        break;
      }
      case "OctalNumber": {
        { const m = _Type$octalNumber(c, src); if (m !== null) return m; }
        break;
      }
      case "String": {
        { const m = _Type$string(c, src); if (m !== null) return m; }
        break;
      }
      case "Template": {
        { const m = _Type$template(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchType: no arm matches" + ' @' + n.offset);
}

export type TypeMemberMatch =
  | { arm: "new_"; newTok?: CstLeaf; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode }
  | { arm: "_"; _Kw?: "+" | "-"; readonlyTok?: CstLeaf; alt: { branch: "ident"; ident: CstLeaf; alt: { branch: "in_"; type: TypeNode; type2?: TypeNode; asTok?: CstLeaf; _Kw?: "+" | "-"; type3: TypeNode } | { branch: "colon"; type: TypeNode; type2?: TypeNode } } | { branch: "expr"; expr: ExprNode; alt: { branch: "typeParams"; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode } | { branch: "seq"; type?: TypeNode } } | { branch: "bracketClose"; type?: TypeNode } }
  | { arm: "readonly"; ident: CstLeaf; type: TypeNode }
  | { arm: "ident"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "number"; number: CstLeaf } | { branch: "string"; string: CstLeaf } | { branch: "privateField"; privateField: CstLeaf }; alt2: { branch: "typeParams"; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode } | { branch: "seq"; type?: TypeNode } };

function _TypeMember$new_(c: readonly CstChild[], src: string): TypeMemberMatch | null {
  let newTok: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "new", "$keyword")) { _t1 = false; break _b2; }
      newTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__nodeOf(c, i, "TypeParams")) { _t4 = false; break _b5; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t6 = i; let _t9 = true;
    _b10: {
      if (!__nodeOf(c, i, "Param")) { _t9 = false; break _b10; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t9) { i = _t6; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t62 = i; let _t7 = true;
      _b8: {
        if (!__nodeOf(c, i, "Param")) { _t7 = false; break _b8; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t7) { i = _t62; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  {
    const _t11 = i; let _t12 = true;
    _b13: {
      if (!__lit(c, i, src, ":", "$punct")) { _t12 = false; break _b13; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t12 = false; break _b13; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t12) i = _t11;
  }
  if (i !== c.length) return null;
  return { arm: "new_", newTok, typeParams, param, type };
}

function _TypeMember$_(c: readonly CstChild[], src: string): TypeMemberMatch | null {
  let _Kw: ("+" | "-") | undefined;
  let readonlyTok: (CstLeaf) | undefined;
  let alt: ({ branch: "ident"; ident: CstLeaf; alt: { branch: "in_"; type: TypeNode; type2?: TypeNode; asTok?: CstLeaf; _Kw?: "+" | "-"; type3: TypeNode } | { branch: "colon"; type: TypeNode; type2?: TypeNode } } | { branch: "expr"; expr: ExprNode; alt: { branch: "typeParams"; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode } | { branch: "seq"; type?: TypeNode } } | { branch: "bracketClose"; type?: TypeNode }) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!(__lit(c, i, src, "+", "$punct") || __lit(c, i, src, "-", "$punct"))) { _t1 = false; break _b2; }
      _Kw = src.slice(c[i].offset, c[i].end) as "+" | "-";
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "readonly", "$keyword")) { _t4 = false; break _b5; }
      readonlyTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, "[", "$punct")) return null;
  i++;
  {
    let _t6 = false;
    if (!_t6) {
      let _t10_ident: (CstLeaf) | undefined;
      let _t10_alt: ({ branch: "in_"; type: TypeNode; type2?: TypeNode; asTok?: CstLeaf; _Kw?: "+" | "-"; type3: TypeNode } | { branch: "colon"; type: TypeNode; type2?: TypeNode }) | undefined;
      const _t7 = i; let _t8 = true;
      _b9: {
        if (!(__tok(c, i, "Ident"))) { _t8 = false; break _b9; }
        _t10_ident = c[i] as CstLeaf;
        i++;
        {
          let _t11 = false;
          if (!_t11) {
            let _t15__t10_type: (TypeNode) | undefined;
            let _t15__t10_type2: (TypeNode) | undefined;
            let _t15__t10_asTok: (CstLeaf) | undefined;
            let _t15__t10__Kw: ("+" | "-") | undefined;
            let _t15__t10_type3: (TypeNode) | undefined;
            const _t12 = i; let _t13 = true;
            _b14: {
              if (!__lit(c, i, src, "in", "$keyword")) { _t13 = false; break _b14; }
              i++;
              if (!__nodeOf(c, i, "Type")) { _t13 = false; break _b14; }
              _t15__t10_type = c[i] as TypeNode;
              i++;
              {
                const _t16 = i; let _t17 = true;
                _b18: {
                  if (!__lit(c, i, src, "as", "$keyword")) { _t17 = false; break _b18; }
                  _t15__t10_asTok = c[i] as CstLeaf;
                  i++;
                  if (!__nodeOf(c, i, "Type")) { _t17 = false; break _b18; }
                  _t15__t10_type2 = c[i] as TypeNode;
                  i++;
                }
                if (!_t17) i = _t16;
              }
              if (!__lit(c, i, src, "]", "$punct")) { _t13 = false; break _b14; }
              i++;
              {
                const _t19 = i; let _t20 = true;
                _b21: {
                  if (!(__lit(c, i, src, "+", "$punct") || __lit(c, i, src, "-", "$punct"))) { _t20 = false; break _b21; }
                  _t15__t10__Kw = src.slice(c[i].offset, c[i].end) as "+" | "-";
                  i++;
                }
                if (!_t20) i = _t19;
              }
              {
                const _t22 = i; let _t23 = true;
                _b24: {
                  if (!__lit(c, i, src, "?", "$punct")) { _t23 = false; break _b24; }
                  i++;
                }
                if (!_t23) i = _t22;
              }
              if (!__lit(c, i, src, ":", "$punct")) { _t13 = false; break _b14; }
              i++;
              if (!__nodeOf(c, i, "Type")) { _t13 = false; break _b14; }
              _t15__t10_type3 = c[i] as TypeNode;
              i++;
            }
            if (_t13) { _t11 = true; _t10_alt = ({ branch: "in_", type: _t15__t10_type!, type2: _t15__t10_type2, asTok: _t15__t10_asTok, _Kw: _t15__t10__Kw, type3: _t15__t10_type3! }) as typeof _t10_alt; }
            else i = _t12;
          }
          if (!_t11) {
            let _t28__t10_type: (TypeNode) | undefined;
            let _t28__t10_type2: (TypeNode) | undefined;
            const _t25 = i; let _t26 = true;
            _b27: {
              if (!__lit(c, i, src, ":", "$punct")) { _t26 = false; break _b27; }
              i++;
              if (!__nodeOf(c, i, "Type")) { _t26 = false; break _b27; }
              _t28__t10_type = c[i] as TypeNode;
              i++;
              if (!__lit(c, i, src, "]", "$punct")) { _t26 = false; break _b27; }
              i++;
              {
                const _t29 = i; let _t30 = true;
                _b31: {
                  if (!__lit(c, i, src, ":", "$punct")) { _t30 = false; break _b31; }
                  i++;
                  if (!__nodeOf(c, i, "Type")) { _t30 = false; break _b31; }
                  _t28__t10_type2 = c[i] as TypeNode;
                  i++;
                }
                if (!_t30) i = _t29;
              }
            }
            if (_t26) { _t11 = true; _t10_alt = ({ branch: "colon", type: _t28__t10_type!, type2: _t28__t10_type2 }) as typeof _t10_alt; }
            else i = _t25;
          }
          if (!_t11) { _t8 = false; break _b9; }
        }
      }
      if (_t8) { _t6 = true; alt = ({ branch: "ident", ident: _t10_ident!, alt: _t10_alt! }) as typeof alt; }
      else i = _t7;
    }
    if (!_t6) {
      let _t35_expr: (ExprNode) | undefined;
      let _t35_alt: ({ branch: "typeParams"; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode } | { branch: "seq"; type?: TypeNode }) | undefined;
      const _t32 = i; let _t33 = true;
      _b34: {
        if (!__nodeOf(c, i, "Expr")) { _t33 = false; break _b34; }
        _t35_expr = c[i] as ExprNode;
        i++;
        if (!__lit(c, i, src, "]", "$punct")) { _t33 = false; break _b34; }
        i++;
        {
          const _t36 = i; let _t37 = true;
          _b38: {
            if (!__lit(c, i, src, "?", "$punct")) { _t37 = false; break _b38; }
            i++;
          }
          if (!_t37) i = _t36;
        }
        {
          let _t39 = false;
          if (!_t39) {
            let _t43__t35_typeParams: (TypeParamsNode) | undefined;
            const _t43__t35_param: (ParamNode)[] = [];
            let _t43__t35_type: (TypeNode) | undefined;
            const _t40 = i; let _t41 = true;
            _b42: {
              {
                const _t44 = i; let _t45 = true;
                _b46: {
                  if (!__nodeOf(c, i, "TypeParams")) { _t45 = false; break _b46; }
                  _t43__t35_typeParams = c[i] as TypeParamsNode;
                  i++;
                }
                if (!_t45) i = _t44;
              }
              if (!__lit(c, i, src, "(", "$punct")) { _t41 = false; break _b42; }
              i++;
              {
                const _t47 = i; let _t50 = true;
                _b51: {
                  if (!__nodeOf(c, i, "Param")) { _t50 = false; break _b51; }
                  _t43__t35_param.push(c[i] as ParamNode);
                  i++;
                }
                if (!_t50) { i = _t47; }
                else for (;;) {
                  if (!__lit(c, i, src, ",", "$punct")) break;
                  i++;
                  const _t472 = i; let _t48 = true;
                  _b49: {
                    if (!__nodeOf(c, i, "Param")) { _t48 = false; break _b49; }
                    _t43__t35_param.push(c[i] as ParamNode);
                    i++;
                  }
                  if (!_t48) { i = _t472; break; }
                }
              }
              if (!__lit(c, i, src, ")", "$punct")) { _t41 = false; break _b42; }
              i++;
              {
                const _t52 = i; let _t53 = true;
                _b54: {
                  if (!__lit(c, i, src, ":", "$punct")) { _t53 = false; break _b54; }
                  i++;
                  if (!__nodeOf(c, i, "Type")) { _t53 = false; break _b54; }
                  _t43__t35_type = c[i] as TypeNode;
                  i++;
                }
                if (!_t53) i = _t52;
              }
            }
            if (_t41) { _t39 = true; _t35_alt = ({ branch: "typeParams", typeParams: _t43__t35_typeParams, param: _t43__t35_param, type: _t43__t35_type }) as typeof _t35_alt; }
            else i = _t40;
          }
          if (!_t39) {
            let _t58__t35_type: (TypeNode) | undefined;
            const _t55 = i; let _t56 = true;
            _b57: {
              {
                const _t59 = i; let _t60 = true;
                _b61: {
                  if (!__lit(c, i, src, ":", "$punct")) { _t60 = false; break _b61; }
                  i++;
                  if (!__nodeOf(c, i, "Type")) { _t60 = false; break _b61; }
                  _t58__t35_type = c[i] as TypeNode;
                  i++;
                }
                if (!_t60) i = _t59;
              }
            }
            if (_t56) { _t39 = true; _t35_alt = ({ branch: "seq", type: _t58__t35_type }) as typeof _t35_alt; }
            else i = _t55;
          }
          if (!_t39) { _t33 = false; break _b34; }
        }
      }
      if (_t33) { _t6 = true; alt = ({ branch: "expr", expr: _t35_expr!, alt: _t35_alt! }) as typeof alt; }
      else i = _t32;
    }
    if (!_t6) {
      let _t65_type: (TypeNode) | undefined;
      const _t62 = i; let _t63 = true;
      _b64: {
        if (!__lit(c, i, src, "]", "$punct")) { _t63 = false; break _b64; }
        i++;
        {
          const _t66 = i; let _t67 = true;
          _b68: {
            if (!__lit(c, i, src, ":", "$punct")) { _t67 = false; break _b68; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t67 = false; break _b68; }
            _t65_type = c[i] as TypeNode;
            i++;
          }
          if (!_t67) i = _t66;
        }
      }
      if (_t63) { _t6 = true; alt = ({ branch: "bracketClose", type: _t65_type }) as typeof alt; }
      else i = _t62;
    }
    if (!_t6) return null;
  }
  if (i !== c.length) return null;
  return { arm: "_", _Kw, readonlyTok, alt: alt! };
}

function _TypeMember$readonly(c: readonly CstChild[], src: string): TypeMemberMatch | null {
  let ident: (CstLeaf) | undefined;
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "readonly", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "?", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "readonly", ident: ident!, type: type! };
}

function _TypeMember$ident(c: readonly CstChild[], src: string): TypeMemberMatch | null {
  let alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "number"; number: CstLeaf } | { branch: "string"; string: CstLeaf } | { branch: "privateField"; privateField: CstLeaf }) | undefined;
  let alt2: ({ branch: "typeParams"; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode } | { branch: "seq"; type?: TypeNode }) | undefined;
  let i = 0;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_ident: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "Ident"))) { _t2 = false; break _b3; }
        _t4_ident = c[i] as CstLeaf;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "ident", ident: _t4_ident! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_number: (CstLeaf) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!(__tok(c, i, "Number"))) { _t6 = false; break _b7; }
        _t8_number = c[i] as CstLeaf;
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "number", number: _t8_number! }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) {
      let _t12_string: (CstLeaf) | undefined;
      const _t9 = i; let _t10 = true;
      _b11: {
        if (!(__tok(c, i, "String"))) { _t10 = false; break _b11; }
        _t12_string = c[i] as CstLeaf;
        i++;
      }
      if (_t10) { _t0 = true; alt = ({ branch: "string", string: _t12_string! }) as typeof alt; }
      else i = _t9;
    }
    if (!_t0) {
      let _t16_privateField: (CstLeaf) | undefined;
      const _t13 = i; let _t14 = true;
      _b15: {
        if (!(__tok(c, i, "PrivateField"))) { _t14 = false; break _b15; }
        _t16_privateField = c[i] as CstLeaf;
        i++;
      }
      if (_t14) { _t0 = true; alt = ({ branch: "privateField", privateField: _t16_privateField! }) as typeof alt; }
      else i = _t13;
    }
    if (!_t0) return null;
  }
  {
    const _t17 = i; let _t18 = true;
    _b19: {
      if (!__lit(c, i, src, "?", "$punct")) { _t18 = false; break _b19; }
      i++;
    }
    if (!_t18) i = _t17;
  }
  {
    let _t20 = false;
    if (!_t20) {
      let _t24_typeParams: (TypeParamsNode) | undefined;
      const _t24_param: (ParamNode)[] = [];
      let _t24_type: (TypeNode) | undefined;
      const _t21 = i; let _t22 = true;
      _b23: {
        {
          const _t25 = i; let _t26 = true;
          _b27: {
            if (!__nodeOf(c, i, "TypeParams")) { _t26 = false; break _b27; }
            _t24_typeParams = c[i] as TypeParamsNode;
            i++;
          }
          if (!_t26) i = _t25;
        }
        if (!__lit(c, i, src, "(", "$punct")) { _t22 = false; break _b23; }
        i++;
        {
          const _t28 = i; let _t31 = true;
          _b32: {
            if (!__nodeOf(c, i, "Param")) { _t31 = false; break _b32; }
            _t24_param.push(c[i] as ParamNode);
            i++;
          }
          if (!_t31) { i = _t28; }
          else for (;;) {
            if (!__lit(c, i, src, ",", "$punct")) break;
            i++;
            const _t282 = i; let _t29 = true;
            _b30: {
              if (!__nodeOf(c, i, "Param")) { _t29 = false; break _b30; }
              _t24_param.push(c[i] as ParamNode);
              i++;
            }
            if (!_t29) { i = _t282; break; }
          }
        }
        if (!__lit(c, i, src, ")", "$punct")) { _t22 = false; break _b23; }
        i++;
        {
          const _t33 = i; let _t34 = true;
          _b35: {
            if (!__lit(c, i, src, ":", "$punct")) { _t34 = false; break _b35; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t34 = false; break _b35; }
            _t24_type = c[i] as TypeNode;
            i++;
          }
          if (!_t34) i = _t33;
        }
      }
      if (_t22) { _t20 = true; alt2 = ({ branch: "typeParams", typeParams: _t24_typeParams, param: _t24_param, type: _t24_type }) as typeof alt2; }
      else i = _t21;
    }
    if (!_t20) {
      let _t39_type: (TypeNode) | undefined;
      const _t36 = i; let _t37 = true;
      _b38: {
        {
          const _t40 = i; let _t41 = true;
          _b42: {
            if (!__lit(c, i, src, ":", "$punct")) { _t41 = false; break _b42; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t41 = false; break _b42; }
            _t39_type = c[i] as TypeNode;
            i++;
          }
          if (!_t41) i = _t40;
        }
      }
      if (_t37) { _t20 = true; alt2 = ({ branch: "seq", type: _t39_type }) as typeof alt2; }
      else i = _t36;
    }
    if (!_t20) return null;
  }
  if (i !== c.length) return null;
  return { arm: "ident", alt: alt!, alt2: alt2! };
}

export function matchTypeMember(n: TypeMemberNode, src: string): TypeMemberMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "TypeParams": {
        { const m = _TypeMember$new_(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 40: {
        { const m = _TypeMember$new_(c, src); if (m !== null) return m; }
        break;
      }
      case 43: {
        { const m = _TypeMember$_(c, src); if (m !== null) return m; }
        break;
      }
      case 45: {
        { const m = _TypeMember$_(c, src); if (m !== null) return m; }
        break;
      }
      case 91: {
        { const m = _TypeMember$_(c, src); if (m !== null) return m; }
        break;
      }
      case 110: {
        { const m = _TypeMember$new_(c, src); if (m !== null) return m; }
        break;
      }
      case 114: {
        { const m = _TypeMember$_(c, src); if (m !== null) return m; }
        { const m = _TypeMember$readonly(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _TypeMember$ident(c, src); if (m !== null) return m; }
        break;
      }
      case "Number": {
        { const m = _TypeMember$ident(c, src); if (m !== null) return m; }
        break;
      }
      case "PrivateField": {
        { const m = _TypeMember$ident(c, src); if (m !== null) return m; }
        break;
      }
      case "String": {
        { const m = _TypeMember$ident(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchTypeMember: no arm matches" + ' @' + n.offset);
}

export type DecoratorExprMatch =
  | { arm: "decorator"; decorator: CstLeaf; alt: ({ branch: "paren"; expr: (ExprNode)[] } | { branch: "lt"; type: (TypeNode)[]; expr: (ExprNode)[] } | { branch: "bang" } | { branch: "dot"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "privateField"; privateField: CstLeaf } } | { branch: "optChain"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "privateField"; privateField: CstLeaf } | { branch: "paren"; expr: (ExprNode)[] } | { branch: "bracket"; expr: ExprNode } } | { branch: "template"; template: CstLeaf | $templateNode })[] }
  | { arm: "_new"; newTarget: NewTargetNode; expr: (ExprNode)[] };

function _DecoratorExpr$decorator(c: readonly CstChild[], src: string): DecoratorExprMatch | null {
  let decorator: (CstLeaf) | undefined;
  const alt: ({ branch: "paren"; expr: (ExprNode)[] } | { branch: "lt"; type: (TypeNode)[]; expr: (ExprNode)[] } | { branch: "bang" } | { branch: "dot"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "privateField"; privateField: CstLeaf } } | { branch: "optChain"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "privateField"; privateField: CstLeaf } | { branch: "paren"; expr: (ExprNode)[] } | { branch: "bracket"; expr: ExprNode } } | { branch: "template"; template: CstLeaf | $templateNode })[] = [];
  let i = 0;
  if (!(__tok(c, i, "Decorator"))) return null;
  decorator = c[i] as CstLeaf;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      {
        let _t3 = false;
        if (!_t3) {
          const _t7_expr: (ExprNode)[] = [];
          const _t4 = i; let _t5 = true;
          _b6: {
            if (!__lit(c, i, src, "(", "$punct")) { _t5 = false; break _b6; }
            i++;
            {
              const _t8 = i; let _t11 = true;
              _b12: {
                if (!__nodeOf(c, i, "Expr")) { _t11 = false; break _b12; }
                _t7_expr.push(c[i] as ExprNode);
                i++;
              }
              if (!_t11) { i = _t8; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t82 = i; let _t9 = true;
                _b10: {
                  if (!__nodeOf(c, i, "Expr")) { _t9 = false; break _b10; }
                  _t7_expr.push(c[i] as ExprNode);
                  i++;
                }
                if (!_t9) { i = _t82; break; }
              }
            }
            if (!__lit(c, i, src, ")", "$punct")) { _t5 = false; break _b6; }
            i++;
          }
          if (_t5) { _t3 = true; alt.push({ branch: "paren", expr: _t7_expr } as never); }
          else i = _t4;
        }
        if (!_t3) {
          const _t16_type: (TypeNode)[] = [];
          const _t16_expr: (ExprNode)[] = [];
          const _t13 = i; let _t14 = true;
          _b15: {
            if (!__lit(c, i, src, "<", "$punct")) { _t14 = false; break _b15; }
            i++;
            {
              const _t17 = i; let _t20 = true;
              _b21: {
                if (!__nodeOf(c, i, "Type")) { _t20 = false; break _b21; }
                _t16_type.push(c[i] as TypeNode);
                i++;
              }
              if (!_t20) { i = _t17; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t172 = i; let _t18 = true;
                _b19: {
                  if (!__nodeOf(c, i, "Type")) { _t18 = false; break _b19; }
                  _t16_type.push(c[i] as TypeNode);
                  i++;
                }
                if (!_t18) { i = _t172; break; }
              }
            }
            if (!__lit(c, i, src, ">", "$punct")) { _t14 = false; break _b15; }
            i++;
            if (!__lit(c, i, src, "(", "$punct")) { _t14 = false; break _b15; }
            i++;
            {
              const _t22 = i; let _t25 = true;
              _b26: {
                if (!__nodeOf(c, i, "Expr")) { _t25 = false; break _b26; }
                _t16_expr.push(c[i] as ExprNode);
                i++;
              }
              if (!_t25) { i = _t22; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t222 = i; let _t23 = true;
                _b24: {
                  if (!__nodeOf(c, i, "Expr")) { _t23 = false; break _b24; }
                  _t16_expr.push(c[i] as ExprNode);
                  i++;
                }
                if (!_t23) { i = _t222; break; }
              }
            }
            if (!__lit(c, i, src, ")", "$punct")) { _t14 = false; break _b15; }
            i++;
          }
          if (_t14) { _t3 = true; alt.push({ branch: "lt", type: _t16_type, expr: _t16_expr } as never); }
          else i = _t13;
        }
        if (!_t3) {
          const _t27 = i; let _t28 = true;
          _b29: {
            if (!__lit(c, i, src, "!", "$punct")) { _t28 = false; break _b29; }
            i++;
          }
          if (_t28) { _t3 = true; alt.push({ branch: "bang" } as never); }
          else i = _t27;
        }
        if (!_t3) {
          let _t34_alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "privateField"; privateField: CstLeaf }) | undefined;
          const _t31 = i; let _t32 = true;
          _b33: {
            if (!__lit(c, i, src, ".", "$punct")) { _t32 = false; break _b33; }
            i++;
            {
              let _t35 = false;
              if (!_t35) {
                let _t39__t34_ident: (CstLeaf) | undefined;
                const _t36 = i; let _t37 = true;
                _b38: {
                  if (!(__tok(c, i, "Ident"))) { _t37 = false; break _b38; }
                  _t39__t34_ident = c[i] as CstLeaf;
                  i++;
                }
                if (_t37) { _t35 = true; _t34_alt = ({ branch: "ident", ident: _t39__t34_ident! }) as typeof _t34_alt; }
                else i = _t36;
              }
              if (!_t35) {
                let _t43__t34_privateField: (CstLeaf) | undefined;
                const _t40 = i; let _t41 = true;
                _b42: {
                  if (!(__tok(c, i, "PrivateField"))) { _t41 = false; break _b42; }
                  _t43__t34_privateField = c[i] as CstLeaf;
                  i++;
                }
                if (_t41) { _t35 = true; _t34_alt = ({ branch: "privateField", privateField: _t43__t34_privateField! }) as typeof _t34_alt; }
                else i = _t40;
              }
              if (!_t35) { _t32 = false; break _b33; }
            }
          }
          if (_t32) { _t3 = true; alt.push({ branch: "dot", alt: _t34_alt! } as never); }
          else i = _t31;
        }
        if (!_t3) {
          let _t47_alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "privateField"; privateField: CstLeaf } | { branch: "paren"; expr: (ExprNode)[] } | { branch: "bracket"; expr: ExprNode }) | undefined;
          const _t44 = i; let _t45 = true;
          _b46: {
            if (!__lit(c, i, src, "?.", "$punct")) { _t45 = false; break _b46; }
            i++;
            {
              let _t48 = false;
              if (!_t48) {
                let _t52__t47_ident: (CstLeaf) | undefined;
                const _t49 = i; let _t50 = true;
                _b51: {
                  if (!(__tok(c, i, "Ident"))) { _t50 = false; break _b51; }
                  _t52__t47_ident = c[i] as CstLeaf;
                  i++;
                }
                if (_t50) { _t48 = true; _t47_alt = ({ branch: "ident", ident: _t52__t47_ident! }) as typeof _t47_alt; }
                else i = _t49;
              }
              if (!_t48) {
                let _t56__t47_privateField: (CstLeaf) | undefined;
                const _t53 = i; let _t54 = true;
                _b55: {
                  if (!(__tok(c, i, "PrivateField"))) { _t54 = false; break _b55; }
                  _t56__t47_privateField = c[i] as CstLeaf;
                  i++;
                }
                if (_t54) { _t48 = true; _t47_alt = ({ branch: "privateField", privateField: _t56__t47_privateField! }) as typeof _t47_alt; }
                else i = _t53;
              }
              if (!_t48) {
                const _t60__t47_expr: (ExprNode)[] = [];
                const _t57 = i; let _t58 = true;
                _b59: {
                  if (!__lit(c, i, src, "(", "$punct")) { _t58 = false; break _b59; }
                  i++;
                  {
                    const _t61 = i; let _t64 = true;
                    _b65: {
                      if (!__nodeOf(c, i, "Expr")) { _t64 = false; break _b65; }
                      _t60__t47_expr.push(c[i] as ExprNode);
                      i++;
                    }
                    if (!_t64) { i = _t61; }
                    else for (;;) {
                      if (!__lit(c, i, src, ",", "$punct")) break;
                      i++;
                      const _t612 = i; let _t62 = true;
                      _b63: {
                        if (!__nodeOf(c, i, "Expr")) { _t62 = false; break _b63; }
                        _t60__t47_expr.push(c[i] as ExprNode);
                        i++;
                      }
                      if (!_t62) { i = _t612; break; }
                    }
                  }
                  if (!__lit(c, i, src, ")", "$punct")) { _t58 = false; break _b59; }
                  i++;
                }
                if (_t58) { _t48 = true; _t47_alt = ({ branch: "paren", expr: _t60__t47_expr }) as typeof _t47_alt; }
                else i = _t57;
              }
              if (!_t48) {
                let _t69__t47_expr: (ExprNode) | undefined;
                const _t66 = i; let _t67 = true;
                _b68: {
                  if (!__lit(c, i, src, "[", "$punct")) { _t67 = false; break _b68; }
                  i++;
                  if (!__nodeOf(c, i, "Expr")) { _t67 = false; break _b68; }
                  _t69__t47_expr = c[i] as ExprNode;
                  i++;
                  if (!__lit(c, i, src, "]", "$punct")) { _t67 = false; break _b68; }
                  i++;
                }
                if (_t67) { _t48 = true; _t47_alt = ({ branch: "bracket", expr: _t69__t47_expr! }) as typeof _t47_alt; }
                else i = _t66;
              }
              if (!_t48) { _t45 = false; break _b46; }
            }
          }
          if (_t45) { _t3 = true; alt.push({ branch: "optChain", alt: _t47_alt! } as never); }
          else i = _t44;
        }
        if (!_t3) {
          let _t73_template: (CstLeaf | $templateNode) | undefined;
          const _t70 = i; let _t71 = true;
          _b72: {
            if (!(__tok(c, i, "Template") || __nodeOf(c, i, '$template'))) { _t71 = false; break _b72; }
            _t73_template = c[i] as CstLeaf | $templateNode;
            i++;
          }
          if (_t71) { _t3 = true; alt.push({ branch: "template", template: _t73_template! } as never); }
          else i = _t70;
        }
        if (!_t3) { _t1 = false; break _b2; }
      }
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (i !== c.length) return null;
  return { arm: "decorator", decorator: decorator!, alt };
}

function _DecoratorExpr$_new(c: readonly CstChild[], src: string): DecoratorExprMatch | null {
  let newTarget: (NewTargetNode) | undefined;
  const expr: (ExprNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "@new", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "NewTarget")) return null;
  newTarget = c[i] as NewTargetNode;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "(", "$punct")) { _t1 = false; break _b2; }
      i++;
      {
        const _t3 = i; let _t6 = true;
        _b7: {
          if (!__nodeOf(c, i, "Expr")) { _t6 = false; break _b7; }
          expr.push(c[i] as ExprNode);
          i++;
        }
        if (!_t6) { i = _t3; }
        else for (;;) {
          if (!__lit(c, i, src, ",", "$punct")) break;
          i++;
          const _t32 = i; let _t4 = true;
          _b5: {
            if (!__nodeOf(c, i, "Expr")) { _t4 = false; break _b5; }
            expr.push(c[i] as ExprNode);
            i++;
          }
          if (!_t4) { i = _t32; break; }
        }
      }
      if (!__lit(c, i, src, ")", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (i !== c.length) return null;
  return { arm: "_new", newTarget: newTarget!, expr };
}

export function matchDecoratorExpr(n: DecoratorExprNode, src: string): DecoratorExprMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 64: {
        { const m = _DecoratorExpr$_new(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "Decorator": {
        { const m = _DecoratorExpr$decorator(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchDecoratorExpr: no arm matches" + ' @' + n.offset);
}

export type TypeofRefMatch =
  | { arm: "ident"; ident: CstLeaf }
  | { arm: "import_"; type: TypeNode }
  | { arm: "led_dot"; left: TypeofRefNode; ident: CstLeaf };

function _TypeofRef$ident(c: readonly CstChild[], src: string): TypeofRefMatch | null {
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident! };
}

function _TypeofRef$import_(c: readonly CstChild[], src: string): TypeofRefMatch | null {
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "import", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "import_", type: type! };
}

function _TypeofRef$led_dot(c: readonly CstChild[], src: string): TypeofRefMatch | null {
  let left: (TypeofRefNode) | undefined;
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "TypeofRef")) return null;
  left = c[i] as TypeofRefNode;
  i++;
  if (!__lit(c, i, src, ".", "$punct")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_dot", left: left!, ident: ident! };
}

export function matchTypeofRef(n: TypeofRefNode, src: string): TypeofRefMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "TypeofRef": {
        { const m = _TypeofRef$led_dot(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 105: {
        { const m = _TypeofRef$import_(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _TypeofRef$ident(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchTypeofRef: no arm matches" + ' @' + n.offset);
}

export type ExprMatch =
  | { arm: "jSXElement"; jSXElement: JSXElementNode }
  | { arm: "true_" }
  | { arm: "false_" }
  | { arm: "null_" }
  | { arm: "undefined" }
  | { arm: "this_" }
  | { arm: "super_" }
  | { arm: "ident"; ident: CstLeaf }
  | { arm: "number"; number: CstLeaf }
  | { arm: "string"; string: CstLeaf }
  | { arm: "template"; template: CstLeaf | $templateNode }
  | { arm: "regex"; regex: CstLeaf }
  | { arm: "spread"; expr: ExprNode }
  | { arm: "led_lt"; left: ExprNode; type: (TypeNode)[]; alt: { branch: "paren"; expr: (ExprNode)[] } | { branch: "template"; template: CstLeaf | $templateNode } }
  | { arm: "led_lt2"; left: ExprNode; type: (TypeNode)[] }
  | { arm: "led_paren"; left: ExprNode; expr: (ExprNode)[] }
  | { arm: "led_dot"; left: ExprNode; alt: { branch: "ident"; ident: CstLeaf } | { branch: "privateField"; privateField: CstLeaf } }
  | { arm: "led_optChain"; left: ExprNode; alt: { branch: "ident"; ident: CstLeaf } | { branch: "privateField"; privateField: CstLeaf } | { branch: "paren"; expr: (ExprNode)[] } | { branch: "bracket"; expr: ExprNode } | { branch: "template"; template: CstLeaf | $templateNode } }
  | { arm: "led_bracket"; left: ExprNode; expr: ExprNode }
  | { arm: "led_bang"; left: ExprNode }
  | { arm: "led_question"; left: ExprNode; expr: ExprNode; expr2: ExprNode }
  | { arm: "led_as"; left: ExprNode; type: TypeNode }
  | { arm: "led_instanceof"; left: ExprNode; expr: ExprNode }
  | { arm: "led_in"; left: ExprNode; expr: ExprNode }
  | { arm: "led_template"; left: ExprNode; template: CstLeaf | $templateNode }
  | { arm: "new_"; newTarget: NewTargetNode; alt?: { branch: "lt"; type: (TypeNode)[]; expr: (ExprNode)[] } | { branch: "paren"; expr: (ExprNode)[] } }
  | { arm: "new_2"; ident: CstLeaf; typeParams?: TypeParamsNode; classHeritage?: ClassHeritageNode; extendsTok?: CstLeaf; type: (TypeNode)[]; implementsTok?: CstLeaf; classMember: (ClassMemberNode)[]; expr: (ExprNode)[] }
  | { arm: "new_3"; typeParams?: TypeParamsNode; classHeritage?: ClassHeritageNode; extendsTok?: CstLeaf; type: (TypeNode)[]; implementsTok?: CstLeaf; classMember: (ClassMemberNode)[]; expr: (ExprNode)[] }
  | { arm: "bracket"; expr: (ExprNode)[]; expr2?: ExprNode }
  | { arm: "brace"; prop: (PropNode)[] }
  | { arm: "async"; asyncTok?: CstLeaf; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode; alt: { branch: "expr"; expr: ExprNode } | { branch: "block"; block: BlockNode } }
  | { arm: "async2"; ident: CstLeaf; alt: { branch: "expr"; expr: ExprNode } | { branch: "block"; block: BlockNode } }
  | { arm: "ident2"; ident: CstLeaf; alt: { branch: "expr"; expr: ExprNode } | { branch: "block"; block: BlockNode } }
  | { arm: "yield_"; alt: { branch: "star"; expr: ExprNode } | { branch: "expr"; expr?: ExprNode } }
  | { arm: "paren"; expr: ExprNode; expr2: (ExprNode)[] }
  | { arm: "led_satisfies"; left: ExprNode; type: TypeNode }
  | { arm: "import_"; alt: { branch: "paren"; expr: ExprNode } | { branch: "dot" } }
  | { arm: "privateField"; privateField: CstLeaf }
  | { arm: "hexNumber"; hexNumber: CstLeaf }
  | { arm: "octalNumber"; octalNumber: CstLeaf }
  | { arm: "binaryNumber"; binaryNumber: CstLeaf }
  | { arm: "bigInt"; bigInt: CstLeaf }
  | { arm: "async3"; asyncTok?: CstLeaf; ident?: CstLeaf; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode; block: BlockNode }
  | { arm: "decoratorExpr"; decoratorExpr: (DecoratorExprNode)[]; ident: CstLeaf; typeParams?: TypeParamsNode; alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[]; classMember: (ClassMemberNode)[] }
  | { arm: "decoratorExpr2"; decoratorExpr: (DecoratorExprNode)[]; typeParams?: TypeParamsNode; alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[]; classMember: (ClassMemberNode)[] }
  | { arm: "binaryOp"; left: ExprNode; op: CstLeaf; right: ExprNode }
  | { arm: "prefixOp"; op: CstLeaf; operand: ExprNode }
  | { arm: "postfixOp"; operand: ExprNode; op: CstLeaf };

function _Expr$jSXElement(c: readonly CstChild[], src: string): ExprMatch | null {
  let jSXElement: (JSXElementNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "JSXElement")) return null;
  jSXElement = c[i] as JSXElementNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "jSXElement", jSXElement: jSXElement! };
}

function _Expr$true_(c: readonly CstChild[], src: string): ExprMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "true", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "true_" };
}

function _Expr$false_(c: readonly CstChild[], src: string): ExprMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "false", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "false_" };
}

function _Expr$null_(c: readonly CstChild[], src: string): ExprMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "null", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "null_" };
}

function _Expr$undefined(c: readonly CstChild[], src: string): ExprMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "undefined", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "undefined" };
}

function _Expr$this_(c: readonly CstChild[], src: string): ExprMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "this", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "this_" };
}

function _Expr$super_(c: readonly CstChild[], src: string): ExprMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "super", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "super_" };
}

function _Expr$ident(c: readonly CstChild[], src: string): ExprMatch | null {
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident! };
}

function _Expr$number(c: readonly CstChild[], src: string): ExprMatch | null {
  let number: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Number"))) return null;
  number = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "number", number: number! };
}

function _Expr$string(c: readonly CstChild[], src: string): ExprMatch | null {
  let string: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "String"))) return null;
  string = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "string", string: string! };
}

function _Expr$template(c: readonly CstChild[], src: string): ExprMatch | null {
  let template: (CstLeaf | $templateNode) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Template") || __nodeOf(c, i, '$template'))) return null;
  template = c[i] as CstLeaf | $templateNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "template", template: template! };
}

function _Expr$regex(c: readonly CstChild[], src: string): ExprMatch | null {
  let regex: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Regex"))) return null;
  regex = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "regex", regex: regex! };
}

function _Expr$spread(c: readonly CstChild[], src: string): ExprMatch | null {
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "...", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "spread", expr: expr! };
}

function _Expr$led_lt(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  const type: (TypeNode)[] = [];
  let alt: ({ branch: "paren"; expr: (ExprNode)[] } | { branch: "template"; template: CstLeaf | $templateNode }) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "<", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "Type")) { _t3 = false; break _b4; }
      type.push(c[i] as TypeNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "Type")) { _t1 = false; break _b2; }
        type.push(c[i] as TypeNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, ">", "$punct")) return null;
  i++;
  {
    let _t5 = false;
    if (!_t5) {
      const _t9_expr: (ExprNode)[] = [];
      const _t6 = i; let _t7 = true;
      _b8: {
        if (!__lit(c, i, src, "(", "$punct")) { _t7 = false; break _b8; }
        i++;
        {
          const _t10 = i; let _t13 = true;
          _b14: {
            if (!__nodeOf(c, i, "Expr")) { _t13 = false; break _b14; }
            _t9_expr.push(c[i] as ExprNode);
            i++;
          }
          if (!_t13) { i = _t10; }
          else for (;;) {
            if (!__lit(c, i, src, ",", "$punct")) break;
            i++;
            const _t102 = i; let _t11 = true;
            _b12: {
              if (!__nodeOf(c, i, "Expr")) { _t11 = false; break _b12; }
              _t9_expr.push(c[i] as ExprNode);
              i++;
            }
            if (!_t11) { i = _t102; break; }
          }
        }
        if (!__lit(c, i, src, ")", "$punct")) { _t7 = false; break _b8; }
        i++;
      }
      if (_t7) { _t5 = true; alt = ({ branch: "paren", expr: _t9_expr }) as typeof alt; }
      else i = _t6;
    }
    if (!_t5) {
      let _t18_template: (CstLeaf | $templateNode) | undefined;
      const _t15 = i; let _t16 = true;
      _b17: {
        if (!(__tok(c, i, "Template") || __nodeOf(c, i, '$template'))) { _t16 = false; break _b17; }
        _t18_template = c[i] as CstLeaf | $templateNode;
        i++;
      }
      if (_t16) { _t5 = true; alt = ({ branch: "template", template: _t18_template! }) as typeof alt; }
      else i = _t15;
    }
    if (!_t5) return null;
  }
  if (i !== c.length) return null;
  return { arm: "led_lt", left: left!, type, alt: alt! };
}

function _Expr$led_lt2(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  const type: (TypeNode)[] = [];
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "<", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "Type")) { _t3 = false; break _b4; }
      type.push(c[i] as TypeNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "Type")) { _t1 = false; break _b2; }
        type.push(c[i] as TypeNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, ">", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_lt2", left: left!, type };
}

function _Expr$led_paren(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  const expr: (ExprNode)[] = [];
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "Expr")) { _t3 = false; break _b4; }
      expr.push(c[i] as ExprNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
        expr.push(c[i] as ExprNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_paren", left: left!, expr };
}

function _Expr$led_dot(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  let alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "privateField"; privateField: CstLeaf }) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, ".", "$punct")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_ident: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "Ident"))) { _t2 = false; break _b3; }
        _t4_ident = c[i] as CstLeaf;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "ident", ident: _t4_ident! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_privateField: (CstLeaf) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!(__tok(c, i, "PrivateField"))) { _t6 = false; break _b7; }
        _t8_privateField = c[i] as CstLeaf;
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "privateField", privateField: _t8_privateField! }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) return null;
  }
  if (i !== c.length) return null;
  return { arm: "led_dot", left: left!, alt: alt! };
}

function _Expr$led_optChain(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  let alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "privateField"; privateField: CstLeaf } | { branch: "paren"; expr: (ExprNode)[] } | { branch: "bracket"; expr: ExprNode } | { branch: "template"; template: CstLeaf | $templateNode }) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "?.", "$punct")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_ident: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "Ident"))) { _t2 = false; break _b3; }
        _t4_ident = c[i] as CstLeaf;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "ident", ident: _t4_ident! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_privateField: (CstLeaf) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!(__tok(c, i, "PrivateField"))) { _t6 = false; break _b7; }
        _t8_privateField = c[i] as CstLeaf;
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "privateField", privateField: _t8_privateField! }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) {
      const _t12_expr: (ExprNode)[] = [];
      const _t9 = i; let _t10 = true;
      _b11: {
        if (!__lit(c, i, src, "(", "$punct")) { _t10 = false; break _b11; }
        i++;
        {
          const _t13 = i; let _t16 = true;
          _b17: {
            if (!__nodeOf(c, i, "Expr")) { _t16 = false; break _b17; }
            _t12_expr.push(c[i] as ExprNode);
            i++;
          }
          if (!_t16) { i = _t13; }
          else for (;;) {
            if (!__lit(c, i, src, ",", "$punct")) break;
            i++;
            const _t132 = i; let _t14 = true;
            _b15: {
              if (!__nodeOf(c, i, "Expr")) { _t14 = false; break _b15; }
              _t12_expr.push(c[i] as ExprNode);
              i++;
            }
            if (!_t14) { i = _t132; break; }
          }
        }
        if (!__lit(c, i, src, ")", "$punct")) { _t10 = false; break _b11; }
        i++;
      }
      if (_t10) { _t0 = true; alt = ({ branch: "paren", expr: _t12_expr }) as typeof alt; }
      else i = _t9;
    }
    if (!_t0) {
      let _t21_expr: (ExprNode) | undefined;
      const _t18 = i; let _t19 = true;
      _b20: {
        if (!__lit(c, i, src, "[", "$punct")) { _t19 = false; break _b20; }
        i++;
        if (!__nodeOf(c, i, "Expr")) { _t19 = false; break _b20; }
        _t21_expr = c[i] as ExprNode;
        i++;
        if (!__lit(c, i, src, "]", "$punct")) { _t19 = false; break _b20; }
        i++;
      }
      if (_t19) { _t0 = true; alt = ({ branch: "bracket", expr: _t21_expr! }) as typeof alt; }
      else i = _t18;
    }
    if (!_t0) {
      let _t25_template: (CstLeaf | $templateNode) | undefined;
      const _t22 = i; let _t23 = true;
      _b24: {
        if (!(__tok(c, i, "Template") || __nodeOf(c, i, '$template'))) { _t23 = false; break _b24; }
        _t25_template = c[i] as CstLeaf | $templateNode;
        i++;
      }
      if (_t23) { _t0 = true; alt = ({ branch: "template", template: _t25_template! }) as typeof alt; }
      else i = _t22;
    }
    if (!_t0) return null;
  }
  if (i !== c.length) return null;
  return { arm: "led_optChain", left: left!, alt: alt! };
}

function _Expr$led_bracket(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "[", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "]", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_bracket", left: left!, expr: expr! };
}

function _Expr$led_bang(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "!", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_bang", left: left! };
}

function _Expr$led_question(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  let expr: (ExprNode) | undefined;
  let expr2: (ExprNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "?", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr2 = c[i] as ExprNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_question", left: left!, expr: expr!, expr2: expr2! };
}

function _Expr$led_as(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "as", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_as", left: left!, type: type! };
}

function _Expr$led_instanceof(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "instanceof", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_instanceof", left: left!, expr: expr! };
}

function _Expr$led_in(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "in", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_in", left: left!, expr: expr! };
}

function _Expr$led_template(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  let template: (CstLeaf | $templateNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!(__tok(c, i, "Template") || __nodeOf(c, i, '$template'))) return null;
  template = c[i] as CstLeaf | $templateNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_template", left: left!, template: template! };
}

function _Expr$new_(c: readonly CstChild[], src: string): ExprMatch | null {
  let newTarget: (NewTargetNode) | undefined;
  let alt: ({ branch: "lt"; type: (TypeNode)[]; expr: (ExprNode)[] } | { branch: "paren"; expr: (ExprNode)[] }) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "new", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "NewTarget")) return null;
  newTarget = c[i] as NewTargetNode;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      {
        let _t3 = false;
        if (!_t3) {
          const _t7_type: (TypeNode)[] = [];
          const _t7_expr: (ExprNode)[] = [];
          const _t4 = i; let _t5 = true;
          _b6: {
            if (!__lit(c, i, src, "<", "$punct")) { _t5 = false; break _b6; }
            i++;
            {
              const _t8 = i; let _t11 = true;
              _b12: {
                if (!__nodeOf(c, i, "Type")) { _t11 = false; break _b12; }
                _t7_type.push(c[i] as TypeNode);
                i++;
              }
              if (!_t11) { i = _t8; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t82 = i; let _t9 = true;
                _b10: {
                  if (!__nodeOf(c, i, "Type")) { _t9 = false; break _b10; }
                  _t7_type.push(c[i] as TypeNode);
                  i++;
                }
                if (!_t9) { i = _t82; break; }
              }
            }
            if (!__lit(c, i, src, ">", "$punct")) { _t5 = false; break _b6; }
            i++;
            {
              const _t13 = i; let _t14 = true;
              _b15: {
                if (!__lit(c, i, src, "(", "$punct")) { _t14 = false; break _b15; }
                i++;
                {
                  const _t16 = i; let _t19 = true;
                  _b20: {
                    if (!__nodeOf(c, i, "Expr")) { _t19 = false; break _b20; }
                    _t7_expr.push(c[i] as ExprNode);
                    i++;
                  }
                  if (!_t19) { i = _t16; }
                  else for (;;) {
                    if (!__lit(c, i, src, ",", "$punct")) break;
                    i++;
                    const _t162 = i; let _t17 = true;
                    _b18: {
                      if (!__nodeOf(c, i, "Expr")) { _t17 = false; break _b18; }
                      _t7_expr.push(c[i] as ExprNode);
                      i++;
                    }
                    if (!_t17) { i = _t162; break; }
                  }
                }
                if (!__lit(c, i, src, ")", "$punct")) { _t14 = false; break _b15; }
                i++;
              }
              if (!_t14) i = _t13;
            }
          }
          if (_t5) { _t3 = true; alt = ({ branch: "lt", type: _t7_type, expr: _t7_expr }) as typeof alt; }
          else i = _t4;
        }
        if (!_t3) {
          const _t24_expr: (ExprNode)[] = [];
          const _t21 = i; let _t22 = true;
          _b23: {
            if (!__lit(c, i, src, "(", "$punct")) { _t22 = false; break _b23; }
            i++;
            {
              const _t25 = i; let _t28 = true;
              _b29: {
                if (!__nodeOf(c, i, "Expr")) { _t28 = false; break _b29; }
                _t24_expr.push(c[i] as ExprNode);
                i++;
              }
              if (!_t28) { i = _t25; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t252 = i; let _t26 = true;
                _b27: {
                  if (!__nodeOf(c, i, "Expr")) { _t26 = false; break _b27; }
                  _t24_expr.push(c[i] as ExprNode);
                  i++;
                }
                if (!_t26) { i = _t252; break; }
              }
            }
            if (!__lit(c, i, src, ")", "$punct")) { _t22 = false; break _b23; }
            i++;
          }
          if (_t22) { _t3 = true; alt = ({ branch: "paren", expr: _t24_expr }) as typeof alt; }
          else i = _t21;
        }
        if (!_t3) { _t1 = false; break _b2; }
      }
    }
    if (!_t1) i = _t0;
  }
  if (i !== c.length) return null;
  return { arm: "new_", newTarget: newTarget!, alt };
}

function _Expr$new_2(c: readonly CstChild[], src: string): ExprMatch | null {
  let ident: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  let classHeritage: (ClassHeritageNode) | undefined;
  let extendsTok: (CstLeaf) | undefined;
  const type: (TypeNode)[] = [];
  let implementsTok: (CstLeaf) | undefined;
  const classMember: (ClassMemberNode)[] = [];
  const expr: (ExprNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "new", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "class", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "TypeParams")) { _t1 = false; break _b2; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "extends", "$keyword")) { _t4 = false; break _b5; }
      extendsTok = c[i] as CstLeaf;
      i++;
      if (!__nodeOf(c, i, "ClassHeritage")) { _t4 = false; break _b5; }
      classHeritage = c[i] as ClassHeritageNode;
      i++;
    }
    if (!_t4) i = _t3;
  }
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__lit(c, i, src, "implements", "$keyword")) { _t7 = false; break _b8; }
      implementsTok = c[i] as CstLeaf;
      i++;
      {
        const _t9 = i; let _t12 = true;
        _b13: {
          if (!__nodeOf(c, i, "Type")) { _t12 = false; break _b13; }
          type.push(c[i] as TypeNode);
          i++;
        }
        if (!_t12) { i = _t9; }
        else for (;;) {
          if (!__lit(c, i, src, ",", "$punct")) break;
          i++;
          const _t92 = i; let _t10 = true;
          _b11: {
            if (!__nodeOf(c, i, "Type")) { _t10 = false; break _b11; }
            type.push(c[i] as TypeNode);
            i++;
          }
          if (!_t10) { i = _t92; break; }
        }
      }
    }
    if (!_t7) i = _t6;
  }
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  for (;;) {
    const _t14 = i; let _t15 = true;
    _b16: {
      if (!__nodeOf(c, i, "ClassMember")) { _t15 = false; break _b16; }
      classMember.push(c[i] as ClassMemberNode);
      i++;
    }
    if (!_t15) { i = _t14; break; }
    if (i === _t14) break;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  {
    const _t17 = i; let _t18 = true;
    _b19: {
      if (!__lit(c, i, src, "(", "$punct")) { _t18 = false; break _b19; }
      i++;
      {
        const _t20 = i; let _t23 = true;
        _b24: {
          if (!__nodeOf(c, i, "Expr")) { _t23 = false; break _b24; }
          expr.push(c[i] as ExprNode);
          i++;
        }
        if (!_t23) { i = _t20; }
        else for (;;) {
          if (!__lit(c, i, src, ",", "$punct")) break;
          i++;
          const _t202 = i; let _t21 = true;
          _b22: {
            if (!__nodeOf(c, i, "Expr")) { _t21 = false; break _b22; }
            expr.push(c[i] as ExprNode);
            i++;
          }
          if (!_t21) { i = _t202; break; }
        }
      }
      if (!__lit(c, i, src, ")", "$punct")) { _t18 = false; break _b19; }
      i++;
    }
    if (!_t18) i = _t17;
  }
  if (i !== c.length) return null;
  return { arm: "new_2", ident: ident!, typeParams, classHeritage, extendsTok, type, implementsTok, classMember, expr };
}

function _Expr$new_3(c: readonly CstChild[], src: string): ExprMatch | null {
  let typeParams: (TypeParamsNode) | undefined;
  let classHeritage: (ClassHeritageNode) | undefined;
  let extendsTok: (CstLeaf) | undefined;
  const type: (TypeNode)[] = [];
  let implementsTok: (CstLeaf) | undefined;
  const classMember: (ClassMemberNode)[] = [];
  const expr: (ExprNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "new", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "class", "$keyword")) return null;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "TypeParams")) { _t1 = false; break _b2; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "extends", "$keyword")) { _t4 = false; break _b5; }
      extendsTok = c[i] as CstLeaf;
      i++;
      if (!__nodeOf(c, i, "ClassHeritage")) { _t4 = false; break _b5; }
      classHeritage = c[i] as ClassHeritageNode;
      i++;
    }
    if (!_t4) i = _t3;
  }
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__lit(c, i, src, "implements", "$keyword")) { _t7 = false; break _b8; }
      implementsTok = c[i] as CstLeaf;
      i++;
      {
        const _t9 = i; let _t12 = true;
        _b13: {
          if (!__nodeOf(c, i, "Type")) { _t12 = false; break _b13; }
          type.push(c[i] as TypeNode);
          i++;
        }
        if (!_t12) { i = _t9; }
        else for (;;) {
          if (!__lit(c, i, src, ",", "$punct")) break;
          i++;
          const _t92 = i; let _t10 = true;
          _b11: {
            if (!__nodeOf(c, i, "Type")) { _t10 = false; break _b11; }
            type.push(c[i] as TypeNode);
            i++;
          }
          if (!_t10) { i = _t92; break; }
        }
      }
    }
    if (!_t7) i = _t6;
  }
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  for (;;) {
    const _t14 = i; let _t15 = true;
    _b16: {
      if (!__nodeOf(c, i, "ClassMember")) { _t15 = false; break _b16; }
      classMember.push(c[i] as ClassMemberNode);
      i++;
    }
    if (!_t15) { i = _t14; break; }
    if (i === _t14) break;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  {
    const _t17 = i; let _t18 = true;
    _b19: {
      if (!__lit(c, i, src, "(", "$punct")) { _t18 = false; break _b19; }
      i++;
      {
        const _t20 = i; let _t23 = true;
        _b24: {
          if (!__nodeOf(c, i, "Expr")) { _t23 = false; break _b24; }
          expr.push(c[i] as ExprNode);
          i++;
        }
        if (!_t23) { i = _t20; }
        else for (;;) {
          if (!__lit(c, i, src, ",", "$punct")) break;
          i++;
          const _t202 = i; let _t21 = true;
          _b22: {
            if (!__nodeOf(c, i, "Expr")) { _t21 = false; break _b22; }
            expr.push(c[i] as ExprNode);
            i++;
          }
          if (!_t21) { i = _t202; break; }
        }
      }
      if (!__lit(c, i, src, ")", "$punct")) { _t18 = false; break _b19; }
      i++;
    }
    if (!_t18) i = _t17;
  }
  if (i !== c.length) return null;
  return { arm: "new_3", typeParams, classHeritage, extendsTok, type, implementsTok, classMember, expr };
}

function _Expr$bracket(c: readonly CstChild[], src: string): ExprMatch | null {
  const expr: (ExprNode)[] = [];
  let expr2: (ExprNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "[", "$punct")) return null;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      {
        const _t3 = i; let _t4 = true;
        _b5: {
          if (!__nodeOf(c, i, "Expr")) { _t4 = false; break _b5; }
          expr.push(c[i] as ExprNode);
          i++;
        }
        if (!_t4) i = _t3;
      }
      if (!__lit(c, i, src, ",", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__nodeOf(c, i, "Expr")) { _t7 = false; break _b8; }
      expr2 = c[i] as ExprNode;
      i++;
    }
    if (!_t7) i = _t6;
  }
  if (!__lit(c, i, src, "]", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "bracket", expr, expr2 };
}

function _Expr$brace(c: readonly CstChild[], src: string): ExprMatch | null {
  const prop: (PropNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "Prop")) { _t3 = false; break _b4; }
      prop.push(c[i] as PropNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "Prop")) { _t1 = false; break _b2; }
        prop.push(c[i] as PropNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "brace", prop };
}

function _Expr$async(c: readonly CstChild[], src: string): ExprMatch | null {
  let asyncTok: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let alt: ({ branch: "expr"; expr: ExprNode } | { branch: "block"; block: BlockNode }) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "async", "$keyword")) { _t1 = false; break _b2; }
      asyncTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__nodeOf(c, i, "TypeParams")) { _t4 = false; break _b5; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t6 = i; let _t9 = true;
    _b10: {
      if (!__nodeOf(c, i, "Param")) { _t9 = false; break _b10; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t9) { i = _t6; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t62 = i; let _t7 = true;
      _b8: {
        if (!__nodeOf(c, i, "Param")) { _t7 = false; break _b8; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t7) { i = _t62; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  {
    const _t11 = i; let _t12 = true;
    _b13: {
      if (!__lit(c, i, src, ":", "$punct")) { _t12 = false; break _b13; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t12 = false; break _b13; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t12) i = _t11;
  }
  if (!__lit(c, i, src, "=>", "$punct")) return null;
  i++;
  {
    let _t14 = false;
    if (!_t14) {
      let _t18_expr: (ExprNode) | undefined;
      const _t15 = i; let _t16 = true;
      _b17: {
        if (!__nodeOf(c, i, "Expr")) { _t16 = false; break _b17; }
        _t18_expr = c[i] as ExprNode;
        i++;
      }
      if (_t16) { _t14 = true; alt = ({ branch: "expr", expr: _t18_expr! }) as typeof alt; }
      else i = _t15;
    }
    if (!_t14) {
      let _t22_block: (BlockNode) | undefined;
      const _t19 = i; let _t20 = true;
      _b21: {
        if (!__nodeOf(c, i, "Block")) { _t20 = false; break _b21; }
        _t22_block = c[i] as BlockNode;
        i++;
      }
      if (_t20) { _t14 = true; alt = ({ branch: "block", block: _t22_block! }) as typeof alt; }
      else i = _t19;
    }
    if (!_t14) return null;
  }
  if (i !== c.length) return null;
  return { arm: "async", asyncTok, typeParams, param, type, alt: alt! };
}

function _Expr$async2(c: readonly CstChild[], src: string): ExprMatch | null {
  let ident: (CstLeaf) | undefined;
  let alt: ({ branch: "expr"; expr: ExprNode } | { branch: "block"; block: BlockNode }) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "async", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (!__lit(c, i, src, "=>", "$punct")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_expr: (ExprNode) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!__nodeOf(c, i, "Expr")) { _t2 = false; break _b3; }
        _t4_expr = c[i] as ExprNode;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "expr", expr: _t4_expr! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_block: (BlockNode) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!__nodeOf(c, i, "Block")) { _t6 = false; break _b7; }
        _t8_block = c[i] as BlockNode;
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "block", block: _t8_block! }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) return null;
  }
  if (i !== c.length) return null;
  return { arm: "async2", ident: ident!, alt: alt! };
}

function _Expr$ident2(c: readonly CstChild[], src: string): ExprMatch | null {
  let ident: (CstLeaf) | undefined;
  let alt: ({ branch: "expr"; expr: ExprNode } | { branch: "block"; block: BlockNode }) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (!__lit(c, i, src, "=>", "$punct")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_expr: (ExprNode) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!__nodeOf(c, i, "Expr")) { _t2 = false; break _b3; }
        _t4_expr = c[i] as ExprNode;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "expr", expr: _t4_expr! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_block: (BlockNode) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!__nodeOf(c, i, "Block")) { _t6 = false; break _b7; }
        _t8_block = c[i] as BlockNode;
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "block", block: _t8_block! }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) return null;
  }
  if (i !== c.length) return null;
  return { arm: "ident2", ident: ident!, alt: alt! };
}

function _Expr$yield_(c: readonly CstChild[], src: string): ExprMatch | null {
  let alt: ({ branch: "star"; expr: ExprNode } | { branch: "expr"; expr?: ExprNode }) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "yield", "$keyword")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_expr: (ExprNode) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!__lit(c, i, src, "*", "$punct")) { _t2 = false; break _b3; }
        i++;
        if (!__nodeOf(c, i, "Expr")) { _t2 = false; break _b3; }
        _t4_expr = c[i] as ExprNode;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "star", expr: _t4_expr! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_expr: (ExprNode) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        {
          const _t9 = i; let _t10 = true;
          _b11: {
            if (!__nodeOf(c, i, "Expr")) { _t10 = false; break _b11; }
            _t8_expr = c[i] as ExprNode;
            i++;
          }
          if (!_t10) i = _t9;
        }
      }
      if (_t6) { _t0 = true; alt = ({ branch: "expr", expr: _t8_expr }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) return null;
  }
  if (i !== c.length) return null;
  return { arm: "yield_", alt: alt! };
}

function _Expr$paren(c: readonly CstChild[], src: string): ExprMatch | null {
  let expr: (ExprNode) | undefined;
  const expr2: (ExprNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, ",", "$punct")) { _t1 = false; break _b2; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
      expr2.push(c[i] as ExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "paren", expr: expr!, expr2 };
}

function _Expr$led_satisfies(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "satisfies", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_satisfies", left: left!, type: type! };
}

function _Expr$import_(c: readonly CstChild[], src: string): ExprMatch | null {
  let alt: ({ branch: "paren"; expr: ExprNode } | { branch: "dot" }) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "import", "$keyword")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_expr: (ExprNode) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!__lit(c, i, src, "(", "$punct")) { _t2 = false; break _b3; }
        i++;
        if (!__nodeOf(c, i, "Expr")) { _t2 = false; break _b3; }
        _t4_expr = c[i] as ExprNode;
        i++;
        if (!__lit(c, i, src, ")", "$punct")) { _t2 = false; break _b3; }
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "paren", expr: _t4_expr! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!__lit(c, i, src, ".", "$punct")) { _t6 = false; break _b7; }
        i++;
        if (!__lit(c, i, src, "meta", "$keyword")) { _t6 = false; break _b7; }
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "dot" }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) return null;
  }
  if (i !== c.length) return null;
  return { arm: "import_", alt: alt! };
}

function _Expr$privateField(c: readonly CstChild[], src: string): ExprMatch | null {
  let privateField: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "PrivateField"))) return null;
  privateField = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "privateField", privateField: privateField! };
}

function _Expr$hexNumber(c: readonly CstChild[], src: string): ExprMatch | null {
  let hexNumber: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "HexNumber"))) return null;
  hexNumber = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "hexNumber", hexNumber: hexNumber! };
}

function _Expr$octalNumber(c: readonly CstChild[], src: string): ExprMatch | null {
  let octalNumber: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "OctalNumber"))) return null;
  octalNumber = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "octalNumber", octalNumber: octalNumber! };
}

function _Expr$binaryNumber(c: readonly CstChild[], src: string): ExprMatch | null {
  let binaryNumber: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "BinaryNumber"))) return null;
  binaryNumber = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "binaryNumber", binaryNumber: binaryNumber! };
}

function _Expr$bigInt(c: readonly CstChild[], src: string): ExprMatch | null {
  let bigInt: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "BigInt"))) return null;
  bigInt = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "bigInt", bigInt: bigInt! };
}

function _Expr$async3(c: readonly CstChild[], src: string): ExprMatch | null {
  let asyncTok: (CstLeaf) | undefined;
  let ident: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let block: (BlockNode) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "async", "$keyword")) { _t1 = false; break _b2; }
      asyncTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (!__lit(c, i, src, "function", "$keyword")) return null;
  i++;
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "*", "$punct")) { _t4 = false; break _b5; }
      i++;
    }
    if (!_t4) i = _t3;
  }
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!(__tok(c, i, "Ident"))) { _t7 = false; break _b8; }
      ident = c[i] as CstLeaf;
      i++;
    }
    if (!_t7) i = _t6;
  }
  {
    const _t9 = i; let _t10 = true;
    _b11: {
      if (!__nodeOf(c, i, "TypeParams")) { _t10 = false; break _b11; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t10) i = _t9;
  }
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t12 = i; let _t15 = true;
    _b16: {
      if (!__nodeOf(c, i, "Param")) { _t15 = false; break _b16; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t15) { i = _t12; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t122 = i; let _t13 = true;
      _b14: {
        if (!__nodeOf(c, i, "Param")) { _t13 = false; break _b14; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t13) { i = _t122; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  {
    const _t17 = i; let _t18 = true;
    _b19: {
      if (!__lit(c, i, src, ":", "$punct")) { _t18 = false; break _b19; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t18 = false; break _b19; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t18) i = _t17;
  }
  if (!__nodeOf(c, i, "Block")) return null;
  block = c[i] as BlockNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "async3", asyncTok, ident, typeParams, param, type, block: block! };
}

function _Expr$decoratorExpr(c: readonly CstChild[], src: string): ExprMatch | null {
  const decoratorExpr: (DecoratorExprNode)[] = [];
  let ident: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[] = [];
  const classMember: (ClassMemberNode)[] = [];
  let i = 0;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "DecoratorExpr")) { _t1 = false; break _b2; }
      decoratorExpr.push(c[i] as DecoratorExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, "class", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__nodeOf(c, i, "TypeParams")) { _t4 = false; break _b5; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t4) i = _t3;
  }
  for (;;) {
    const _t6 = i; let _t7 = true;
    _b8: {
      {
        let _t9 = false;
        if (!_t9) {
          const _t13_alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] = [];
          const _t10 = i; let _t11 = true;
          _b12: {
            if (!__lit(c, i, src, "extends", "$keyword")) { _t11 = false; break _b12; }
            i++;
            {
              const _t14 = i; let _t17 = true;
              _b18: {
                {
                  let _t19 = false;
                  if (!_t19) {
                    let _t23__t13_classHeritage: (ClassHeritageNode) | undefined;
                    const _t20 = i; let _t21 = true;
                    _b22: {
                      if (!__nodeOf(c, i, "ClassHeritage")) { _t21 = false; break _b22; }
                      _t23__t13_classHeritage = c[i] as ClassHeritageNode;
                      i++;
                    }
                    if (_t21) { _t19 = true; _t13_alt.push({ branch: "classHeritage", classHeritage: _t23__t13_classHeritage! } as never); }
                    else i = _t20;
                  }
                  if (!_t19) { _t17 = false; break _b18; }
                }
              }
              if (!_t17) { i = _t14; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t142 = i; let _t15 = true;
                _b16: {
                  {
                    let _t24 = false;
                    if (!_t24) {
                      let _t28__t13_classHeritage: (ClassHeritageNode) | undefined;
                      const _t25 = i; let _t26 = true;
                      _b27: {
                        if (!__nodeOf(c, i, "ClassHeritage")) { _t26 = false; break _b27; }
                        _t28__t13_classHeritage = c[i] as ClassHeritageNode;
                        i++;
                      }
                      if (_t26) { _t24 = true; _t13_alt.push({ branch: "classHeritage", classHeritage: _t28__t13_classHeritage! } as never); }
                      else i = _t25;
                    }
                    if (!_t24) { _t15 = false; break _b16; }
                  }
                }
                if (!_t15) { i = _t142; break; }
              }
            }
          }
          if (_t11) { _t9 = true; alt.push({ branch: "extends_", alt: _t13_alt } as never); }
          else i = _t10;
        }
        if (!_t9) {
          const _t32_alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] = [];
          const _t29 = i; let _t30 = true;
          _b31: {
            if (!__lit(c, i, src, "implements", "$keyword")) { _t30 = false; break _b31; }
            i++;
            {
              const _t33 = i; let _t36 = true;
              _b37: {
                {
                  let _t38 = false;
                  if (!_t38) {
                    let _t42__t32_classHeritage: (ClassHeritageNode) | undefined;
                    const _t39 = i; let _t40 = true;
                    _b41: {
                      if (!__nodeOf(c, i, "ClassHeritage")) { _t40 = false; break _b41; }
                      _t42__t32_classHeritage = c[i] as ClassHeritageNode;
                      i++;
                    }
                    if (_t40) { _t38 = true; _t32_alt.push({ branch: "classHeritage", classHeritage: _t42__t32_classHeritage! } as never); }
                    else i = _t39;
                  }
                  if (!_t38) { _t36 = false; break _b37; }
                }
              }
              if (!_t36) { i = _t33; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t332 = i; let _t34 = true;
                _b35: {
                  {
                    let _t43 = false;
                    if (!_t43) {
                      let _t47__t32_classHeritage: (ClassHeritageNode) | undefined;
                      const _t44 = i; let _t45 = true;
                      _b46: {
                        if (!__nodeOf(c, i, "ClassHeritage")) { _t45 = false; break _b46; }
                        _t47__t32_classHeritage = c[i] as ClassHeritageNode;
                        i++;
                      }
                      if (_t45) { _t43 = true; _t32_alt.push({ branch: "classHeritage", classHeritage: _t47__t32_classHeritage! } as never); }
                      else i = _t44;
                    }
                    if (!_t43) { _t34 = false; break _b35; }
                  }
                }
                if (!_t34) { i = _t332; break; }
              }
            }
          }
          if (_t30) { _t9 = true; alt.push({ branch: "implements", alt: _t32_alt } as never); }
          else i = _t29;
        }
        if (!_t9) { _t7 = false; break _b8; }
      }
    }
    if (!_t7) { i = _t6; break; }
    if (i === _t6) break;
  }
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  for (;;) {
    const _t48 = i; let _t49 = true;
    _b50: {
      if (!__nodeOf(c, i, "ClassMember")) { _t49 = false; break _b50; }
      classMember.push(c[i] as ClassMemberNode);
      i++;
    }
    if (!_t49) { i = _t48; break; }
    if (i === _t48) break;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "decoratorExpr", decoratorExpr, ident: ident!, typeParams, alt, classMember };
}

function _Expr$decoratorExpr2(c: readonly CstChild[], src: string): ExprMatch | null {
  const decoratorExpr: (DecoratorExprNode)[] = [];
  let typeParams: (TypeParamsNode) | undefined;
  const alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[] = [];
  const classMember: (ClassMemberNode)[] = [];
  let i = 0;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "DecoratorExpr")) { _t1 = false; break _b2; }
      decoratorExpr.push(c[i] as DecoratorExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, "class", "$keyword")) return null;
  i++;
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__nodeOf(c, i, "TypeParams")) { _t4 = false; break _b5; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t4) i = _t3;
  }
  for (;;) {
    const _t6 = i; let _t7 = true;
    _b8: {
      {
        let _t9 = false;
        if (!_t9) {
          const _t13_alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] = [];
          const _t10 = i; let _t11 = true;
          _b12: {
            if (!__lit(c, i, src, "extends", "$keyword")) { _t11 = false; break _b12; }
            i++;
            {
              const _t14 = i; let _t17 = true;
              _b18: {
                {
                  let _t19 = false;
                  if (!_t19) {
                    let _t23__t13_classHeritage: (ClassHeritageNode) | undefined;
                    const _t20 = i; let _t21 = true;
                    _b22: {
                      if (!__nodeOf(c, i, "ClassHeritage")) { _t21 = false; break _b22; }
                      _t23__t13_classHeritage = c[i] as ClassHeritageNode;
                      i++;
                    }
                    if (_t21) { _t19 = true; _t13_alt.push({ branch: "classHeritage", classHeritage: _t23__t13_classHeritage! } as never); }
                    else i = _t20;
                  }
                  if (!_t19) { _t17 = false; break _b18; }
                }
              }
              if (!_t17) { i = _t14; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t142 = i; let _t15 = true;
                _b16: {
                  {
                    let _t24 = false;
                    if (!_t24) {
                      let _t28__t13_classHeritage: (ClassHeritageNode) | undefined;
                      const _t25 = i; let _t26 = true;
                      _b27: {
                        if (!__nodeOf(c, i, "ClassHeritage")) { _t26 = false; break _b27; }
                        _t28__t13_classHeritage = c[i] as ClassHeritageNode;
                        i++;
                      }
                      if (_t26) { _t24 = true; _t13_alt.push({ branch: "classHeritage", classHeritage: _t28__t13_classHeritage! } as never); }
                      else i = _t25;
                    }
                    if (!_t24) { _t15 = false; break _b16; }
                  }
                }
                if (!_t15) { i = _t142; break; }
              }
            }
          }
          if (_t11) { _t9 = true; alt.push({ branch: "extends_", alt: _t13_alt } as never); }
          else i = _t10;
        }
        if (!_t9) {
          const _t32_alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] = [];
          const _t29 = i; let _t30 = true;
          _b31: {
            if (!__lit(c, i, src, "implements", "$keyword")) { _t30 = false; break _b31; }
            i++;
            {
              const _t33 = i; let _t36 = true;
              _b37: {
                {
                  let _t38 = false;
                  if (!_t38) {
                    let _t42__t32_classHeritage: (ClassHeritageNode) | undefined;
                    const _t39 = i; let _t40 = true;
                    _b41: {
                      if (!__nodeOf(c, i, "ClassHeritage")) { _t40 = false; break _b41; }
                      _t42__t32_classHeritage = c[i] as ClassHeritageNode;
                      i++;
                    }
                    if (_t40) { _t38 = true; _t32_alt.push({ branch: "classHeritage", classHeritage: _t42__t32_classHeritage! } as never); }
                    else i = _t39;
                  }
                  if (!_t38) { _t36 = false; break _b37; }
                }
              }
              if (!_t36) { i = _t33; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t332 = i; let _t34 = true;
                _b35: {
                  {
                    let _t43 = false;
                    if (!_t43) {
                      let _t47__t32_classHeritage: (ClassHeritageNode) | undefined;
                      const _t44 = i; let _t45 = true;
                      _b46: {
                        if (!__nodeOf(c, i, "ClassHeritage")) { _t45 = false; break _b46; }
                        _t47__t32_classHeritage = c[i] as ClassHeritageNode;
                        i++;
                      }
                      if (_t45) { _t43 = true; _t32_alt.push({ branch: "classHeritage", classHeritage: _t47__t32_classHeritage! } as never); }
                      else i = _t44;
                    }
                    if (!_t43) { _t34 = false; break _b35; }
                  }
                }
                if (!_t34) { i = _t332; break; }
              }
            }
          }
          if (_t30) { _t9 = true; alt.push({ branch: "implements", alt: _t32_alt } as never); }
          else i = _t29;
        }
        if (!_t9) { _t7 = false; break _b8; }
      }
    }
    if (!_t7) { i = _t6; break; }
    if (i === _t6) break;
  }
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  for (;;) {
    const _t48 = i; let _t49 = true;
    _b50: {
      if (!__nodeOf(c, i, "ClassMember")) { _t49 = false; break _b50; }
      classMember.push(c[i] as ClassMemberNode);
      i++;
    }
    if (!_t49) { i = _t48; break; }
    if (i === _t48) break;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "decoratorExpr2", decoratorExpr, typeParams, alt, classMember };
}

function _Expr$binaryOp(c: readonly CstChild[], src: string): ExprMatch | null {
  let left: (ExprNode) | undefined;
  let op: (CstLeaf) | undefined;
  let right: (ExprNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  left = c[i] as ExprNode;
  i++;
  if (!(__tok(c, i, "$operator"))) return null;
  op = c[i] as CstLeaf;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  right = c[i] as ExprNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "binaryOp", left: left!, op: op!, right: right! };
}

function _Expr$prefixOp(c: readonly CstChild[], src: string): ExprMatch | null {
  let op: (CstLeaf) | undefined;
  let operand: (ExprNode) | undefined;
  let i = 0;
  if (!(__tok(c, i, "$operator"))) return null;
  op = c[i] as CstLeaf;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  operand = c[i] as ExprNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "prefixOp", op: op!, operand: operand! };
}

function _Expr$postfixOp(c: readonly CstChild[], src: string): ExprMatch | null {
  let operand: (ExprNode) | undefined;
  let op: (CstLeaf) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  operand = c[i] as ExprNode;
  i++;
  if (!(__tok(c, i, "$operator"))) return null;
  op = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "postfixOp", operand: operand!, op: op! };
}

export function matchExpr(n: ExprNode, src: string): ExprMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "$template": {
        { const m = _Expr$template(c, src); if (m !== null) return m; }
        break;
      }
      case "DecoratorExpr": {
        { const m = _Expr$decoratorExpr(c, src); if (m !== null) return m; }
        { const m = _Expr$decoratorExpr2(c, src); if (m !== null) return m; }
        break;
      }
      case "Expr": {
        const k1 = c[1] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
        if (k1 === undefined) {
        } else if (k1.tokenType === undefined) {
          switch (k1.rule) {
            case "$template": {
            { const m = _Expr$led_template(c, src); if (m !== null) return m; }
              break;
            }
            default: {
              break;
            }
          }
        } else if (k1.tokenType === '$keyword' || k1.tokenType === '$punct') {
          switch (src.charCodeAt(k1.offset)) {
            case 33: {
            { const m = _Expr$led_bang(c, src); if (m !== null) return m; }
              break;
            }
            case 40: {
            { const m = _Expr$led_paren(c, src); if (m !== null) return m; }
              break;
            }
            case 46: {
            { const m = _Expr$led_dot(c, src); if (m !== null) return m; }
              break;
            }
            case 60: {
            { const m = _Expr$led_lt(c, src); if (m !== null) return m; }
            { const m = _Expr$led_lt2(c, src); if (m !== null) return m; }
              break;
            }
            case 63: {
            { const m = _Expr$led_optChain(c, src); if (m !== null) return m; }
            { const m = _Expr$led_question(c, src); if (m !== null) return m; }
              break;
            }
            case 91: {
            { const m = _Expr$led_bracket(c, src); if (m !== null) return m; }
              break;
            }
            case 97: {
            { const m = _Expr$led_as(c, src); if (m !== null) return m; }
              break;
            }
            case 105: {
            { const m = _Expr$led_instanceof(c, src); if (m !== null) return m; }
            { const m = _Expr$led_in(c, src); if (m !== null) return m; }
              break;
            }
            case 115: {
            { const m = _Expr$led_satisfies(c, src); if (m !== null) return m; }
              break;
            }
            default: {
              break;
            }
          }
        } else {
          switch (k1.tokenType) {
            case "$operator": {
            { const m = _Expr$binaryOp(c, src); if (m !== null) return m; }
            { const m = _Expr$postfixOp(c, src); if (m !== null) return m; }
              break;
            }
            case "Template": {
            { const m = _Expr$led_template(c, src); if (m !== null) return m; }
              break;
            }
            default: {
              break;
            }
          }
        }
        break;
      }
      case "JSXElement": {
        { const m = _Expr$jSXElement(c, src); if (m !== null) return m; }
        break;
      }
      case "TypeParams": {
        { const m = _Expr$async(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 40: {
        { const m = _Expr$async(c, src); if (m !== null) return m; }
        { const m = _Expr$paren(c, src); if (m !== null) return m; }
        break;
      }
      case 46: {
        { const m = _Expr$spread(c, src); if (m !== null) return m; }
        break;
      }
      case 91: {
        { const m = _Expr$bracket(c, src); if (m !== null) return m; }
        break;
      }
      case 97: {
        { const m = _Expr$async(c, src); if (m !== null) return m; }
        { const m = _Expr$async2(c, src); if (m !== null) return m; }
        { const m = _Expr$async3(c, src); if (m !== null) return m; }
        break;
      }
      case 99: {
        { const m = _Expr$decoratorExpr(c, src); if (m !== null) return m; }
        { const m = _Expr$decoratorExpr2(c, src); if (m !== null) return m; }
        break;
      }
      case 102: {
        { const m = _Expr$false_(c, src); if (m !== null) return m; }
        { const m = _Expr$async3(c, src); if (m !== null) return m; }
        break;
      }
      case 105: {
        { const m = _Expr$import_(c, src); if (m !== null) return m; }
        break;
      }
      case 110: {
        { const m = _Expr$null_(c, src); if (m !== null) return m; }
        { const m = _Expr$new_(c, src); if (m !== null) return m; }
        { const m = _Expr$new_2(c, src); if (m !== null) return m; }
        { const m = _Expr$new_3(c, src); if (m !== null) return m; }
        break;
      }
      case 115: {
        { const m = _Expr$super_(c, src); if (m !== null) return m; }
        break;
      }
      case 116: {
        { const m = _Expr$true_(c, src); if (m !== null) return m; }
        { const m = _Expr$this_(c, src); if (m !== null) return m; }
        break;
      }
      case 117: {
        { const m = _Expr$undefined(c, src); if (m !== null) return m; }
        break;
      }
      case 121: {
        { const m = _Expr$yield_(c, src); if (m !== null) return m; }
        break;
      }
      case 123: {
        { const m = _Expr$brace(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "$operator": {
        { const m = _Expr$prefixOp(c, src); if (m !== null) return m; }
        break;
      }
      case "BigInt": {
        { const m = _Expr$bigInt(c, src); if (m !== null) return m; }
        break;
      }
      case "BinaryNumber": {
        { const m = _Expr$binaryNumber(c, src); if (m !== null) return m; }
        break;
      }
      case "HexNumber": {
        { const m = _Expr$hexNumber(c, src); if (m !== null) return m; }
        break;
      }
      case "Ident": {
        { const m = _Expr$ident(c, src); if (m !== null) return m; }
        { const m = _Expr$ident2(c, src); if (m !== null) return m; }
        break;
      }
      case "Number": {
        { const m = _Expr$number(c, src); if (m !== null) return m; }
        break;
      }
      case "OctalNumber": {
        { const m = _Expr$octalNumber(c, src); if (m !== null) return m; }
        break;
      }
      case "PrivateField": {
        { const m = _Expr$privateField(c, src); if (m !== null) return m; }
        break;
      }
      case "Regex": {
        { const m = _Expr$regex(c, src); if (m !== null) return m; }
        break;
      }
      case "String": {
        { const m = _Expr$string(c, src); if (m !== null) return m; }
        break;
      }
      case "Template": {
        { const m = _Expr$template(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchExpr: no arm matches" + ' @' + n.offset);
}

export type PropMatch =
  | { arm: "spread"; expr: ExprNode }
  | { arm: "public"; publicKw: ("public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async" | "export" | "declare" | "in" | "out")[]; getKw: "get" | "set"; memberName: MemberNameNode; param: (ParamNode)[]; type?: TypeNode; block: BlockNode }
  | { arm: "public2"; publicKw: ("public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async" | "export" | "declare" | "in" | "out")[]; memberName: MemberNameNode; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode; block: BlockNode }
  | { arm: "async"; asyncTok?: CstLeaf; memberName: MemberNameNode; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode; block: BlockNode }
  | { arm: "public3"; publicKw: ("public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async" | "export" | "declare" | "in" | "out")[]; memberName: MemberNameNode; expr: ExprNode }
  | { arm: "memberName"; memberName: MemberNameNode; expr: ExprNode }
  | { arm: "bracket"; expr: ExprNode; expr2: (ExprNode)[]; expr3: ExprNode }
  | { arm: "ident"; ident: CstLeaf; expr?: ExprNode };

function _Prop$spread(c: readonly CstChild[], src: string): PropMatch | null {
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "...", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "spread", expr: expr! };
}

function _Prop$public(c: readonly CstChild[], src: string): PropMatch | null {
  const publicKw: ("public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async" | "export" | "declare" | "in" | "out")[] = [];
  let getKw: ("get" | "set") | undefined;
  let memberName: (MemberNameNode) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let block: (BlockNode) | undefined;
  let i = 0;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!(__lit(c, i, src, "public", "$keyword") || __lit(c, i, src, "private", "$keyword") || __lit(c, i, src, "protected", "$keyword") || __lit(c, i, src, "static", "$keyword") || __lit(c, i, src, "abstract", "$keyword") || __lit(c, i, src, "readonly", "$keyword") || __lit(c, i, src, "override", "$keyword") || __lit(c, i, src, "accessor", "$keyword") || __lit(c, i, src, "async", "$keyword") || __lit(c, i, src, "export", "$keyword") || __lit(c, i, src, "declare", "$keyword") || __lit(c, i, src, "in", "$keyword") || __lit(c, i, src, "out", "$keyword"))) { _t1 = false; break _b2; }
      publicKw.push(src.slice(c[i].offset, c[i].end) as "public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async" | "export" | "declare" | "in" | "out");
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!(__lit(c, i, src, "get", "$keyword") || __lit(c, i, src, "set", "$keyword"))) return null;
  getKw = src.slice(c[i].offset, c[i].end) as "get" | "set";
  i++;
  if (!__nodeOf(c, i, "MemberName")) return null;
  memberName = c[i] as MemberNameNode;
  i++;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      {
        const _t6 = i; let _t9 = true;
        _b10: {
          if (!__nodeOf(c, i, "Param")) { _t9 = false; break _b10; }
          param.push(c[i] as ParamNode);
          i++;
        }
        if (!_t9) { i = _t6; }
        else for (;;) {
          if (!__lit(c, i, src, ",", "$punct")) break;
          i++;
          const _t62 = i; let _t7 = true;
          _b8: {
            if (!__nodeOf(c, i, "Param")) { _t7 = false; break _b8; }
            param.push(c[i] as ParamNode);
            i++;
          }
          if (!_t7) { i = _t62; break; }
        }
      }
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  {
    const _t11 = i; let _t12 = true;
    _b13: {
      if (!__lit(c, i, src, ":", "$punct")) { _t12 = false; break _b13; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t12 = false; break _b13; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t12) i = _t11;
  }
  if (!__nodeOf(c, i, "Block")) return null;
  block = c[i] as BlockNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "public", publicKw, getKw: getKw!, memberName: memberName!, param, type, block: block! };
}

function _Prop$public2(c: readonly CstChild[], src: string): PropMatch | null {
  const publicKw: ("public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async" | "export" | "declare" | "in" | "out")[] = [];
  let memberName: (MemberNameNode) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let block: (BlockNode) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!(__lit(c, i, src, "public", "$keyword") || __lit(c, i, src, "private", "$keyword") || __lit(c, i, src, "protected", "$keyword") || __lit(c, i, src, "static", "$keyword") || __lit(c, i, src, "abstract", "$keyword") || __lit(c, i, src, "readonly", "$keyword") || __lit(c, i, src, "override", "$keyword") || __lit(c, i, src, "accessor", "$keyword") || __lit(c, i, src, "async", "$keyword") || __lit(c, i, src, "export", "$keyword") || __lit(c, i, src, "declare", "$keyword") || __lit(c, i, src, "in", "$keyword") || __lit(c, i, src, "out", "$keyword"))) { _t1 = false; break _b2; }
      publicKw.push(src.slice(c[i].offset, c[i].end) as "public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async" | "export" | "declare" | "in" | "out");
      i++;
    }
    if (!_t1) return null;
  }
  for (;;) {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!(__lit(c, i, src, "public", "$keyword") || __lit(c, i, src, "private", "$keyword") || __lit(c, i, src, "protected", "$keyword") || __lit(c, i, src, "static", "$keyword") || __lit(c, i, src, "abstract", "$keyword") || __lit(c, i, src, "readonly", "$keyword") || __lit(c, i, src, "override", "$keyword") || __lit(c, i, src, "accessor", "$keyword") || __lit(c, i, src, "async", "$keyword") || __lit(c, i, src, "export", "$keyword") || __lit(c, i, src, "declare", "$keyword") || __lit(c, i, src, "in", "$keyword") || __lit(c, i, src, "out", "$keyword"))) { _t4 = false; break _b5; }
      publicKw.push(src.slice(c[i].offset, c[i].end) as "public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async" | "export" | "declare" | "in" | "out");
      i++;
    }
    if (!_t4) { i = _t3; break; }
    if (i === _t3) break;
  }
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__lit(c, i, src, "*", "$punct")) { _t7 = false; break _b8; }
      i++;
    }
    if (!_t7) i = _t6;
  }
  if (!__nodeOf(c, i, "MemberName")) return null;
  memberName = c[i] as MemberNameNode;
  i++;
  {
    const _t9 = i; let _t10 = true;
    _b11: {
      if (!__lit(c, i, src, "?", "$punct")) { _t10 = false; break _b11; }
      i++;
    }
    if (!_t10) i = _t9;
  }
  {
    const _t12 = i; let _t13 = true;
    _b14: {
      if (!__lit(c, i, src, "!", "$punct")) { _t13 = false; break _b14; }
      i++;
    }
    if (!_t13) i = _t12;
  }
  {
    const _t15 = i; let _t16 = true;
    _b17: {
      if (!__nodeOf(c, i, "TypeParams")) { _t16 = false; break _b17; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t16) i = _t15;
  }
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t18 = i; let _t21 = true;
    _b22: {
      if (!__nodeOf(c, i, "Param")) { _t21 = false; break _b22; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t21) { i = _t18; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t182 = i; let _t19 = true;
      _b20: {
        if (!__nodeOf(c, i, "Param")) { _t19 = false; break _b20; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t19) { i = _t182; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  {
    const _t23 = i; let _t24 = true;
    _b25: {
      if (!__lit(c, i, src, ":", "$punct")) { _t24 = false; break _b25; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t24 = false; break _b25; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t24) i = _t23;
  }
  if (!__nodeOf(c, i, "Block")) return null;
  block = c[i] as BlockNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "public2", publicKw, memberName: memberName!, typeParams, param, type, block: block! };
}

function _Prop$async(c: readonly CstChild[], src: string): PropMatch | null {
  let asyncTok: (CstLeaf) | undefined;
  let memberName: (MemberNameNode) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let block: (BlockNode) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "async", "$keyword")) { _t1 = false; break _b2; }
      asyncTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "*", "$punct")) { _t4 = false; break _b5; }
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!__nodeOf(c, i, "MemberName")) return null;
  memberName = c[i] as MemberNameNode;
  i++;
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__lit(c, i, src, "?", "$punct")) { _t7 = false; break _b8; }
      i++;
    }
    if (!_t7) i = _t6;
  }
  {
    const _t9 = i; let _t10 = true;
    _b11: {
      if (!__lit(c, i, src, "!", "$punct")) { _t10 = false; break _b11; }
      i++;
    }
    if (!_t10) i = _t9;
  }
  {
    const _t12 = i; let _t13 = true;
    _b14: {
      if (!__nodeOf(c, i, "TypeParams")) { _t13 = false; break _b14; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t13) i = _t12;
  }
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t15 = i; let _t18 = true;
    _b19: {
      if (!__nodeOf(c, i, "Param")) { _t18 = false; break _b19; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t18) { i = _t15; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t152 = i; let _t16 = true;
      _b17: {
        if (!__nodeOf(c, i, "Param")) { _t16 = false; break _b17; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t16) { i = _t152; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  {
    const _t20 = i; let _t21 = true;
    _b22: {
      if (!__lit(c, i, src, ":", "$punct")) { _t21 = false; break _b22; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t21 = false; break _b22; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t21) i = _t20;
  }
  if (!__nodeOf(c, i, "Block")) return null;
  block = c[i] as BlockNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "async", asyncTok, memberName: memberName!, typeParams, param, type, block: block! };
}

function _Prop$public3(c: readonly CstChild[], src: string): PropMatch | null {
  const publicKw: ("public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async" | "export" | "declare" | "in" | "out")[] = [];
  let memberName: (MemberNameNode) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!(__lit(c, i, src, "public", "$keyword") || __lit(c, i, src, "private", "$keyword") || __lit(c, i, src, "protected", "$keyword") || __lit(c, i, src, "static", "$keyword") || __lit(c, i, src, "abstract", "$keyword") || __lit(c, i, src, "readonly", "$keyword") || __lit(c, i, src, "override", "$keyword") || __lit(c, i, src, "accessor", "$keyword") || __lit(c, i, src, "async", "$keyword") || __lit(c, i, src, "export", "$keyword") || __lit(c, i, src, "declare", "$keyword") || __lit(c, i, src, "in", "$keyword") || __lit(c, i, src, "out", "$keyword"))) { _t1 = false; break _b2; }
      publicKw.push(src.slice(c[i].offset, c[i].end) as "public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async" | "export" | "declare" | "in" | "out");
      i++;
    }
    if (!_t1) return null;
  }
  for (;;) {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!(__lit(c, i, src, "public", "$keyword") || __lit(c, i, src, "private", "$keyword") || __lit(c, i, src, "protected", "$keyword") || __lit(c, i, src, "static", "$keyword") || __lit(c, i, src, "abstract", "$keyword") || __lit(c, i, src, "readonly", "$keyword") || __lit(c, i, src, "override", "$keyword") || __lit(c, i, src, "accessor", "$keyword") || __lit(c, i, src, "async", "$keyword") || __lit(c, i, src, "export", "$keyword") || __lit(c, i, src, "declare", "$keyword") || __lit(c, i, src, "in", "$keyword") || __lit(c, i, src, "out", "$keyword"))) { _t4 = false; break _b5; }
      publicKw.push(src.slice(c[i].offset, c[i].end) as "public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async" | "export" | "declare" | "in" | "out");
      i++;
    }
    if (!_t4) { i = _t3; break; }
    if (i === _t3) break;
  }
  if (!__nodeOf(c, i, "MemberName")) return null;
  memberName = c[i] as MemberNameNode;
  i++;
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__lit(c, i, src, "?", "$punct")) { _t7 = false; break _b8; }
      i++;
    }
    if (!_t7) i = _t6;
  }
  {
    const _t9 = i; let _t10 = true;
    _b11: {
      if (!__lit(c, i, src, "!", "$punct")) { _t10 = false; break _b11; }
      i++;
    }
    if (!_t10) i = _t9;
  }
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "public3", publicKw, memberName: memberName!, expr: expr! };
}

function _Prop$memberName(c: readonly CstChild[], src: string): PropMatch | null {
  let memberName: (MemberNameNode) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "MemberName")) return null;
  memberName = c[i] as MemberNameNode;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "?", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "!", "$punct")) { _t4 = false; break _b5; }
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "memberName", memberName: memberName!, expr: expr! };
}

function _Prop$bracket(c: readonly CstChild[], src: string): PropMatch | null {
  let expr: (ExprNode) | undefined;
  const expr2: (ExprNode)[] = [];
  let expr3: (ExprNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "[", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, ",", "$punct")) { _t1 = false; break _b2; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
      expr2.push(c[i] as ExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, "]", "$punct")) return null;
  i++;
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr3 = c[i] as ExprNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "bracket", expr: expr!, expr2, expr3: expr3! };
}

function _Prop$ident(c: readonly CstChild[], src: string): PropMatch | null {
  let ident: (CstLeaf) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "?", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "!", "$punct")) { _t4 = false; break _b5; }
      i++;
    }
    if (!_t4) i = _t3;
  }
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__lit(c, i, src, "=", "$punct")) { _t7 = false; break _b8; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t7 = false; break _b8; }
      expr = c[i] as ExprNode;
      i++;
    }
    if (!_t7) i = _t6;
  }
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident!, expr };
}

export function matchProp(n: PropNode, src: string): PropMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "MemberName": {
        { const m = _Prop$async(c, src); if (m !== null) return m; }
        { const m = _Prop$memberName(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 42: {
        { const m = _Prop$async(c, src); if (m !== null) return m; }
        break;
      }
      case 46: {
        { const m = _Prop$spread(c, src); if (m !== null) return m; }
        break;
      }
      case 91: {
        { const m = _Prop$bracket(c, src); if (m !== null) return m; }
        break;
      }
      case 97: {
        { const m = _Prop$public(c, src); if (m !== null) return m; }
        { const m = _Prop$public2(c, src); if (m !== null) return m; }
        { const m = _Prop$async(c, src); if (m !== null) return m; }
        { const m = _Prop$public3(c, src); if (m !== null) return m; }
        break;
      }
      case 100: {
        { const m = _Prop$public(c, src); if (m !== null) return m; }
        { const m = _Prop$public2(c, src); if (m !== null) return m; }
        { const m = _Prop$public3(c, src); if (m !== null) return m; }
        break;
      }
      case 101: {
        { const m = _Prop$public(c, src); if (m !== null) return m; }
        { const m = _Prop$public2(c, src); if (m !== null) return m; }
        { const m = _Prop$public3(c, src); if (m !== null) return m; }
        break;
      }
      case 103: {
        { const m = _Prop$public(c, src); if (m !== null) return m; }
        break;
      }
      case 105: {
        { const m = _Prop$public(c, src); if (m !== null) return m; }
        { const m = _Prop$public2(c, src); if (m !== null) return m; }
        { const m = _Prop$public3(c, src); if (m !== null) return m; }
        break;
      }
      case 111: {
        { const m = _Prop$public(c, src); if (m !== null) return m; }
        { const m = _Prop$public2(c, src); if (m !== null) return m; }
        { const m = _Prop$public3(c, src); if (m !== null) return m; }
        break;
      }
      case 112: {
        { const m = _Prop$public(c, src); if (m !== null) return m; }
        { const m = _Prop$public2(c, src); if (m !== null) return m; }
        { const m = _Prop$public3(c, src); if (m !== null) return m; }
        break;
      }
      case 114: {
        { const m = _Prop$public(c, src); if (m !== null) return m; }
        { const m = _Prop$public2(c, src); if (m !== null) return m; }
        { const m = _Prop$public3(c, src); if (m !== null) return m; }
        break;
      }
      case 115: {
        { const m = _Prop$public(c, src); if (m !== null) return m; }
        { const m = _Prop$public2(c, src); if (m !== null) return m; }
        { const m = _Prop$public3(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _Prop$ident(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchProp: no arm matches" + ' @' + n.offset);
}

export type MemberNameMatch =
  | { arm: "ident"; ident: CstLeaf }
  | { arm: "privateField"; privateField: CstLeaf }
  | { arm: "string"; string: CstLeaf }
  | { arm: "number"; number: CstLeaf }
  | { arm: "hexNumber"; hexNumber: CstLeaf }
  | { arm: "octalNumber"; octalNumber: CstLeaf }
  | { arm: "binaryNumber"; binaryNumber: CstLeaf }
  | { arm: "bigInt"; bigInt: CstLeaf }
  | { arm: "bracket"; expr: ExprNode };

function _MemberName$ident(c: readonly CstChild[], src: string): MemberNameMatch | null {
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident! };
}

function _MemberName$privateField(c: readonly CstChild[], src: string): MemberNameMatch | null {
  let privateField: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "PrivateField"))) return null;
  privateField = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "privateField", privateField: privateField! };
}

function _MemberName$string(c: readonly CstChild[], src: string): MemberNameMatch | null {
  let string: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "String"))) return null;
  string = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "string", string: string! };
}

function _MemberName$number(c: readonly CstChild[], src: string): MemberNameMatch | null {
  let number: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Number"))) return null;
  number = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "number", number: number! };
}

function _MemberName$hexNumber(c: readonly CstChild[], src: string): MemberNameMatch | null {
  let hexNumber: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "HexNumber"))) return null;
  hexNumber = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "hexNumber", hexNumber: hexNumber! };
}

function _MemberName$octalNumber(c: readonly CstChild[], src: string): MemberNameMatch | null {
  let octalNumber: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "OctalNumber"))) return null;
  octalNumber = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "octalNumber", octalNumber: octalNumber! };
}

function _MemberName$binaryNumber(c: readonly CstChild[], src: string): MemberNameMatch | null {
  let binaryNumber: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "BinaryNumber"))) return null;
  binaryNumber = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "binaryNumber", binaryNumber: binaryNumber! };
}

function _MemberName$bigInt(c: readonly CstChild[], src: string): MemberNameMatch | null {
  let bigInt: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "BigInt"))) return null;
  bigInt = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "bigInt", bigInt: bigInt! };
}

function _MemberName$bracket(c: readonly CstChild[], src: string): MemberNameMatch | null {
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "[", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "]", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "bracket", expr: expr! };
}

export function matchMemberName(n: MemberNameNode, src: string): MemberNameMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 91: {
        { const m = _MemberName$bracket(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "BigInt": {
        { const m = _MemberName$bigInt(c, src); if (m !== null) return m; }
        break;
      }
      case "BinaryNumber": {
        { const m = _MemberName$binaryNumber(c, src); if (m !== null) return m; }
        break;
      }
      case "HexNumber": {
        { const m = _MemberName$hexNumber(c, src); if (m !== null) return m; }
        break;
      }
      case "Ident": {
        { const m = _MemberName$ident(c, src); if (m !== null) return m; }
        break;
      }
      case "Number": {
        { const m = _MemberName$number(c, src); if (m !== null) return m; }
        break;
      }
      case "OctalNumber": {
        { const m = _MemberName$octalNumber(c, src); if (m !== null) return m; }
        break;
      }
      case "PrivateField": {
        { const m = _MemberName$privateField(c, src); if (m !== null) return m; }
        break;
      }
      case "String": {
        { const m = _MemberName$string(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchMemberName: no arm matches" + ' @' + n.offset);
}

export type NewTargetMatch =
  | { arm: "ident"; ident: CstLeaf }
  | { arm: "led_dot"; left: NewTargetNode; ident: CstLeaf }
  | { arm: "led_bracket"; left: NewTargetNode; expr: ExprNode }
  | { arm: "paren"; expr: ExprNode };

function _NewTarget$ident(c: readonly CstChild[], src: string): NewTargetMatch | null {
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident! };
}

function _NewTarget$led_dot(c: readonly CstChild[], src: string): NewTargetMatch | null {
  let left: (NewTargetNode) | undefined;
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "NewTarget")) return null;
  left = c[i] as NewTargetNode;
  i++;
  if (!__lit(c, i, src, ".", "$punct")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_dot", left: left!, ident: ident! };
}

function _NewTarget$led_bracket(c: readonly CstChild[], src: string): NewTargetMatch | null {
  let left: (NewTargetNode) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "NewTarget")) return null;
  left = c[i] as NewTargetNode;
  i++;
  if (!__lit(c, i, src, "[", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "]", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_bracket", left: left!, expr: expr! };
}

function _NewTarget$paren(c: readonly CstChild[], src: string): NewTargetMatch | null {
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "paren", expr: expr! };
}

export function matchNewTarget(n: NewTargetNode, src: string): NewTargetMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "NewTarget": {
        { const m = _NewTarget$led_dot(c, src); if (m !== null) return m; }
        { const m = _NewTarget$led_bracket(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 40: {
        { const m = _NewTarget$paren(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _NewTarget$ident(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchNewTarget: no arm matches" + ' @' + n.offset);
}

export type ClassHeritageMatch =
  | { arm: "ident"; ident: CstLeaf }
  | { arm: "number"; number: CstLeaf }
  | { arm: "string"; string: CstLeaf }
  | { arm: "true_" }
  | { arm: "false_" }
  | { arm: "null_" }
  | { arm: "undefined" }
  | { arm: "paren"; expr: ExprNode }
  | { arm: "class_"; ident?: CstLeaf; typeParams?: TypeParamsNode; classHeritage?: ClassHeritageNode; extendsTok?: CstLeaf; type: (TypeNode)[]; implementsTok?: CstLeaf; classMember: (ClassMemberNode)[] }
  | { arm: "led_dot"; left: ClassHeritageNode; ident: CstLeaf }
  | { arm: "led_optChain"; left: ClassHeritageNode; ident: CstLeaf }
  | { arm: "led_lt"; left: ClassHeritageNode; type: (TypeNode)[] }
  | { arm: "led_paren"; left: ClassHeritageNode; expr: (ExprNode)[] };

function _ClassHeritage$ident(c: readonly CstChild[], src: string): ClassHeritageMatch | null {
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident! };
}

function _ClassHeritage$number(c: readonly CstChild[], src: string): ClassHeritageMatch | null {
  let number: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Number"))) return null;
  number = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "number", number: number! };
}

function _ClassHeritage$string(c: readonly CstChild[], src: string): ClassHeritageMatch | null {
  let string: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "String"))) return null;
  string = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "string", string: string! };
}

function _ClassHeritage$true_(c: readonly CstChild[], src: string): ClassHeritageMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "true", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "true_" };
}

function _ClassHeritage$false_(c: readonly CstChild[], src: string): ClassHeritageMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "false", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "false_" };
}

function _ClassHeritage$null_(c: readonly CstChild[], src: string): ClassHeritageMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "null", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "null_" };
}

function _ClassHeritage$undefined(c: readonly CstChild[], src: string): ClassHeritageMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "undefined", "$keyword")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "undefined" };
}

function _ClassHeritage$paren(c: readonly CstChild[], src: string): ClassHeritageMatch | null {
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "paren", expr: expr! };
}

function _ClassHeritage$class_(c: readonly CstChild[], src: string): ClassHeritageMatch | null {
  let ident: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  let classHeritage: (ClassHeritageNode) | undefined;
  let extendsTok: (CstLeaf) | undefined;
  const type: (TypeNode)[] = [];
  let implementsTok: (CstLeaf) | undefined;
  const classMember: (ClassMemberNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "class", "$keyword")) return null;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!(__tok(c, i, "Ident"))) { _t1 = false; break _b2; }
      ident = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__nodeOf(c, i, "TypeParams")) { _t4 = false; break _b5; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t4) i = _t3;
  }
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__lit(c, i, src, "extends", "$keyword")) { _t7 = false; break _b8; }
      extendsTok = c[i] as CstLeaf;
      i++;
      if (!__nodeOf(c, i, "ClassHeritage")) { _t7 = false; break _b8; }
      classHeritage = c[i] as ClassHeritageNode;
      i++;
    }
    if (!_t7) i = _t6;
  }
  {
    const _t9 = i; let _t10 = true;
    _b11: {
      if (!__lit(c, i, src, "implements", "$keyword")) { _t10 = false; break _b11; }
      implementsTok = c[i] as CstLeaf;
      i++;
      {
        const _t12 = i; let _t15 = true;
        _b16: {
          if (!__nodeOf(c, i, "Type")) { _t15 = false; break _b16; }
          type.push(c[i] as TypeNode);
          i++;
        }
        if (!_t15) { i = _t12; }
        else for (;;) {
          if (!__lit(c, i, src, ",", "$punct")) break;
          i++;
          const _t122 = i; let _t13 = true;
          _b14: {
            if (!__nodeOf(c, i, "Type")) { _t13 = false; break _b14; }
            type.push(c[i] as TypeNode);
            i++;
          }
          if (!_t13) { i = _t122; break; }
        }
      }
    }
    if (!_t10) i = _t9;
  }
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  for (;;) {
    const _t17 = i; let _t18 = true;
    _b19: {
      if (!__nodeOf(c, i, "ClassMember")) { _t18 = false; break _b19; }
      classMember.push(c[i] as ClassMemberNode);
      i++;
    }
    if (!_t18) { i = _t17; break; }
    if (i === _t17) break;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "class_", ident, typeParams, classHeritage, extendsTok, type, implementsTok, classMember };
}

function _ClassHeritage$led_dot(c: readonly CstChild[], src: string): ClassHeritageMatch | null {
  let left: (ClassHeritageNode) | undefined;
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "ClassHeritage")) return null;
  left = c[i] as ClassHeritageNode;
  i++;
  if (!__lit(c, i, src, ".", "$punct")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_dot", left: left!, ident: ident! };
}

function _ClassHeritage$led_optChain(c: readonly CstChild[], src: string): ClassHeritageMatch | null {
  let left: (ClassHeritageNode) | undefined;
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "ClassHeritage")) return null;
  left = c[i] as ClassHeritageNode;
  i++;
  if (!__lit(c, i, src, "?.", "$punct")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_optChain", left: left!, ident: ident! };
}

function _ClassHeritage$led_lt(c: readonly CstChild[], src: string): ClassHeritageMatch | null {
  let left: (ClassHeritageNode) | undefined;
  const type: (TypeNode)[] = [];
  let i = 0;
  if (!__nodeOf(c, i, "ClassHeritage")) return null;
  left = c[i] as ClassHeritageNode;
  i++;
  if (!__lit(c, i, src, "<", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "Type")) { _t3 = false; break _b4; }
      type.push(c[i] as TypeNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "Type")) { _t1 = false; break _b2; }
        type.push(c[i] as TypeNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, ">", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_lt", left: left!, type };
}

function _ClassHeritage$led_paren(c: readonly CstChild[], src: string): ClassHeritageMatch | null {
  let left: (ClassHeritageNode) | undefined;
  const expr: (ExprNode)[] = [];
  let i = 0;
  if (!__nodeOf(c, i, "ClassHeritage")) return null;
  left = c[i] as ClassHeritageNode;
  i++;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "Expr")) { _t3 = false; break _b4; }
      expr.push(c[i] as ExprNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
        expr.push(c[i] as ExprNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "led_paren", left: left!, expr };
}

export function matchClassHeritage(n: ClassHeritageNode, src: string): ClassHeritageMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "ClassHeritage": {
        { const m = _ClassHeritage$led_dot(c, src); if (m !== null) return m; }
        { const m = _ClassHeritage$led_optChain(c, src); if (m !== null) return m; }
        { const m = _ClassHeritage$led_lt(c, src); if (m !== null) return m; }
        { const m = _ClassHeritage$led_paren(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 40: {
        { const m = _ClassHeritage$paren(c, src); if (m !== null) return m; }
        break;
      }
      case 99: {
        { const m = _ClassHeritage$class_(c, src); if (m !== null) return m; }
        break;
      }
      case 102: {
        { const m = _ClassHeritage$false_(c, src); if (m !== null) return m; }
        break;
      }
      case 110: {
        { const m = _ClassHeritage$null_(c, src); if (m !== null) return m; }
        break;
      }
      case 116: {
        { const m = _ClassHeritage$true_(c, src); if (m !== null) return m; }
        break;
      }
      case 117: {
        { const m = _ClassHeritage$undefined(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _ClassHeritage$ident(c, src); if (m !== null) return m; }
        break;
      }
      case "Number": {
        { const m = _ClassHeritage$number(c, src); if (m !== null) return m; }
        break;
      }
      case "String": {
        { const m = _ClassHeritage$string(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchClassHeritage: no arm matches" + ' @' + n.offset);
}

export type StmtMatch =
  | { arm: "block"; block: BlockNode }
  | { arm: "let_"; let_Kw: "let" | "const" | "var"; binding: (BindingNode)[] }
  | { arm: "if_"; expr: ExprNode; expr2: (ExprNode)[]; stmt: StmtNode; stmt2?: StmtNode; elseTok?: CstLeaf }
  | { arm: "for_"; awaitTok?: CstLeaf; forHead: ForHeadNode; stmt: StmtNode }
  | { arm: "while_"; expr: ExprNode; expr2: (ExprNode)[]; stmt: StmtNode }
  | { arm: "do_"; stmt: StmtNode; expr: ExprNode; expr2: (ExprNode)[] }
  | { arm: "switch_"; expr: ExprNode; expr2: (ExprNode)[]; switchCase: (SwitchCaseNode)[] }
  | { arm: "return_"; expr?: ExprNode; expr2: (ExprNode)[] }
  | { arm: "throw_"; expr: ExprNode; expr2: (ExprNode)[] }
  | { arm: "break_"; ident?: CstLeaf }
  | { arm: "continue_"; ident?: CstLeaf }
  | { arm: "try_"; block: BlockNode; alt?: { branch: "param"; param: ParamNode } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }; block2?: BlockNode; catchTok?: CstLeaf; block3?: BlockNode; finallyTok?: CstLeaf }
  | { arm: "ident"; ident: CstLeaf; stmt: StmtNode }
  | { arm: "semi" }
  | { arm: "debugger_" }
  | { arm: "with_"; expr: ExprNode; stmt: StmtNode }
  | { arm: "await_"; awaitTok?: CstLeaf; binding: (BindingNode)[] }
  | { arm: "decl"; decl: DeclNode }
  | { arm: "expr"; expr: ExprNode; expr2: (ExprNode)[] };

function _Stmt$block(c: readonly CstChild[], src: string): StmtMatch | null {
  let block: (BlockNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Block")) return null;
  block = c[i] as BlockNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "block", block: block! };
}

function _Stmt$let_(c: readonly CstChild[], src: string): StmtMatch | null {
  let let_Kw: ("let" | "const" | "var") | undefined;
  const binding: (BindingNode)[] = [];
  let i = 0;
  if (!(__lit(c, i, src, "let", "$keyword") || __lit(c, i, src, "const", "$keyword") || __lit(c, i, src, "var", "$keyword"))) return null;
  let_Kw = src.slice(c[i].offset, c[i].end) as "let" | "const" | "var";
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "Binding")) { _t3 = false; break _b4; }
      binding.push(c[i] as BindingNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "Binding")) { _t1 = false; break _b2; }
        binding.push(c[i] as BindingNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  {
    const _t5 = i; let _t6 = true;
    _b7: {
      if (!__lit(c, i, src, ";", "$punct")) { _t6 = false; break _b7; }
      i++;
    }
    if (!_t6) i = _t5;
  }
  if (i !== c.length) return null;
  return { arm: "let_", let_Kw: let_Kw!, binding };
}

function _Stmt$if_(c: readonly CstChild[], src: string): StmtMatch | null {
  let expr: (ExprNode) | undefined;
  const expr2: (ExprNode)[] = [];
  let stmt: (StmtNode) | undefined;
  let stmt2: (StmtNode) | undefined;
  let elseTok: (CstLeaf) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "if", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, ",", "$punct")) { _t1 = false; break _b2; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
      expr2.push(c[i] as ExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Stmt")) return null;
  stmt = c[i] as StmtNode;
  i++;
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "else", "$keyword")) { _t4 = false; break _b5; }
      elseTok = c[i] as CstLeaf;
      i++;
      if (!__nodeOf(c, i, "Stmt")) { _t4 = false; break _b5; }
      stmt2 = c[i] as StmtNode;
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (i !== c.length) return null;
  return { arm: "if_", expr: expr!, expr2, stmt: stmt!, stmt2, elseTok };
}

function _Stmt$for_(c: readonly CstChild[], src: string): StmtMatch | null {
  let awaitTok: (CstLeaf) | undefined;
  let forHead: (ForHeadNode) | undefined;
  let stmt: (StmtNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "for", "$keyword")) return null;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "await", "$keyword")) { _t1 = false; break _b2; }
      awaitTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "ForHead")) return null;
  forHead = c[i] as ForHeadNode;
  i++;
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Stmt")) return null;
  stmt = c[i] as StmtNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "for_", awaitTok, forHead: forHead!, stmt: stmt! };
}

function _Stmt$while_(c: readonly CstChild[], src: string): StmtMatch | null {
  let expr: (ExprNode) | undefined;
  const expr2: (ExprNode)[] = [];
  let stmt: (StmtNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "while", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, ",", "$punct")) { _t1 = false; break _b2; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
      expr2.push(c[i] as ExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Stmt")) return null;
  stmt = c[i] as StmtNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "while_", expr: expr!, expr2, stmt: stmt! };
}

function _Stmt$do_(c: readonly CstChild[], src: string): StmtMatch | null {
  let stmt: (StmtNode) | undefined;
  let expr: (ExprNode) | undefined;
  const expr2: (ExprNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "do", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "Stmt")) return null;
  stmt = c[i] as StmtNode;
  i++;
  if (!__lit(c, i, src, "while", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, ",", "$punct")) { _t1 = false; break _b2; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
      expr2.push(c[i] as ExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, ";", "$punct")) { _t4 = false; break _b5; }
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (i !== c.length) return null;
  return { arm: "do_", stmt: stmt!, expr: expr!, expr2 };
}

function _Stmt$switch_(c: readonly CstChild[], src: string): StmtMatch | null {
  let expr: (ExprNode) | undefined;
  const expr2: (ExprNode)[] = [];
  const switchCase: (SwitchCaseNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "switch", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, ",", "$punct")) { _t1 = false; break _b2; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
      expr2.push(c[i] as ExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  for (;;) {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__nodeOf(c, i, "SwitchCase")) { _t4 = false; break _b5; }
      switchCase.push(c[i] as SwitchCaseNode);
      i++;
    }
    if (!_t4) { i = _t3; break; }
    if (i === _t3) break;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "switch_", expr: expr!, expr2, switchCase };
}

function _Stmt$return_(c: readonly CstChild[], src: string): StmtMatch | null {
  let expr: (ExprNode) | undefined;
  const expr2: (ExprNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "return", "$keyword")) return null;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
      expr = c[i] as ExprNode;
      i++;
      for (;;) {
        const _t3 = i; let _t4 = true;
        _b5: {
          if (!__lit(c, i, src, ",", "$punct")) { _t4 = false; break _b5; }
          i++;
          if (!__nodeOf(c, i, "Expr")) { _t4 = false; break _b5; }
          expr2.push(c[i] as ExprNode);
          i++;
        }
        if (!_t4) { i = _t3; break; }
        if (i === _t3) break;
      }
    }
    if (!_t1) i = _t0;
  }
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__lit(c, i, src, ";", "$punct")) { _t7 = false; break _b8; }
      i++;
    }
    if (!_t7) i = _t6;
  }
  if (i !== c.length) return null;
  return { arm: "return_", expr, expr2 };
}

function _Stmt$throw_(c: readonly CstChild[], src: string): StmtMatch | null {
  let expr: (ExprNode) | undefined;
  const expr2: (ExprNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "throw", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, ",", "$punct")) { _t1 = false; break _b2; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
      expr2.push(c[i] as ExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, ";", "$punct")) { _t4 = false; break _b5; }
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (i !== c.length) return null;
  return { arm: "throw_", expr: expr!, expr2 };
}

function _Stmt$break_(c: readonly CstChild[], src: string): StmtMatch | null {
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "break", "$keyword")) return null;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!(__tok(c, i, "Ident"))) { _t1 = false; break _b2; }
      ident = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, ";", "$punct")) { _t4 = false; break _b5; }
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (i !== c.length) return null;
  return { arm: "break_", ident };
}

function _Stmt$continue_(c: readonly CstChild[], src: string): StmtMatch | null {
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "continue", "$keyword")) return null;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!(__tok(c, i, "Ident"))) { _t1 = false; break _b2; }
      ident = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, ";", "$punct")) { _t4 = false; break _b5; }
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (i !== c.length) return null;
  return { arm: "continue_", ident };
}

function _Stmt$try_(c: readonly CstChild[], src: string): StmtMatch | null {
  let block: (BlockNode) | undefined;
  let alt: ({ branch: "param"; param: ParamNode } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }) | undefined;
  let block2: (BlockNode) | undefined;
  let catchTok: (CstLeaf) | undefined;
  let block3: (BlockNode) | undefined;
  let finallyTok: (CstLeaf) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "try", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "Block")) return null;
  block = c[i] as BlockNode;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "catch", "$keyword")) { _t1 = false; break _b2; }
      catchTok = c[i] as CstLeaf;
      i++;
      {
        const _t3 = i; let _t4 = true;
        _b5: {
          if (!__lit(c, i, src, "(", "$punct")) { _t4 = false; break _b5; }
          i++;
          {
            let _t6 = false;
            if (!_t6) {
              let _t10_param: (ParamNode) | undefined;
              const _t7 = i; let _t8 = true;
              _b9: {
                if (!__nodeOf(c, i, "Param")) { _t8 = false; break _b9; }
                _t10_param = c[i] as ParamNode;
                i++;
              }
              if (_t8) { _t6 = true; alt = ({ branch: "param", param: _t10_param! }) as typeof alt; }
              else i = _t7;
            }
            if (!_t6) {
              let _t14_bindingPattern: (BindingPatternNode) | undefined;
              const _t11 = i; let _t12 = true;
              _b13: {
                if (!__nodeOf(c, i, "BindingPattern")) { _t12 = false; break _b13; }
                _t14_bindingPattern = c[i] as BindingPatternNode;
                i++;
              }
              if (_t12) { _t6 = true; alt = ({ branch: "bindingPattern", bindingPattern: _t14_bindingPattern! }) as typeof alt; }
              else i = _t11;
            }
            if (!_t6) { _t4 = false; break _b5; }
          }
          if (!__lit(c, i, src, ")", "$punct")) { _t4 = false; break _b5; }
          i++;
        }
        if (!_t4) i = _t3;
      }
      if (!__nodeOf(c, i, "Block")) { _t1 = false; break _b2; }
      block2 = c[i] as BlockNode;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t15 = i; let _t16 = true;
    _b17: {
      if (!__lit(c, i, src, "finally", "$keyword")) { _t16 = false; break _b17; }
      finallyTok = c[i] as CstLeaf;
      i++;
      if (!__nodeOf(c, i, "Block")) { _t16 = false; break _b17; }
      block3 = c[i] as BlockNode;
      i++;
    }
    if (!_t16) i = _t15;
  }
  if (i !== c.length) return null;
  return { arm: "try_", block: block!, alt, block2, catchTok, block3, finallyTok };
}

function _Stmt$ident(c: readonly CstChild[], src: string): StmtMatch | null {
  let ident: (CstLeaf) | undefined;
  let stmt: (StmtNode) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Stmt")) return null;
  stmt = c[i] as StmtNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident!, stmt: stmt! };
}

function _Stmt$semi(c: readonly CstChild[], src: string): StmtMatch | null {
  let i = 0;
  if (!__lit(c, i, src, ";", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "semi" };
}

function _Stmt$debugger_(c: readonly CstChild[], src: string): StmtMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "debugger", "$keyword")) return null;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, ";", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (i !== c.length) return null;
  return { arm: "debugger_" };
}

function _Stmt$with_(c: readonly CstChild[], src: string): StmtMatch | null {
  let expr: (ExprNode) | undefined;
  let stmt: (StmtNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "with", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Stmt")) return null;
  stmt = c[i] as StmtNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "with_", expr: expr!, stmt: stmt! };
}

function _Stmt$await_(c: readonly CstChild[], src: string): StmtMatch | null {
  let awaitTok: (CstLeaf) | undefined;
  const binding: (BindingNode)[] = [];
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "await", "$keyword")) { _t1 = false; break _b2; }
      awaitTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (!__lit(c, i, src, "using", "$keyword")) return null;
  i++;
  {
    const _t3 = i; let _t6 = true;
    _b7: {
      if (!__nodeOf(c, i, "Binding")) { _t6 = false; break _b7; }
      binding.push(c[i] as BindingNode);
      i++;
    }
    if (!_t6) { i = _t3; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t32 = i; let _t4 = true;
      _b5: {
        if (!__nodeOf(c, i, "Binding")) { _t4 = false; break _b5; }
        binding.push(c[i] as BindingNode);
        i++;
      }
      if (!_t4) { i = _t32; break; }
    }
  }
  {
    const _t8 = i; let _t9 = true;
    _b10: {
      if (!__lit(c, i, src, ";", "$punct")) { _t9 = false; break _b10; }
      i++;
    }
    if (!_t9) i = _t8;
  }
  if (i !== c.length) return null;
  return { arm: "await_", awaitTok, binding };
}

function _Stmt$decl(c: readonly CstChild[], src: string): StmtMatch | null {
  let decl: (DeclNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Decl")) return null;
  decl = c[i] as DeclNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "decl", decl: decl! };
}

function _Stmt$expr(c: readonly CstChild[], src: string): StmtMatch | null {
  let expr: (ExprNode) | undefined;
  const expr2: (ExprNode)[] = [];
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, ",", "$punct")) { _t1 = false; break _b2; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
      expr2.push(c[i] as ExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, ";", "$punct")) { _t4 = false; break _b5; }
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (i !== c.length) return null;
  return { arm: "expr", expr: expr!, expr2 };
}

export function matchStmt(n: StmtNode, src: string): StmtMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "Block": {
        { const m = _Stmt$block(c, src); if (m !== null) return m; }
        break;
      }
      case "Decl": {
        { const m = _Stmt$decl(c, src); if (m !== null) return m; }
        break;
      }
      case "Expr": {
        { const m = _Stmt$expr(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 59: {
        { const m = _Stmt$semi(c, src); if (m !== null) return m; }
        break;
      }
      case 97: {
        { const m = _Stmt$await_(c, src); if (m !== null) return m; }
        break;
      }
      case 98: {
        { const m = _Stmt$break_(c, src); if (m !== null) return m; }
        break;
      }
      case 99: {
        { const m = _Stmt$let_(c, src); if (m !== null) return m; }
        { const m = _Stmt$continue_(c, src); if (m !== null) return m; }
        break;
      }
      case 100: {
        { const m = _Stmt$do_(c, src); if (m !== null) return m; }
        { const m = _Stmt$debugger_(c, src); if (m !== null) return m; }
        break;
      }
      case 102: {
        { const m = _Stmt$for_(c, src); if (m !== null) return m; }
        break;
      }
      case 105: {
        { const m = _Stmt$if_(c, src); if (m !== null) return m; }
        break;
      }
      case 108: {
        { const m = _Stmt$let_(c, src); if (m !== null) return m; }
        break;
      }
      case 114: {
        { const m = _Stmt$return_(c, src); if (m !== null) return m; }
        break;
      }
      case 115: {
        { const m = _Stmt$switch_(c, src); if (m !== null) return m; }
        break;
      }
      case 116: {
        { const m = _Stmt$throw_(c, src); if (m !== null) return m; }
        { const m = _Stmt$try_(c, src); if (m !== null) return m; }
        break;
      }
      case 117: {
        { const m = _Stmt$await_(c, src); if (m !== null) return m; }
        break;
      }
      case 118: {
        { const m = _Stmt$let_(c, src); if (m !== null) return m; }
        break;
      }
      case 119: {
        { const m = _Stmt$while_(c, src); if (m !== null) return m; }
        { const m = _Stmt$with_(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _Stmt$ident(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchStmt: no arm matches" + ' @' + n.offset);
}

export type BlockMatch =
  | { arm: "brace"; stmt: (StmtNode)[] };

function _Block$brace(c: readonly CstChild[], src: string): BlockMatch | null {
  const stmt: (StmtNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "Stmt")) { _t1 = false; break _b2; }
      stmt.push(c[i] as StmtNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "brace", stmt };
}

export function matchBlock(n: BlockNode, src: string): BlockMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 123: {
        { const m = _Block$brace(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
    }
  }
  throw new Error("matchBlock: no arm matches" + ' @' + n.offset);
}

export type BindingPropertyMatch =
  | { arm: "ident"; ident: CstLeaf; bindingElement: BindingElementNode }
  | { arm: "ident2"; ident: CstLeaf; expr?: ExprNode }
  | { arm: "string"; alt: { branch: "string"; string: CstLeaf } | { branch: "number"; number: CstLeaf } | { branch: "bracket"; expr: ExprNode }; bindingElement: BindingElementNode }
  | { arm: "spread"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }; bindingElement?: BindingElementNode; expr?: ExprNode };

function _BindingProperty$ident(c: readonly CstChild[], src: string): BindingPropertyMatch | null {
  let ident: (CstLeaf) | undefined;
  let bindingElement: (BindingElementNode) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "BindingElement")) return null;
  bindingElement = c[i] as BindingElementNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident!, bindingElement: bindingElement! };
}

function _BindingProperty$ident2(c: readonly CstChild[], src: string): BindingPropertyMatch | null {
  let ident: (CstLeaf) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "=", "$punct")) { _t1 = false; break _b2; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
      expr = c[i] as ExprNode;
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (i !== c.length) return null;
  return { arm: "ident2", ident: ident!, expr };
}

function _BindingProperty$string(c: readonly CstChild[], src: string): BindingPropertyMatch | null {
  let alt: ({ branch: "string"; string: CstLeaf } | { branch: "number"; number: CstLeaf } | { branch: "bracket"; expr: ExprNode }) | undefined;
  let bindingElement: (BindingElementNode) | undefined;
  let i = 0;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_string: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "String"))) { _t2 = false; break _b3; }
        _t4_string = c[i] as CstLeaf;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "string", string: _t4_string! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_number: (CstLeaf) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!(__tok(c, i, "Number"))) { _t6 = false; break _b7; }
        _t8_number = c[i] as CstLeaf;
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "number", number: _t8_number! }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) {
      let _t12_expr: (ExprNode) | undefined;
      const _t9 = i; let _t10 = true;
      _b11: {
        if (!__lit(c, i, src, "[", "$punct")) { _t10 = false; break _b11; }
        i++;
        if (!__nodeOf(c, i, "Expr")) { _t10 = false; break _b11; }
        _t12_expr = c[i] as ExprNode;
        i++;
        if (!__lit(c, i, src, "]", "$punct")) { _t10 = false; break _b11; }
        i++;
      }
      if (_t10) { _t0 = true; alt = ({ branch: "bracket", expr: _t12_expr! }) as typeof alt; }
      else i = _t9;
    }
    if (!_t0) return null;
  }
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "BindingElement")) return null;
  bindingElement = c[i] as BindingElementNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "string", alt: alt!, bindingElement: bindingElement! };
}

function _BindingProperty$spread(c: readonly CstChild[], src: string): BindingPropertyMatch | null {
  let alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }) | undefined;
  let bindingElement: (BindingElementNode) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "...", "$punct")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_ident: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "Ident"))) { _t2 = false; break _b3; }
        _t4_ident = c[i] as CstLeaf;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "ident", ident: _t4_ident! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_bindingPattern: (BindingPatternNode) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!__nodeOf(c, i, "BindingPattern")) { _t6 = false; break _b7; }
        _t8_bindingPattern = c[i] as BindingPatternNode;
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "bindingPattern", bindingPattern: _t8_bindingPattern! }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) return null;
  }
  {
    const _t9 = i; let _t10 = true;
    _b11: {
      if (!__lit(c, i, src, ":", "$punct")) { _t10 = false; break _b11; }
      i++;
      if (!__nodeOf(c, i, "BindingElement")) { _t10 = false; break _b11; }
      bindingElement = c[i] as BindingElementNode;
      i++;
    }
    if (!_t10) i = _t9;
  }
  {
    const _t12 = i; let _t13 = true;
    _b14: {
      if (!__lit(c, i, src, "=", "$punct")) { _t13 = false; break _b14; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t13 = false; break _b14; }
      expr = c[i] as ExprNode;
      i++;
    }
    if (!_t13) i = _t12;
  }
  if (i !== c.length) return null;
  return { arm: "spread", alt: alt!, bindingElement, expr };
}

export function matchBindingProperty(n: BindingPropertyNode, src: string): BindingPropertyMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 46: {
        { const m = _BindingProperty$spread(c, src); if (m !== null) return m; }
        break;
      }
      case 91: {
        { const m = _BindingProperty$string(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _BindingProperty$ident(c, src); if (m !== null) return m; }
        { const m = _BindingProperty$ident2(c, src); if (m !== null) return m; }
        break;
      }
      case "Number": {
        { const m = _BindingProperty$string(c, src); if (m !== null) return m; }
        break;
      }
      case "String": {
        { const m = _BindingProperty$string(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchBindingProperty: no arm matches" + ' @' + n.offset);
}

export type BindingElementMatch =
  | { arm: "seq"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }; expr?: ExprNode };

function _BindingElement$seq(c: readonly CstChild[], src: string): BindingElementMatch | null {
  let alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_ident: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "Ident"))) { _t2 = false; break _b3; }
        _t4_ident = c[i] as CstLeaf;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "ident", ident: _t4_ident! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_bindingPattern: (BindingPatternNode) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!__nodeOf(c, i, "BindingPattern")) { _t6 = false; break _b7; }
        _t8_bindingPattern = c[i] as BindingPatternNode;
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "bindingPattern", bindingPattern: _t8_bindingPattern! }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) return null;
  }
  {
    const _t9 = i; let _t10 = true;
    _b11: {
      if (!__lit(c, i, src, "=", "$punct")) { _t10 = false; break _b11; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t10 = false; break _b11; }
      expr = c[i] as ExprNode;
      i++;
    }
    if (!_t10) i = _t9;
  }
  if (i !== c.length) return null;
  return { arm: "seq", alt: alt!, expr };
}

export function matchBindingElement(n: BindingElementNode, src: string): BindingElementMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "BindingPattern": {
        { const m = _BindingElement$seq(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _BindingElement$seq(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchBindingElement: no arm matches" + ' @' + n.offset);
}

export type ArrayBindingElementMatch =
  | { arm: "bindingElement"; bindingElement: BindingElementNode }
  | { arm: "spread"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode } };

function _ArrayBindingElement$bindingElement(c: readonly CstChild[], src: string): ArrayBindingElementMatch | null {
  let bindingElement: (BindingElementNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "BindingElement")) return null;
  bindingElement = c[i] as BindingElementNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "bindingElement", bindingElement: bindingElement! };
}

function _ArrayBindingElement$spread(c: readonly CstChild[], src: string): ArrayBindingElementMatch | null {
  let alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "...", "$punct")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_ident: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "Ident"))) { _t2 = false; break _b3; }
        _t4_ident = c[i] as CstLeaf;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "ident", ident: _t4_ident! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_bindingPattern: (BindingPatternNode) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!__nodeOf(c, i, "BindingPattern")) { _t6 = false; break _b7; }
        _t8_bindingPattern = c[i] as BindingPatternNode;
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "bindingPattern", bindingPattern: _t8_bindingPattern! }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) return null;
  }
  if (i !== c.length) return null;
  return { arm: "spread", alt: alt! };
}

export function matchArrayBindingElement(n: ArrayBindingElementNode, src: string): ArrayBindingElementMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "BindingElement": {
        { const m = _ArrayBindingElement$bindingElement(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 46: {
        { const m = _ArrayBindingElement$spread(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
    }
  }
  throw new Error("matchArrayBindingElement: no arm matches" + ' @' + n.offset);
}

export type BindingPatternMatch =
  | { arm: "brace"; bindingProperty: (BindingPropertyNode)[] }
  | { arm: "bracket"; arrayBindingElement?: ArrayBindingElementNode; arrayBindingElement2: (ArrayBindingElementNode)[] };

function _BindingPattern$brace(c: readonly CstChild[], src: string): BindingPatternMatch | null {
  const bindingProperty: (BindingPropertyNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "BindingProperty")) { _t3 = false; break _b4; }
      bindingProperty.push(c[i] as BindingPropertyNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "BindingProperty")) { _t1 = false; break _b2; }
        bindingProperty.push(c[i] as BindingPropertyNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "brace", bindingProperty };
}

function _BindingPattern$bracket(c: readonly CstChild[], src: string): BindingPatternMatch | null {
  let arrayBindingElement: (ArrayBindingElementNode) | undefined;
  const arrayBindingElement2: (ArrayBindingElementNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "[", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "ArrayBindingElement")) { _t1 = false; break _b2; }
      arrayBindingElement = c[i] as ArrayBindingElementNode;
      i++;
    }
    if (!_t1) i = _t0;
  }
  for (;;) {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, ",", "$punct")) { _t4 = false; break _b5; }
      i++;
      {
        const _t6 = i; let _t7 = true;
        _b8: {
          if (!__nodeOf(c, i, "ArrayBindingElement")) { _t7 = false; break _b8; }
          arrayBindingElement2.push(c[i] as ArrayBindingElementNode);
          i++;
        }
        if (!_t7) i = _t6;
      }
    }
    if (!_t4) { i = _t3; break; }
    if (i === _t3) break;
  }
  if (!__lit(c, i, src, "]", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "bracket", arrayBindingElement, arrayBindingElement2 };
}

export function matchBindingPattern(n: BindingPatternNode, src: string): BindingPatternMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 91: {
        { const m = _BindingPattern$bracket(c, src); if (m !== null) return m; }
        break;
      }
      case 123: {
        { const m = _BindingPattern$brace(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
    }
  }
  throw new Error("matchBindingPattern: no arm matches" + ' @' + n.offset);
}

export type BindingMatch =
  | { arm: "seq"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }; type?: TypeNode; expr?: ExprNode };

function _Binding$seq(c: readonly CstChild[], src: string): BindingMatch | null {
  let alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }) | undefined;
  let type: (TypeNode) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_ident: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "Ident"))) { _t2 = false; break _b3; }
        _t4_ident = c[i] as CstLeaf;
        i++;
        {
          const _t5 = i; let _t6 = true;
          _b7: {
            if (!__lit(c, i, src, "!", "$punct")) { _t6 = false; break _b7; }
            i++;
          }
          if (!_t6) i = _t5;
        }
      }
      if (_t2) { _t0 = true; alt = ({ branch: "ident", ident: _t4_ident! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t11_bindingPattern: (BindingPatternNode) | undefined;
      const _t8 = i; let _t9 = true;
      _b10: {
        if (!__nodeOf(c, i, "BindingPattern")) { _t9 = false; break _b10; }
        _t11_bindingPattern = c[i] as BindingPatternNode;
        i++;
      }
      if (_t9) { _t0 = true; alt = ({ branch: "bindingPattern", bindingPattern: _t11_bindingPattern! }) as typeof alt; }
      else i = _t8;
    }
    if (!_t0) return null;
  }
  {
    const _t12 = i; let _t13 = true;
    _b14: {
      if (!__lit(c, i, src, ":", "$punct")) { _t13 = false; break _b14; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t13 = false; break _b14; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t13) i = _t12;
  }
  {
    const _t15 = i; let _t16 = true;
    _b17: {
      if (!__lit(c, i, src, "=", "$punct")) { _t16 = false; break _b17; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t16 = false; break _b17; }
      expr = c[i] as ExprNode;
      i++;
    }
    if (!_t16) i = _t15;
  }
  if (i !== c.length) return null;
  return { arm: "seq", alt: alt!, type, expr };
}

export function matchBinding(n: BindingNode, src: string): BindingMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "BindingPattern": {
        { const m = _Binding$seq(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _Binding$seq(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchBinding: no arm matches" + ' @' + n.offset);
}

export type ForBindingMatch =
  | { arm: "seq"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }; type?: TypeNode; expr?: ExprNode };

function _ForBinding$seq(c: readonly CstChild[], src: string): ForBindingMatch | null {
  let alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }) | undefined;
  let type: (TypeNode) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_ident: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "Ident"))) { _t2 = false; break _b3; }
        _t4_ident = c[i] as CstLeaf;
        i++;
        {
          const _t5 = i; let _t6 = true;
          _b7: {
            if (!__lit(c, i, src, "!", "$punct")) { _t6 = false; break _b7; }
            i++;
          }
          if (!_t6) i = _t5;
        }
      }
      if (_t2) { _t0 = true; alt = ({ branch: "ident", ident: _t4_ident! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t11_bindingPattern: (BindingPatternNode) | undefined;
      const _t8 = i; let _t9 = true;
      _b10: {
        if (!__nodeOf(c, i, "BindingPattern")) { _t9 = false; break _b10; }
        _t11_bindingPattern = c[i] as BindingPatternNode;
        i++;
      }
      if (_t9) { _t0 = true; alt = ({ branch: "bindingPattern", bindingPattern: _t11_bindingPattern! }) as typeof alt; }
      else i = _t8;
    }
    if (!_t0) return null;
  }
  {
    const _t12 = i; let _t13 = true;
    _b14: {
      if (!__lit(c, i, src, ":", "$punct")) { _t13 = false; break _b14; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t13 = false; break _b14; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t13) i = _t12;
  }
  {
    const _t15 = i; let _t16 = true;
    _b17: {
      if (!__lit(c, i, src, "=", "$punct")) { _t16 = false; break _b17; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t16 = false; break _b17; }
      expr = c[i] as ExprNode;
      i++;
    }
    if (!_t16) i = _t15;
  }
  if (i !== c.length) return null;
  return { arm: "seq", alt: alt!, type, expr };
}

export function matchForBinding(n: ForBindingNode, src: string): ForBindingMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "BindingPattern": {
        { const m = _ForBinding$seq(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _ForBinding$seq(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchForBinding: no arm matches" + ' @' + n.offset);
}

export type ParamMatch =
  | { arm: "this_"; type: TypeNode }
  | { arm: "decoratorExpr"; decoratorExpr?: DecoratorExprNode; publicKw: ("public" | "private" | "protected" | "readonly" | "override" | "static" | "abstract" | "accessor" | "async" | "export" | "declare" | "in" | "out")[]; alt: { branch: "ident"; ident: CstLeaf; type?: TypeNode; expr?: ExprNode } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode; type?: TypeNode; expr?: ExprNode } | { branch: "spread"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }; type?: TypeNode; expr?: ExprNode } }
  | { arm: "decoratorExpr2"; decoratorExpr?: DecoratorExprNode; alt: { branch: "ident"; ident: CstLeaf; type?: TypeNode; expr?: ExprNode } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode; type?: TypeNode; expr?: ExprNode } | { branch: "spread"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }; type?: TypeNode; expr?: ExprNode } };

function _Param$this_(c: readonly CstChild[], src: string): ParamMatch | null {
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "this", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "this_", type: type! };
}

function _Param$decoratorExpr(c: readonly CstChild[], src: string): ParamMatch | null {
  let decoratorExpr: (DecoratorExprNode) | undefined;
  const publicKw: ("public" | "private" | "protected" | "readonly" | "override" | "static" | "abstract" | "accessor" | "async" | "export" | "declare" | "in" | "out")[] = [];
  let alt: ({ branch: "ident"; ident: CstLeaf; type?: TypeNode; expr?: ExprNode } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode; type?: TypeNode; expr?: ExprNode } | { branch: "spread"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }; type?: TypeNode; expr?: ExprNode }) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "DecoratorExpr")) { _t1 = false; break _b2; }
      decoratorExpr = c[i] as DecoratorExprNode;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!(__lit(c, i, src, "public", "$keyword") || __lit(c, i, src, "private", "$keyword") || __lit(c, i, src, "protected", "$keyword") || __lit(c, i, src, "readonly", "$keyword") || __lit(c, i, src, "override", "$keyword") || __lit(c, i, src, "static", "$keyword") || __lit(c, i, src, "abstract", "$keyword") || __lit(c, i, src, "accessor", "$keyword") || __lit(c, i, src, "async", "$keyword") || __lit(c, i, src, "export", "$keyword") || __lit(c, i, src, "declare", "$keyword") || __lit(c, i, src, "in", "$keyword") || __lit(c, i, src, "out", "$keyword"))) { _t4 = false; break _b5; }
      publicKw.push(src.slice(c[i].offset, c[i].end) as "public" | "private" | "protected" | "readonly" | "override" | "static" | "abstract" | "accessor" | "async" | "export" | "declare" | "in" | "out");
      i++;
    }
    if (!_t4) return null;
  }
  for (;;) {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!(__lit(c, i, src, "public", "$keyword") || __lit(c, i, src, "private", "$keyword") || __lit(c, i, src, "protected", "$keyword") || __lit(c, i, src, "readonly", "$keyword") || __lit(c, i, src, "override", "$keyword") || __lit(c, i, src, "static", "$keyword") || __lit(c, i, src, "abstract", "$keyword") || __lit(c, i, src, "accessor", "$keyword") || __lit(c, i, src, "async", "$keyword") || __lit(c, i, src, "export", "$keyword") || __lit(c, i, src, "declare", "$keyword") || __lit(c, i, src, "in", "$keyword") || __lit(c, i, src, "out", "$keyword"))) { _t7 = false; break _b8; }
      publicKw.push(src.slice(c[i].offset, c[i].end) as "public" | "private" | "protected" | "readonly" | "override" | "static" | "abstract" | "accessor" | "async" | "export" | "declare" | "in" | "out");
      i++;
    }
    if (!_t7) { i = _t6; break; }
    if (i === _t6) break;
  }
  {
    let _t9 = false;
    if (!_t9) {
      let _t13_ident: (CstLeaf) | undefined;
      let _t13_type: (TypeNode) | undefined;
      let _t13_expr: (ExprNode) | undefined;
      const _t10 = i; let _t11 = true;
      _b12: {
        if (!(__tok(c, i, "Ident"))) { _t11 = false; break _b12; }
        _t13_ident = c[i] as CstLeaf;
        i++;
        {
          const _t14 = i; let _t15 = true;
          _b16: {
            if (!__lit(c, i, src, "?", "$punct")) { _t15 = false; break _b16; }
            i++;
          }
          if (!_t15) i = _t14;
        }
        {
          const _t17 = i; let _t18 = true;
          _b19: {
            if (!__lit(c, i, src, ":", "$punct")) { _t18 = false; break _b19; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t18 = false; break _b19; }
            _t13_type = c[i] as TypeNode;
            i++;
          }
          if (!_t18) i = _t17;
        }
        {
          const _t20 = i; let _t21 = true;
          _b22: {
            if (!__lit(c, i, src, "=", "$punct")) { _t21 = false; break _b22; }
            i++;
            if (!__nodeOf(c, i, "Expr")) { _t21 = false; break _b22; }
            _t13_expr = c[i] as ExprNode;
            i++;
          }
          if (!_t21) i = _t20;
        }
      }
      if (_t11) { _t9 = true; alt = ({ branch: "ident", ident: _t13_ident!, type: _t13_type, expr: _t13_expr }) as typeof alt; }
      else i = _t10;
    }
    if (!_t9) {
      let _t26_bindingPattern: (BindingPatternNode) | undefined;
      let _t26_type: (TypeNode) | undefined;
      let _t26_expr: (ExprNode) | undefined;
      const _t23 = i; let _t24 = true;
      _b25: {
        if (!__nodeOf(c, i, "BindingPattern")) { _t24 = false; break _b25; }
        _t26_bindingPattern = c[i] as BindingPatternNode;
        i++;
        {
          const _t27 = i; let _t28 = true;
          _b29: {
            if (!__lit(c, i, src, "?", "$punct")) { _t28 = false; break _b29; }
            i++;
          }
          if (!_t28) i = _t27;
        }
        {
          const _t30 = i; let _t31 = true;
          _b32: {
            if (!__lit(c, i, src, ":", "$punct")) { _t31 = false; break _b32; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t31 = false; break _b32; }
            _t26_type = c[i] as TypeNode;
            i++;
          }
          if (!_t31) i = _t30;
        }
        {
          const _t33 = i; let _t34 = true;
          _b35: {
            if (!__lit(c, i, src, "=", "$punct")) { _t34 = false; break _b35; }
            i++;
            if (!__nodeOf(c, i, "Expr")) { _t34 = false; break _b35; }
            _t26_expr = c[i] as ExprNode;
            i++;
          }
          if (!_t34) i = _t33;
        }
      }
      if (_t24) { _t9 = true; alt = ({ branch: "bindingPattern", bindingPattern: _t26_bindingPattern!, type: _t26_type, expr: _t26_expr }) as typeof alt; }
      else i = _t23;
    }
    if (!_t9) {
      let _t39_alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }) | undefined;
      let _t39_type: (TypeNode) | undefined;
      let _t39_expr: (ExprNode) | undefined;
      const _t36 = i; let _t37 = true;
      _b38: {
        if (!__lit(c, i, src, "...", "$punct")) { _t37 = false; break _b38; }
        i++;
        {
          let _t40 = false;
          if (!_t40) {
            let _t44__t39_ident: (CstLeaf) | undefined;
            const _t41 = i; let _t42 = true;
            _b43: {
              if (!(__tok(c, i, "Ident"))) { _t42 = false; break _b43; }
              _t44__t39_ident = c[i] as CstLeaf;
              i++;
            }
            if (_t42) { _t40 = true; _t39_alt = ({ branch: "ident", ident: _t44__t39_ident! }) as typeof _t39_alt; }
            else i = _t41;
          }
          if (!_t40) {
            let _t48__t39_bindingPattern: (BindingPatternNode) | undefined;
            const _t45 = i; let _t46 = true;
            _b47: {
              if (!__nodeOf(c, i, "BindingPattern")) { _t46 = false; break _b47; }
              _t48__t39_bindingPattern = c[i] as BindingPatternNode;
              i++;
            }
            if (_t46) { _t40 = true; _t39_alt = ({ branch: "bindingPattern", bindingPattern: _t48__t39_bindingPattern! }) as typeof _t39_alt; }
            else i = _t45;
          }
          if (!_t40) { _t37 = false; break _b38; }
        }
        {
          const _t49 = i; let _t50 = true;
          _b51: {
            if (!__lit(c, i, src, "?", "$punct")) { _t50 = false; break _b51; }
            i++;
          }
          if (!_t50) i = _t49;
        }
        {
          const _t52 = i; let _t53 = true;
          _b54: {
            if (!__lit(c, i, src, ":", "$punct")) { _t53 = false; break _b54; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t53 = false; break _b54; }
            _t39_type = c[i] as TypeNode;
            i++;
          }
          if (!_t53) i = _t52;
        }
        {
          const _t55 = i; let _t56 = true;
          _b57: {
            if (!__lit(c, i, src, "=", "$punct")) { _t56 = false; break _b57; }
            i++;
            if (!__nodeOf(c, i, "Expr")) { _t56 = false; break _b57; }
            _t39_expr = c[i] as ExprNode;
            i++;
          }
          if (!_t56) i = _t55;
        }
      }
      if (_t37) { _t9 = true; alt = ({ branch: "spread", alt: _t39_alt!, type: _t39_type, expr: _t39_expr }) as typeof alt; }
      else i = _t36;
    }
    if (!_t9) return null;
  }
  if (i !== c.length) return null;
  return { arm: "decoratorExpr", decoratorExpr, publicKw, alt: alt! };
}

function _Param$decoratorExpr2(c: readonly CstChild[], src: string): ParamMatch | null {
  let decoratorExpr: (DecoratorExprNode) | undefined;
  let alt: ({ branch: "ident"; ident: CstLeaf; type?: TypeNode; expr?: ExprNode } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode; type?: TypeNode; expr?: ExprNode } | { branch: "spread"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }; type?: TypeNode; expr?: ExprNode }) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "DecoratorExpr")) { _t1 = false; break _b2; }
      decoratorExpr = c[i] as DecoratorExprNode;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    let _t3 = false;
    if (!_t3) {
      let _t7_ident: (CstLeaf) | undefined;
      let _t7_type: (TypeNode) | undefined;
      let _t7_expr: (ExprNode) | undefined;
      const _t4 = i; let _t5 = true;
      _b6: {
        if (!(__tok(c, i, "Ident"))) { _t5 = false; break _b6; }
        _t7_ident = c[i] as CstLeaf;
        i++;
        {
          const _t8 = i; let _t9 = true;
          _b10: {
            if (!__lit(c, i, src, "?", "$punct")) { _t9 = false; break _b10; }
            i++;
          }
          if (!_t9) i = _t8;
        }
        {
          const _t11 = i; let _t12 = true;
          _b13: {
            if (!__lit(c, i, src, ":", "$punct")) { _t12 = false; break _b13; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t12 = false; break _b13; }
            _t7_type = c[i] as TypeNode;
            i++;
          }
          if (!_t12) i = _t11;
        }
        {
          const _t14 = i; let _t15 = true;
          _b16: {
            if (!__lit(c, i, src, "=", "$punct")) { _t15 = false; break _b16; }
            i++;
            if (!__nodeOf(c, i, "Expr")) { _t15 = false; break _b16; }
            _t7_expr = c[i] as ExprNode;
            i++;
          }
          if (!_t15) i = _t14;
        }
      }
      if (_t5) { _t3 = true; alt = ({ branch: "ident", ident: _t7_ident!, type: _t7_type, expr: _t7_expr }) as typeof alt; }
      else i = _t4;
    }
    if (!_t3) {
      let _t20_bindingPattern: (BindingPatternNode) | undefined;
      let _t20_type: (TypeNode) | undefined;
      let _t20_expr: (ExprNode) | undefined;
      const _t17 = i; let _t18 = true;
      _b19: {
        if (!__nodeOf(c, i, "BindingPattern")) { _t18 = false; break _b19; }
        _t20_bindingPattern = c[i] as BindingPatternNode;
        i++;
        {
          const _t21 = i; let _t22 = true;
          _b23: {
            if (!__lit(c, i, src, "?", "$punct")) { _t22 = false; break _b23; }
            i++;
          }
          if (!_t22) i = _t21;
        }
        {
          const _t24 = i; let _t25 = true;
          _b26: {
            if (!__lit(c, i, src, ":", "$punct")) { _t25 = false; break _b26; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t25 = false; break _b26; }
            _t20_type = c[i] as TypeNode;
            i++;
          }
          if (!_t25) i = _t24;
        }
        {
          const _t27 = i; let _t28 = true;
          _b29: {
            if (!__lit(c, i, src, "=", "$punct")) { _t28 = false; break _b29; }
            i++;
            if (!__nodeOf(c, i, "Expr")) { _t28 = false; break _b29; }
            _t20_expr = c[i] as ExprNode;
            i++;
          }
          if (!_t28) i = _t27;
        }
      }
      if (_t18) { _t3 = true; alt = ({ branch: "bindingPattern", bindingPattern: _t20_bindingPattern!, type: _t20_type, expr: _t20_expr }) as typeof alt; }
      else i = _t17;
    }
    if (!_t3) {
      let _t33_alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "bindingPattern"; bindingPattern: BindingPatternNode }) | undefined;
      let _t33_type: (TypeNode) | undefined;
      let _t33_expr: (ExprNode) | undefined;
      const _t30 = i; let _t31 = true;
      _b32: {
        if (!__lit(c, i, src, "...", "$punct")) { _t31 = false; break _b32; }
        i++;
        {
          let _t34 = false;
          if (!_t34) {
            let _t38__t33_ident: (CstLeaf) | undefined;
            const _t35 = i; let _t36 = true;
            _b37: {
              if (!(__tok(c, i, "Ident"))) { _t36 = false; break _b37; }
              _t38__t33_ident = c[i] as CstLeaf;
              i++;
            }
            if (_t36) { _t34 = true; _t33_alt = ({ branch: "ident", ident: _t38__t33_ident! }) as typeof _t33_alt; }
            else i = _t35;
          }
          if (!_t34) {
            let _t42__t33_bindingPattern: (BindingPatternNode) | undefined;
            const _t39 = i; let _t40 = true;
            _b41: {
              if (!__nodeOf(c, i, "BindingPattern")) { _t40 = false; break _b41; }
              _t42__t33_bindingPattern = c[i] as BindingPatternNode;
              i++;
            }
            if (_t40) { _t34 = true; _t33_alt = ({ branch: "bindingPattern", bindingPattern: _t42__t33_bindingPattern! }) as typeof _t33_alt; }
            else i = _t39;
          }
          if (!_t34) { _t31 = false; break _b32; }
        }
        {
          const _t43 = i; let _t44 = true;
          _b45: {
            if (!__lit(c, i, src, "?", "$punct")) { _t44 = false; break _b45; }
            i++;
          }
          if (!_t44) i = _t43;
        }
        {
          const _t46 = i; let _t47 = true;
          _b48: {
            if (!__lit(c, i, src, ":", "$punct")) { _t47 = false; break _b48; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t47 = false; break _b48; }
            _t33_type = c[i] as TypeNode;
            i++;
          }
          if (!_t47) i = _t46;
        }
        {
          const _t49 = i; let _t50 = true;
          _b51: {
            if (!__lit(c, i, src, "=", "$punct")) { _t50 = false; break _b51; }
            i++;
            if (!__nodeOf(c, i, "Expr")) { _t50 = false; break _b51; }
            _t33_expr = c[i] as ExprNode;
            i++;
          }
          if (!_t50) i = _t49;
        }
      }
      if (_t31) { _t3 = true; alt = ({ branch: "spread", alt: _t33_alt!, type: _t33_type, expr: _t33_expr }) as typeof alt; }
      else i = _t30;
    }
    if (!_t3) return null;
  }
  if (i !== c.length) return null;
  return { arm: "decoratorExpr2", decoratorExpr, alt: alt! };
}

export function matchParam(n: ParamNode, src: string): ParamMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "BindingPattern": {
        { const m = _Param$decoratorExpr2(c, src); if (m !== null) return m; }
        break;
      }
      case "DecoratorExpr": {
        { const m = _Param$decoratorExpr(c, src); if (m !== null) return m; }
        { const m = _Param$decoratorExpr2(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 46: {
        { const m = _Param$decoratorExpr2(c, src); if (m !== null) return m; }
        break;
      }
      case 97: {
        { const m = _Param$decoratorExpr(c, src); if (m !== null) return m; }
        break;
      }
      case 100: {
        { const m = _Param$decoratorExpr(c, src); if (m !== null) return m; }
        break;
      }
      case 101: {
        { const m = _Param$decoratorExpr(c, src); if (m !== null) return m; }
        break;
      }
      case 105: {
        { const m = _Param$decoratorExpr(c, src); if (m !== null) return m; }
        break;
      }
      case 111: {
        { const m = _Param$decoratorExpr(c, src); if (m !== null) return m; }
        break;
      }
      case 112: {
        { const m = _Param$decoratorExpr(c, src); if (m !== null) return m; }
        break;
      }
      case 114: {
        { const m = _Param$decoratorExpr(c, src); if (m !== null) return m; }
        break;
      }
      case 115: {
        { const m = _Param$decoratorExpr(c, src); if (m !== null) return m; }
        break;
      }
      case 116: {
        { const m = _Param$this_(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _Param$decoratorExpr2(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchParam: no arm matches" + ' @' + n.offset);
}

export type ForHeadMatch =
  | { arm: "let_"; alt: { branch: "let_" } | { branch: "const_" } | { branch: "var_" } | { branch: "using" } | { branch: "await_" }; forBinding: (ForBindingNode)[]; alt2: { branch: "semi"; expr?: ExprNode; expr2: (ExprNode)[]; expr3?: ExprNode; expr4: (ExprNode)[] } | { branch: "in_"; in_Kw: "in" | "of"; expr: ExprNode } }
  | { arm: "seq"; expr?: ExprNode; expr2: (ExprNode)[]; expr3?: ExprNode; expr4: (ExprNode)[]; expr5?: ExprNode; expr6: (ExprNode)[] }
  | { arm: "expr"; expr: ExprNode; in_Kw: "in" | "of"; expr2: ExprNode };

function _ForHead$let_(c: readonly CstChild[], src: string): ForHeadMatch | null {
  let alt: ({ branch: "let_" } | { branch: "const_" } | { branch: "var_" } | { branch: "using" } | { branch: "await_" }) | undefined;
  const forBinding: (ForBindingNode)[] = [];
  let alt2: ({ branch: "semi"; expr?: ExprNode; expr2: (ExprNode)[]; expr3?: ExprNode; expr4: (ExprNode)[] } | { branch: "in_"; in_Kw: "in" | "of"; expr: ExprNode }) | undefined;
  let i = 0;
  {
    let _t0 = false;
    if (!_t0) {
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!__lit(c, i, src, "let", "$keyword")) { _t2 = false; break _b3; }
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "let_" }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!__lit(c, i, src, "const", "$keyword")) { _t6 = false; break _b7; }
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "const_" }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) {
      const _t9 = i; let _t10 = true;
      _b11: {
        if (!__lit(c, i, src, "var", "$keyword")) { _t10 = false; break _b11; }
        i++;
      }
      if (_t10) { _t0 = true; alt = ({ branch: "var_" }) as typeof alt; }
      else i = _t9;
    }
    if (!_t0) {
      const _t13 = i; let _t14 = true;
      _b15: {
        if (!__lit(c, i, src, "using", "$keyword")) { _t14 = false; break _b15; }
        i++;
      }
      if (_t14) { _t0 = true; alt = ({ branch: "using" }) as typeof alt; }
      else i = _t13;
    }
    if (!_t0) {
      const _t17 = i; let _t18 = true;
      _b19: {
        if (!__lit(c, i, src, "await", "$keyword")) { _t18 = false; break _b19; }
        i++;
        if (!__lit(c, i, src, "using", "$keyword")) { _t18 = false; break _b19; }
        i++;
      }
      if (_t18) { _t0 = true; alt = ({ branch: "await_" }) as typeof alt; }
      else i = _t17;
    }
    if (!_t0) return null;
  }
  {
    const _t21 = i; let _t24 = true;
    _b25: {
      if (!__nodeOf(c, i, "ForBinding")) { _t24 = false; break _b25; }
      forBinding.push(c[i] as ForBindingNode);
      i++;
    }
    if (!_t24) { i = _t21; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t212 = i; let _t22 = true;
      _b23: {
        if (!__nodeOf(c, i, "ForBinding")) { _t22 = false; break _b23; }
        forBinding.push(c[i] as ForBindingNode);
        i++;
      }
      if (!_t22) { i = _t212; break; }
    }
  }
  {
    let _t26 = false;
    if (!_t26) {
      let _t30_expr: (ExprNode) | undefined;
      const _t30_expr2: (ExprNode)[] = [];
      let _t30_expr3: (ExprNode) | undefined;
      const _t30_expr4: (ExprNode)[] = [];
      const _t27 = i; let _t28 = true;
      _b29: {
        if (!__lit(c, i, src, ";", "$punct")) { _t28 = false; break _b29; }
        i++;
        {
          const _t31 = i; let _t32 = true;
          _b33: {
            if (!__nodeOf(c, i, "Expr")) { _t32 = false; break _b33; }
            _t30_expr = c[i] as ExprNode;
            i++;
            for (;;) {
              const _t34 = i; let _t35 = true;
              _b36: {
                if (!__lit(c, i, src, ",", "$punct")) { _t35 = false; break _b36; }
                i++;
                if (!__nodeOf(c, i, "Expr")) { _t35 = false; break _b36; }
                _t30_expr2.push(c[i] as ExprNode);
                i++;
              }
              if (!_t35) { i = _t34; break; }
              if (i === _t34) break;
            }
          }
          if (!_t32) i = _t31;
        }
        if (!__lit(c, i, src, ";", "$punct")) { _t28 = false; break _b29; }
        i++;
        {
          const _t37 = i; let _t38 = true;
          _b39: {
            if (!__nodeOf(c, i, "Expr")) { _t38 = false; break _b39; }
            _t30_expr3 = c[i] as ExprNode;
            i++;
            for (;;) {
              const _t40 = i; let _t41 = true;
              _b42: {
                if (!__lit(c, i, src, ",", "$punct")) { _t41 = false; break _b42; }
                i++;
                if (!__nodeOf(c, i, "Expr")) { _t41 = false; break _b42; }
                _t30_expr4.push(c[i] as ExprNode);
                i++;
              }
              if (!_t41) { i = _t40; break; }
              if (i === _t40) break;
            }
          }
          if (!_t38) i = _t37;
        }
      }
      if (_t28) { _t26 = true; alt2 = ({ branch: "semi", expr: _t30_expr, expr2: _t30_expr2, expr3: _t30_expr3, expr4: _t30_expr4 }) as typeof alt2; }
      else i = _t27;
    }
    if (!_t26) {
      let _t46_in_Kw: ("in" | "of") | undefined;
      let _t46_expr: (ExprNode) | undefined;
      const _t43 = i; let _t44 = true;
      _b45: {
        if (!(__lit(c, i, src, "in", "$keyword") || __lit(c, i, src, "of", "$keyword"))) { _t44 = false; break _b45; }
        _t46_in_Kw = src.slice(c[i].offset, c[i].end) as "in" | "of";
        i++;
        if (!__nodeOf(c, i, "Expr")) { _t44 = false; break _b45; }
        _t46_expr = c[i] as ExprNode;
        i++;
      }
      if (_t44) { _t26 = true; alt2 = ({ branch: "in_", in_Kw: _t46_in_Kw!, expr: _t46_expr! }) as typeof alt2; }
      else i = _t43;
    }
    if (!_t26) return null;
  }
  if (i !== c.length) return null;
  return { arm: "let_", alt: alt!, forBinding, alt2: alt2! };
}

function _ForHead$seq(c: readonly CstChild[], src: string): ForHeadMatch | null {
  let expr: (ExprNode) | undefined;
  const expr2: (ExprNode)[] = [];
  let expr3: (ExprNode) | undefined;
  const expr4: (ExprNode)[] = [];
  let expr5: (ExprNode) | undefined;
  const expr6: (ExprNode)[] = [];
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
      expr = c[i] as ExprNode;
      i++;
      for (;;) {
        const _t3 = i; let _t4 = true;
        _b5: {
          if (!__lit(c, i, src, ",", "$punct")) { _t4 = false; break _b5; }
          i++;
          if (!__nodeOf(c, i, "Expr")) { _t4 = false; break _b5; }
          expr2.push(c[i] as ExprNode);
          i++;
        }
        if (!_t4) { i = _t3; break; }
        if (i === _t3) break;
      }
    }
    if (!_t1) i = _t0;
  }
  if (!__lit(c, i, src, ";", "$punct")) return null;
  i++;
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__nodeOf(c, i, "Expr")) { _t7 = false; break _b8; }
      expr3 = c[i] as ExprNode;
      i++;
      for (;;) {
        const _t9 = i; let _t10 = true;
        _b11: {
          if (!__lit(c, i, src, ",", "$punct")) { _t10 = false; break _b11; }
          i++;
          if (!__nodeOf(c, i, "Expr")) { _t10 = false; break _b11; }
          expr4.push(c[i] as ExprNode);
          i++;
        }
        if (!_t10) { i = _t9; break; }
        if (i === _t9) break;
      }
    }
    if (!_t7) i = _t6;
  }
  if (!__lit(c, i, src, ";", "$punct")) return null;
  i++;
  {
    const _t12 = i; let _t13 = true;
    _b14: {
      if (!__nodeOf(c, i, "Expr")) { _t13 = false; break _b14; }
      expr5 = c[i] as ExprNode;
      i++;
      for (;;) {
        const _t15 = i; let _t16 = true;
        _b17: {
          if (!__lit(c, i, src, ",", "$punct")) { _t16 = false; break _b17; }
          i++;
          if (!__nodeOf(c, i, "Expr")) { _t16 = false; break _b17; }
          expr6.push(c[i] as ExprNode);
          i++;
        }
        if (!_t16) { i = _t15; break; }
        if (i === _t15) break;
      }
    }
    if (!_t13) i = _t12;
  }
  if (i !== c.length) return null;
  return { arm: "seq", expr, expr2, expr3, expr4, expr5, expr6 };
}

function _ForHead$expr(c: readonly CstChild[], src: string): ForHeadMatch | null {
  let expr: (ExprNode) | undefined;
  let in_Kw: ("in" | "of") | undefined;
  let expr2: (ExprNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (!(__lit(c, i, src, "in", "$keyword") || __lit(c, i, src, "of", "$keyword"))) return null;
  in_Kw = src.slice(c[i].offset, c[i].end) as "in" | "of";
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr2 = c[i] as ExprNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "expr", expr: expr!, in_Kw: in_Kw!, expr2: expr2! };
}

export function matchForHead(n: ForHeadNode, src: string): ForHeadMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "Expr": {
        { const m = _ForHead$seq(c, src); if (m !== null) return m; }
        { const m = _ForHead$expr(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 59: {
        { const m = _ForHead$seq(c, src); if (m !== null) return m; }
        break;
      }
      case 97: {
        { const m = _ForHead$let_(c, src); if (m !== null) return m; }
        break;
      }
      case 99: {
        { const m = _ForHead$let_(c, src); if (m !== null) return m; }
        break;
      }
      case 108: {
        { const m = _ForHead$let_(c, src); if (m !== null) return m; }
        break;
      }
      case 117: {
        { const m = _ForHead$let_(c, src); if (m !== null) return m; }
        break;
      }
      case 118: {
        { const m = _ForHead$let_(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
    }
  }
  throw new Error("matchForHead: no arm matches" + ' @' + n.offset);
}

export type SwitchCaseMatch =
  | { arm: "case_"; expr: ExprNode; expr2: (ExprNode)[] }
  | { arm: "default_" }
  | { arm: "stmt"; stmt: StmtNode };

function _SwitchCase$case_(c: readonly CstChild[], src: string): SwitchCaseMatch | null {
  let expr: (ExprNode) | undefined;
  const expr2: (ExprNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "case", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, ",", "$punct")) { _t1 = false; break _b2; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
      expr2.push(c[i] as ExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "case_", expr: expr!, expr2 };
}

function _SwitchCase$default_(c: readonly CstChild[], src: string): SwitchCaseMatch | null {
  let i = 0;
  if (!__lit(c, i, src, "default", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "default_" };
}

function _SwitchCase$stmt(c: readonly CstChild[], src: string): SwitchCaseMatch | null {
  let stmt: (StmtNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "Stmt")) return null;
  stmt = c[i] as StmtNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "stmt", stmt: stmt! };
}

export function matchSwitchCase(n: SwitchCaseNode, src: string): SwitchCaseMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "Stmt": {
        { const m = _SwitchCase$stmt(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 99: {
        { const m = _SwitchCase$case_(c, src); if (m !== null) return m; }
        break;
      }
      case 100: {
        { const m = _SwitchCase$default_(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
    }
  }
  throw new Error("matchSwitchCase: no arm matches" + ' @' + n.offset);
}

export type TypeParamsMatch =
  | { arm: "lt"; typeParam: (TypeParamNode)[] };

function _TypeParams$lt(c: readonly CstChild[], src: string): TypeParamsMatch | null {
  const typeParam: (TypeParamNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "<", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "TypeParam")) { _t3 = false; break _b4; }
      typeParam.push(c[i] as TypeParamNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "TypeParam")) { _t1 = false; break _b2; }
        typeParam.push(c[i] as TypeParamNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, ">", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "lt", typeParam };
}

export function matchTypeParams(n: TypeParamsNode, src: string): TypeParamsMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 60: {
        { const m = _TypeParams$lt(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
    }
  }
  throw new Error("matchTypeParams: no arm matches" + ' @' + n.offset);
}

export type TypeParamMatch =
  | { arm: "const_"; const_Kw: ("const" | "in" | "out" | "public" | "private" | "protected" | "readonly")[]; ident: CstLeaf; type?: TypeNode; extendsTok?: CstLeaf; type2?: TypeNode }
  | { arm: "const_2"; const_Kw: "const" | "in" | "out" | "public" | "private" | "protected" | "readonly"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "in_" } | { branch: "out" }; type?: TypeNode; extendsTok?: CstLeaf; type2?: TypeNode }
  | { arm: "ident"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "in_" } | { branch: "out" }; type?: TypeNode; extendsTok?: CstLeaf; type2?: TypeNode };

function _TypeParam$const_(c: readonly CstChild[], src: string): TypeParamMatch | null {
  const const_Kw: ("const" | "in" | "out" | "public" | "private" | "protected" | "readonly")[] = [];
  let ident: (CstLeaf) | undefined;
  let type: (TypeNode) | undefined;
  let extendsTok: (CstLeaf) | undefined;
  let type2: (TypeNode) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!(__lit(c, i, src, "const", "$keyword") || __lit(c, i, src, "in", "$keyword") || __lit(c, i, src, "out", "$keyword") || __lit(c, i, src, "public", "$keyword") || __lit(c, i, src, "private", "$keyword") || __lit(c, i, src, "protected", "$keyword") || __lit(c, i, src, "readonly", "$keyword"))) { _t1 = false; break _b2; }
      const_Kw.push(src.slice(c[i].offset, c[i].end) as "const" | "in" | "out" | "public" | "private" | "protected" | "readonly");
      i++;
    }
    if (!_t1) return null;
  }
  for (;;) {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!(__lit(c, i, src, "const", "$keyword") || __lit(c, i, src, "in", "$keyword") || __lit(c, i, src, "out", "$keyword") || __lit(c, i, src, "public", "$keyword") || __lit(c, i, src, "private", "$keyword") || __lit(c, i, src, "protected", "$keyword") || __lit(c, i, src, "readonly", "$keyword"))) { _t4 = false; break _b5; }
      const_Kw.push(src.slice(c[i].offset, c[i].end) as "const" | "in" | "out" | "public" | "private" | "protected" | "readonly");
      i++;
    }
    if (!_t4) { i = _t3; break; }
    if (i === _t3) break;
  }
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__lit(c, i, src, "extends", "$keyword")) { _t7 = false; break _b8; }
      extendsTok = c[i] as CstLeaf;
      i++;
      if (!__nodeOf(c, i, "Type")) { _t7 = false; break _b8; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t7) i = _t6;
  }
  {
    const _t9 = i; let _t10 = true;
    _b11: {
      if (!__lit(c, i, src, "=", "$punct")) { _t10 = false; break _b11; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t10 = false; break _b11; }
      type2 = c[i] as TypeNode;
      i++;
    }
    if (!_t10) i = _t9;
  }
  if (i !== c.length) return null;
  return { arm: "const_", const_Kw, ident: ident!, type, extendsTok, type2 };
}

function _TypeParam$const_2(c: readonly CstChild[], src: string): TypeParamMatch | null {
  let const_Kw: ("const" | "in" | "out" | "public" | "private" | "protected" | "readonly") | undefined;
  let alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "in_" } | { branch: "out" }) | undefined;
  let type: (TypeNode) | undefined;
  let extendsTok: (CstLeaf) | undefined;
  let type2: (TypeNode) | undefined;
  let i = 0;
  if (!(__lit(c, i, src, "const", "$keyword") || __lit(c, i, src, "in", "$keyword") || __lit(c, i, src, "out", "$keyword") || __lit(c, i, src, "public", "$keyword") || __lit(c, i, src, "private", "$keyword") || __lit(c, i, src, "protected", "$keyword") || __lit(c, i, src, "readonly", "$keyword"))) return null;
  const_Kw = src.slice(c[i].offset, c[i].end) as "const" | "in" | "out" | "public" | "private" | "protected" | "readonly";
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_ident: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "Ident"))) { _t2 = false; break _b3; }
        _t4_ident = c[i] as CstLeaf;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "ident", ident: _t4_ident! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!__lit(c, i, src, "in", "$keyword")) { _t6 = false; break _b7; }
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "in_" }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) {
      const _t9 = i; let _t10 = true;
      _b11: {
        if (!__lit(c, i, src, "out", "$keyword")) { _t10 = false; break _b11; }
        i++;
      }
      if (_t10) { _t0 = true; alt = ({ branch: "out" }) as typeof alt; }
      else i = _t9;
    }
    if (!_t0) return null;
  }
  {
    const _t13 = i; let _t14 = true;
    _b15: {
      if (!__lit(c, i, src, "extends", "$keyword")) { _t14 = false; break _b15; }
      extendsTok = c[i] as CstLeaf;
      i++;
      if (!__nodeOf(c, i, "Type")) { _t14 = false; break _b15; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t14) i = _t13;
  }
  {
    const _t16 = i; let _t17 = true;
    _b18: {
      if (!__lit(c, i, src, "=", "$punct")) { _t17 = false; break _b18; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t17 = false; break _b18; }
      type2 = c[i] as TypeNode;
      i++;
    }
    if (!_t17) i = _t16;
  }
  if (i !== c.length) return null;
  return { arm: "const_2", const_Kw: const_Kw!, alt: alt!, type, extendsTok, type2 };
}

function _TypeParam$ident(c: readonly CstChild[], src: string): TypeParamMatch | null {
  let alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "in_" } | { branch: "out" }) | undefined;
  let type: (TypeNode) | undefined;
  let extendsTok: (CstLeaf) | undefined;
  let type2: (TypeNode) | undefined;
  let i = 0;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_ident: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "Ident"))) { _t2 = false; break _b3; }
        _t4_ident = c[i] as CstLeaf;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "ident", ident: _t4_ident! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!__lit(c, i, src, "in", "$keyword")) { _t6 = false; break _b7; }
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "in_" }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) {
      const _t9 = i; let _t10 = true;
      _b11: {
        if (!__lit(c, i, src, "out", "$keyword")) { _t10 = false; break _b11; }
        i++;
      }
      if (_t10) { _t0 = true; alt = ({ branch: "out" }) as typeof alt; }
      else i = _t9;
    }
    if (!_t0) return null;
  }
  {
    const _t13 = i; let _t14 = true;
    _b15: {
      if (!__lit(c, i, src, "extends", "$keyword")) { _t14 = false; break _b15; }
      extendsTok = c[i] as CstLeaf;
      i++;
      if (!__nodeOf(c, i, "Type")) { _t14 = false; break _b15; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t14) i = _t13;
  }
  {
    const _t16 = i; let _t17 = true;
    _b18: {
      if (!__lit(c, i, src, "=", "$punct")) { _t17 = false; break _b18; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t17 = false; break _b18; }
      type2 = c[i] as TypeNode;
      i++;
    }
    if (!_t17) i = _t16;
  }
  if (i !== c.length) return null;
  return { arm: "ident", alt: alt!, type, extendsTok, type2 };
}

export function matchTypeParam(n: TypeParamNode, src: string): TypeParamMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 99: {
        { const m = _TypeParam$const_(c, src); if (m !== null) return m; }
        { const m = _TypeParam$const_2(c, src); if (m !== null) return m; }
        break;
      }
      case 105: {
        { const m = _TypeParam$const_(c, src); if (m !== null) return m; }
        { const m = _TypeParam$const_2(c, src); if (m !== null) return m; }
        { const m = _TypeParam$ident(c, src); if (m !== null) return m; }
        break;
      }
      case 111: {
        { const m = _TypeParam$const_(c, src); if (m !== null) return m; }
        { const m = _TypeParam$const_2(c, src); if (m !== null) return m; }
        { const m = _TypeParam$ident(c, src); if (m !== null) return m; }
        break;
      }
      case 112: {
        { const m = _TypeParam$const_(c, src); if (m !== null) return m; }
        { const m = _TypeParam$const_2(c, src); if (m !== null) return m; }
        break;
      }
      case 114: {
        { const m = _TypeParam$const_(c, src); if (m !== null) return m; }
        { const m = _TypeParam$const_2(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _TypeParam$ident(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchTypeParam: no arm matches" + ' @' + n.offset);
}

export type DeclMatch =
  | { arm: "async"; asyncTok?: CstLeaf; ident: CstLeaf; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode; alt: { branch: "block"; block: BlockNode } | { branch: "semi" } }
  | { arm: "interface"; ident: CstLeaf; typeParams?: TypeParamsNode; type: (TypeNode)[]; extendsTok?: CstLeaf; interfaceMember: (InterfaceMemberNode)[]; _Kw: (";" | ",")[] }
  | { arm: "type"; ident: CstLeaf; typeParams?: TypeParamsNode; type: TypeNode }
  | { arm: "decoratorExpr"; decoratorExpr: (DecoratorExprNode)[]; abstractTok?: CstLeaf; ident: CstLeaf; typeParams?: TypeParamsNode; alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[]; classMember: (ClassMemberNode)[] }
  | { arm: "decoratorExpr2"; decoratorExpr: (DecoratorExprNode)[]; abstractTok?: CstLeaf; typeParams?: TypeParamsNode; alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[]; classMember: (ClassMemberNode)[] }
  | { arm: "enum_"; ident: CstLeaf; enumMember: (EnumMemberNode)[] }
  | { arm: "declare"; ident: CstLeaf; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode }
  | { arm: "declare2"; alt: { branch: "decl"; decl: DeclNode } | { branch: "stmt"; stmt: StmtNode } }
  | { arm: "namespace"; ident: CstLeaf; ident2: (CstLeaf)[]; stmt: (StmtNode)[] }
  | { arm: "module"; alt: { branch: "ident"; ident: CstLeaf; ident2: (CstLeaf)[] } | { branch: "string"; string: CstLeaf }; stmt: (StmtNode)[] }
  | { arm: "export_"; alt: { branch: "decl"; decl: DeclNode } | { branch: "stmt"; stmt: StmtNode } }
  | { arm: "decoratorExpr3"; decoratorExpr: (DecoratorExprNode)[]; alt: { branch: "decl"; decl: DeclNode } | { branch: "let_"; let_Kw: "let" | "const" | "var"; binding: (BindingNode)[] } | { branch: "await_"; awaitTok?: CstLeaf; binding: BindingNode; binding2: (BindingNode)[] } }
  | { arm: "export_2"; decoratorExpr: (DecoratorExprNode)[]; alt: { branch: "async"; asyncTok?: CstLeaf; ident?: CstLeaf; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode; alt: { branch: "block"; block: BlockNode } | { branch: "semi" } } | { branch: "abstract"; ident: CstLeaf; typeParams?: TypeParamsNode; alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[]; classMember: (ClassMemberNode)[] } | { branch: "abstract2"; typeParams?: TypeParamsNode; alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[]; classMember: (ClassMemberNode)[] } | { branch: "expr"; expr: ExprNode } }
  | { arm: "export_3"; alt: { branch: "from"; string: CstLeaf } | { branch: "as"; ident: CstLeaf; string: CstLeaf } }
  | { arm: "export_4"; exportSpecifier: (ExportSpecifierNode)[]; string?: CstLeaf; fromTok?: CstLeaf }
  | { arm: "export_5"; expr: ExprNode }
  | { arm: "export_6"; exportSpecifier: (ExportSpecifierNode)[]; string?: CstLeaf; fromTok?: CstLeaf }
  | { arm: "const_"; ident: CstLeaf; enumMember: (EnumMemberNode)[] }
  | { arm: "import_"; alt: { branch: "importClause"; importClause: ImportClauseNode; string: CstLeaf } | { branch: "type"; importClause: ImportClauseNode; string: CstLeaf } | { branch: "ident"; ident: CstLeaf; expr: ExprNode } | { branch: "string"; string: CstLeaf } }
  | { arm: "decoratorExpr4"; decoratorExpr: (DecoratorExprNode)[]; alt: { branch: "decl"; decl: DeclNode } | { branch: "stmt"; stmt: StmtNode } };

function _Decl$async(c: readonly CstChild[], src: string): DeclMatch | null {
  let asyncTok: (CstLeaf) | undefined;
  let ident: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let alt: ({ branch: "block"; block: BlockNode } | { branch: "semi" }) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "async", "$keyword")) { _t1 = false; break _b2; }
      asyncTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (!__lit(c, i, src, "function", "$keyword")) return null;
  i++;
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "*", "$punct")) { _t4 = false; break _b5; }
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__nodeOf(c, i, "TypeParams")) { _t7 = false; break _b8; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t7) i = _t6;
  }
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t9 = i; let _t12 = true;
    _b13: {
      if (!__nodeOf(c, i, "Param")) { _t12 = false; break _b13; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t12) { i = _t9; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t92 = i; let _t10 = true;
      _b11: {
        if (!__nodeOf(c, i, "Param")) { _t10 = false; break _b11; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t10) { i = _t92; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  {
    const _t14 = i; let _t15 = true;
    _b16: {
      if (!__lit(c, i, src, ":", "$punct")) { _t15 = false; break _b16; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t15 = false; break _b16; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t15) i = _t14;
  }
  {
    let _t17 = false;
    if (!_t17) {
      let _t21_block: (BlockNode) | undefined;
      const _t18 = i; let _t19 = true;
      _b20: {
        if (!__nodeOf(c, i, "Block")) { _t19 = false; break _b20; }
        _t21_block = c[i] as BlockNode;
        i++;
      }
      if (_t19) { _t17 = true; alt = ({ branch: "block", block: _t21_block! }) as typeof alt; }
      else i = _t18;
    }
    if (!_t17) {
      const _t22 = i; let _t23 = true;
      _b24: {
        {
          const _t26 = i; let _t27 = true;
          _b28: {
            if (!__lit(c, i, src, ";", "$punct")) { _t27 = false; break _b28; }
            i++;
          }
          if (!_t27) i = _t26;
        }
      }
      if (_t23) { _t17 = true; alt = ({ branch: "semi" }) as typeof alt; }
      else i = _t22;
    }
    if (!_t17) return null;
  }
  if (i !== c.length) return null;
  return { arm: "async", asyncTok, ident: ident!, typeParams, param, type, alt: alt! };
}

function _Decl$interface(c: readonly CstChild[], src: string): DeclMatch | null {
  let ident: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const type: (TypeNode)[] = [];
  let extendsTok: (CstLeaf) | undefined;
  const interfaceMember: (InterfaceMemberNode)[] = [];
  const _Kw: (";" | ",")[] = [];
  let i = 0;
  if (!__lit(c, i, src, "interface", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "TypeParams")) { _t1 = false; break _b2; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "extends", "$keyword")) { _t4 = false; break _b5; }
      extendsTok = c[i] as CstLeaf;
      i++;
      {
        const _t6 = i; let _t9 = true;
        _b10: {
          if (!__nodeOf(c, i, "Type")) { _t9 = false; break _b10; }
          type.push(c[i] as TypeNode);
          i++;
        }
        if (!_t9) { i = _t6; }
        else for (;;) {
          if (!__lit(c, i, src, ",", "$punct")) break;
          i++;
          const _t62 = i; let _t7 = true;
          _b8: {
            if (!__nodeOf(c, i, "Type")) { _t7 = false; break _b8; }
            type.push(c[i] as TypeNode);
            i++;
          }
          if (!_t7) { i = _t62; break; }
        }
      }
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  for (;;) {
    const _t11 = i; let _t12 = true;
    _b13: {
      if (!__nodeOf(c, i, "InterfaceMember")) { _t12 = false; break _b13; }
      interfaceMember.push(c[i] as InterfaceMemberNode);
      i++;
      {
        const _t14 = i; let _t15 = true;
        _b16: {
          if (!(__lit(c, i, src, ";", "$punct") || __lit(c, i, src, ",", "$punct"))) { _t15 = false; break _b16; }
          _Kw.push(src.slice(c[i].offset, c[i].end) as ";" | ",");
          i++;
        }
        if (!_t15) i = _t14;
      }
    }
    if (!_t12) { i = _t11; break; }
    if (i === _t11) break;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "interface", ident: ident!, typeParams, type, extendsTok, interfaceMember, _Kw };
}

function _Decl$type(c: readonly CstChild[], src: string): DeclMatch | null {
  let ident: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "type", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "TypeParams")) { _t1 = false; break _b2; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (!__lit(c, i, src, "=", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, ";", "$punct")) { _t4 = false; break _b5; }
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (i !== c.length) return null;
  return { arm: "type", ident: ident!, typeParams, type: type! };
}

function _Decl$decoratorExpr(c: readonly CstChild[], src: string): DeclMatch | null {
  const decoratorExpr: (DecoratorExprNode)[] = [];
  let abstractTok: (CstLeaf) | undefined;
  let ident: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[] = [];
  const classMember: (ClassMemberNode)[] = [];
  let i = 0;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "DecoratorExpr")) { _t1 = false; break _b2; }
      decoratorExpr.push(c[i] as DecoratorExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "abstract", "$keyword")) { _t4 = false; break _b5; }
      abstractTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, "class", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__nodeOf(c, i, "TypeParams")) { _t7 = false; break _b8; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t7) i = _t6;
  }
  for (;;) {
    const _t9 = i; let _t10 = true;
    _b11: {
      {
        let _t12 = false;
        if (!_t12) {
          const _t16_alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] = [];
          const _t13 = i; let _t14 = true;
          _b15: {
            if (!__lit(c, i, src, "extends", "$keyword")) { _t14 = false; break _b15; }
            i++;
            {
              const _t17 = i; let _t20 = true;
              _b21: {
                {
                  let _t22 = false;
                  if (!_t22) {
                    let _t26__t16_classHeritage: (ClassHeritageNode) | undefined;
                    const _t23 = i; let _t24 = true;
                    _b25: {
                      if (!__nodeOf(c, i, "ClassHeritage")) { _t24 = false; break _b25; }
                      _t26__t16_classHeritage = c[i] as ClassHeritageNode;
                      i++;
                    }
                    if (_t24) { _t22 = true; _t16_alt.push({ branch: "classHeritage", classHeritage: _t26__t16_classHeritage! } as never); }
                    else i = _t23;
                  }
                  if (!_t22) { _t20 = false; break _b21; }
                }
              }
              if (!_t20) { i = _t17; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t172 = i; let _t18 = true;
                _b19: {
                  {
                    let _t27 = false;
                    if (!_t27) {
                      let _t31__t16_classHeritage: (ClassHeritageNode) | undefined;
                      const _t28 = i; let _t29 = true;
                      _b30: {
                        if (!__nodeOf(c, i, "ClassHeritage")) { _t29 = false; break _b30; }
                        _t31__t16_classHeritage = c[i] as ClassHeritageNode;
                        i++;
                      }
                      if (_t29) { _t27 = true; _t16_alt.push({ branch: "classHeritage", classHeritage: _t31__t16_classHeritage! } as never); }
                      else i = _t28;
                    }
                    if (!_t27) { _t18 = false; break _b19; }
                  }
                }
                if (!_t18) { i = _t172; break; }
              }
            }
          }
          if (_t14) { _t12 = true; alt.push({ branch: "extends_", alt: _t16_alt } as never); }
          else i = _t13;
        }
        if (!_t12) {
          const _t35_alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] = [];
          const _t32 = i; let _t33 = true;
          _b34: {
            if (!__lit(c, i, src, "implements", "$keyword")) { _t33 = false; break _b34; }
            i++;
            {
              const _t36 = i; let _t39 = true;
              _b40: {
                {
                  let _t41 = false;
                  if (!_t41) {
                    let _t45__t35_classHeritage: (ClassHeritageNode) | undefined;
                    const _t42 = i; let _t43 = true;
                    _b44: {
                      if (!__nodeOf(c, i, "ClassHeritage")) { _t43 = false; break _b44; }
                      _t45__t35_classHeritage = c[i] as ClassHeritageNode;
                      i++;
                    }
                    if (_t43) { _t41 = true; _t35_alt.push({ branch: "classHeritage", classHeritage: _t45__t35_classHeritage! } as never); }
                    else i = _t42;
                  }
                  if (!_t41) { _t39 = false; break _b40; }
                }
              }
              if (!_t39) { i = _t36; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t362 = i; let _t37 = true;
                _b38: {
                  {
                    let _t46 = false;
                    if (!_t46) {
                      let _t50__t35_classHeritage: (ClassHeritageNode) | undefined;
                      const _t47 = i; let _t48 = true;
                      _b49: {
                        if (!__nodeOf(c, i, "ClassHeritage")) { _t48 = false; break _b49; }
                        _t50__t35_classHeritage = c[i] as ClassHeritageNode;
                        i++;
                      }
                      if (_t48) { _t46 = true; _t35_alt.push({ branch: "classHeritage", classHeritage: _t50__t35_classHeritage! } as never); }
                      else i = _t47;
                    }
                    if (!_t46) { _t37 = false; break _b38; }
                  }
                }
                if (!_t37) { i = _t362; break; }
              }
            }
          }
          if (_t33) { _t12 = true; alt.push({ branch: "implements", alt: _t35_alt } as never); }
          else i = _t32;
        }
        if (!_t12) { _t10 = false; break _b11; }
      }
    }
    if (!_t10) { i = _t9; break; }
    if (i === _t9) break;
  }
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  for (;;) {
    const _t51 = i; let _t52 = true;
    _b53: {
      if (!__nodeOf(c, i, "ClassMember")) { _t52 = false; break _b53; }
      classMember.push(c[i] as ClassMemberNode);
      i++;
    }
    if (!_t52) { i = _t51; break; }
    if (i === _t51) break;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "decoratorExpr", decoratorExpr, abstractTok, ident: ident!, typeParams, alt, classMember };
}

function _Decl$decoratorExpr2(c: readonly CstChild[], src: string): DeclMatch | null {
  const decoratorExpr: (DecoratorExprNode)[] = [];
  let abstractTok: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[] = [];
  const classMember: (ClassMemberNode)[] = [];
  let i = 0;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "DecoratorExpr")) { _t1 = false; break _b2; }
      decoratorExpr.push(c[i] as DecoratorExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "abstract", "$keyword")) { _t4 = false; break _b5; }
      abstractTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, "class", "$keyword")) return null;
  i++;
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__nodeOf(c, i, "TypeParams")) { _t7 = false; break _b8; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t7) i = _t6;
  }
  for (;;) {
    const _t9 = i; let _t10 = true;
    _b11: {
      {
        let _t12 = false;
        if (!_t12) {
          const _t16_alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] = [];
          const _t13 = i; let _t14 = true;
          _b15: {
            if (!__lit(c, i, src, "extends", "$keyword")) { _t14 = false; break _b15; }
            i++;
            {
              const _t17 = i; let _t20 = true;
              _b21: {
                {
                  let _t22 = false;
                  if (!_t22) {
                    let _t26__t16_classHeritage: (ClassHeritageNode) | undefined;
                    const _t23 = i; let _t24 = true;
                    _b25: {
                      if (!__nodeOf(c, i, "ClassHeritage")) { _t24 = false; break _b25; }
                      _t26__t16_classHeritage = c[i] as ClassHeritageNode;
                      i++;
                    }
                    if (_t24) { _t22 = true; _t16_alt.push({ branch: "classHeritage", classHeritage: _t26__t16_classHeritage! } as never); }
                    else i = _t23;
                  }
                  if (!_t22) { _t20 = false; break _b21; }
                }
              }
              if (!_t20) { i = _t17; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t172 = i; let _t18 = true;
                _b19: {
                  {
                    let _t27 = false;
                    if (!_t27) {
                      let _t31__t16_classHeritage: (ClassHeritageNode) | undefined;
                      const _t28 = i; let _t29 = true;
                      _b30: {
                        if (!__nodeOf(c, i, "ClassHeritage")) { _t29 = false; break _b30; }
                        _t31__t16_classHeritage = c[i] as ClassHeritageNode;
                        i++;
                      }
                      if (_t29) { _t27 = true; _t16_alt.push({ branch: "classHeritage", classHeritage: _t31__t16_classHeritage! } as never); }
                      else i = _t28;
                    }
                    if (!_t27) { _t18 = false; break _b19; }
                  }
                }
                if (!_t18) { i = _t172; break; }
              }
            }
          }
          if (_t14) { _t12 = true; alt.push({ branch: "extends_", alt: _t16_alt } as never); }
          else i = _t13;
        }
        if (!_t12) {
          const _t35_alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] = [];
          const _t32 = i; let _t33 = true;
          _b34: {
            if (!__lit(c, i, src, "implements", "$keyword")) { _t33 = false; break _b34; }
            i++;
            {
              const _t36 = i; let _t39 = true;
              _b40: {
                {
                  let _t41 = false;
                  if (!_t41) {
                    let _t45__t35_classHeritage: (ClassHeritageNode) | undefined;
                    const _t42 = i; let _t43 = true;
                    _b44: {
                      if (!__nodeOf(c, i, "ClassHeritage")) { _t43 = false; break _b44; }
                      _t45__t35_classHeritage = c[i] as ClassHeritageNode;
                      i++;
                    }
                    if (_t43) { _t41 = true; _t35_alt.push({ branch: "classHeritage", classHeritage: _t45__t35_classHeritage! } as never); }
                    else i = _t42;
                  }
                  if (!_t41) { _t39 = false; break _b40; }
                }
              }
              if (!_t39) { i = _t36; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t362 = i; let _t37 = true;
                _b38: {
                  {
                    let _t46 = false;
                    if (!_t46) {
                      let _t50__t35_classHeritage: (ClassHeritageNode) | undefined;
                      const _t47 = i; let _t48 = true;
                      _b49: {
                        if (!__nodeOf(c, i, "ClassHeritage")) { _t48 = false; break _b49; }
                        _t50__t35_classHeritage = c[i] as ClassHeritageNode;
                        i++;
                      }
                      if (_t48) { _t46 = true; _t35_alt.push({ branch: "classHeritage", classHeritage: _t50__t35_classHeritage! } as never); }
                      else i = _t47;
                    }
                    if (!_t46) { _t37 = false; break _b38; }
                  }
                }
                if (!_t37) { i = _t362; break; }
              }
            }
          }
          if (_t33) { _t12 = true; alt.push({ branch: "implements", alt: _t35_alt } as never); }
          else i = _t32;
        }
        if (!_t12) { _t10 = false; break _b11; }
      }
    }
    if (!_t10) { i = _t9; break; }
    if (i === _t9) break;
  }
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  for (;;) {
    const _t51 = i; let _t52 = true;
    _b53: {
      if (!__nodeOf(c, i, "ClassMember")) { _t52 = false; break _b53; }
      classMember.push(c[i] as ClassMemberNode);
      i++;
    }
    if (!_t52) { i = _t51; break; }
    if (i === _t51) break;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "decoratorExpr2", decoratorExpr, abstractTok, typeParams, alt, classMember };
}

function _Decl$enum_(c: readonly CstChild[], src: string): DeclMatch | null {
  let ident: (CstLeaf) | undefined;
  const enumMember: (EnumMemberNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "enum", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "EnumMember")) { _t3 = false; break _b4; }
      enumMember.push(c[i] as EnumMemberNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "EnumMember")) { _t1 = false; break _b2; }
        enumMember.push(c[i] as EnumMemberNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "enum_", ident: ident!, enumMember };
}

function _Decl$declare(c: readonly CstChild[], src: string): DeclMatch | null {
  let ident: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "declare", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "function", "$keyword")) return null;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "*", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__nodeOf(c, i, "TypeParams")) { _t4 = false; break _b5; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t6 = i; let _t9 = true;
    _b10: {
      if (!__nodeOf(c, i, "Param")) { _t9 = false; break _b10; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t9) { i = _t6; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t62 = i; let _t7 = true;
      _b8: {
        if (!__nodeOf(c, i, "Param")) { _t7 = false; break _b8; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t7) { i = _t62; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  {
    const _t11 = i; let _t12 = true;
    _b13: {
      if (!__lit(c, i, src, ":", "$punct")) { _t12 = false; break _b13; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t12 = false; break _b13; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t12) i = _t11;
  }
  {
    const _t14 = i; let _t15 = true;
    _b16: {
      if (!__lit(c, i, src, ";", "$punct")) { _t15 = false; break _b16; }
      i++;
    }
    if (!_t15) i = _t14;
  }
  if (i !== c.length) return null;
  return { arm: "declare", ident: ident!, typeParams, param, type };
}

function _Decl$declare2(c: readonly CstChild[], src: string): DeclMatch | null {
  let alt: ({ branch: "decl"; decl: DeclNode } | { branch: "stmt"; stmt: StmtNode }) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "declare", "$keyword")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_decl: (DeclNode) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!__nodeOf(c, i, "Decl")) { _t2 = false; break _b3; }
        _t4_decl = c[i] as DeclNode;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "decl", decl: _t4_decl! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_stmt: (StmtNode) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!__nodeOf(c, i, "Stmt")) { _t6 = false; break _b7; }
        _t8_stmt = c[i] as StmtNode;
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "stmt", stmt: _t8_stmt! }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) return null;
  }
  if (i !== c.length) return null;
  return { arm: "declare2", alt: alt! };
}

function _Decl$namespace(c: readonly CstChild[], src: string): DeclMatch | null {
  let ident: (CstLeaf) | undefined;
  const ident2: (CstLeaf)[] = [];
  const stmt: (StmtNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "namespace", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, ".", "$punct")) { _t1 = false; break _b2; }
      i++;
      if (!(__tok(c, i, "Ident"))) { _t1 = false; break _b2; }
      ident2.push(c[i] as CstLeaf);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  for (;;) {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__nodeOf(c, i, "Stmt")) { _t4 = false; break _b5; }
      stmt.push(c[i] as StmtNode);
      i++;
    }
    if (!_t4) { i = _t3; break; }
    if (i === _t3) break;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "namespace", ident: ident!, ident2, stmt };
}

function _Decl$module(c: readonly CstChild[], src: string): DeclMatch | null {
  let alt: ({ branch: "ident"; ident: CstLeaf; ident2: (CstLeaf)[] } | { branch: "string"; string: CstLeaf }) | undefined;
  const stmt: (StmtNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "module", "$keyword")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_ident: (CstLeaf) | undefined;
      const _t4_ident2: (CstLeaf)[] = [];
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "Ident"))) { _t2 = false; break _b3; }
        _t4_ident = c[i] as CstLeaf;
        i++;
        for (;;) {
          const _t5 = i; let _t6 = true;
          _b7: {
            if (!__lit(c, i, src, ".", "$punct")) { _t6 = false; break _b7; }
            i++;
            if (!(__tok(c, i, "Ident"))) { _t6 = false; break _b7; }
            _t4_ident2.push(c[i] as CstLeaf);
            i++;
          }
          if (!_t6) { i = _t5; break; }
          if (i === _t5) break;
        }
      }
      if (_t2) { _t0 = true; alt = ({ branch: "ident", ident: _t4_ident!, ident2: _t4_ident2 }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t11_string: (CstLeaf) | undefined;
      const _t8 = i; let _t9 = true;
      _b10: {
        if (!(__tok(c, i, "String"))) { _t9 = false; break _b10; }
        _t11_string = c[i] as CstLeaf;
        i++;
      }
      if (_t9) { _t0 = true; alt = ({ branch: "string", string: _t11_string! }) as typeof alt; }
      else i = _t8;
    }
    if (!_t0) return null;
  }
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  for (;;) {
    const _t12 = i; let _t13 = true;
    _b14: {
      if (!__nodeOf(c, i, "Stmt")) { _t13 = false; break _b14; }
      stmt.push(c[i] as StmtNode);
      i++;
    }
    if (!_t13) { i = _t12; break; }
    if (i === _t12) break;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "module", alt: alt!, stmt };
}

function _Decl$export_(c: readonly CstChild[], src: string): DeclMatch | null {
  let alt: ({ branch: "decl"; decl: DeclNode } | { branch: "stmt"; stmt: StmtNode }) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "export", "$keyword")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_decl: (DeclNode) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!__nodeOf(c, i, "Decl")) { _t2 = false; break _b3; }
        _t4_decl = c[i] as DeclNode;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "decl", decl: _t4_decl! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_stmt: (StmtNode) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!__nodeOf(c, i, "Stmt")) { _t6 = false; break _b7; }
        _t8_stmt = c[i] as StmtNode;
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "stmt", stmt: _t8_stmt! }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) return null;
  }
  if (i !== c.length) return null;
  return { arm: "export_", alt: alt! };
}

function _Decl$decoratorExpr3(c: readonly CstChild[], src: string): DeclMatch | null {
  const decoratorExpr: (DecoratorExprNode)[] = [];
  let alt: ({ branch: "decl"; decl: DeclNode } | { branch: "let_"; let_Kw: "let" | "const" | "var"; binding: (BindingNode)[] } | { branch: "await_"; awaitTok?: CstLeaf; binding: BindingNode; binding2: (BindingNode)[] }) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "DecoratorExpr")) { _t1 = false; break _b2; }
      decoratorExpr.push(c[i] as DecoratorExprNode);
      i++;
    }
    if (!_t1) return null;
  }
  for (;;) {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__nodeOf(c, i, "DecoratorExpr")) { _t4 = false; break _b5; }
      decoratorExpr.push(c[i] as DecoratorExprNode);
      i++;
    }
    if (!_t4) { i = _t3; break; }
    if (i === _t3) break;
  }
  {
    let _t6 = false;
    if (!_t6) {
      let _t10_decl: (DeclNode) | undefined;
      const _t7 = i; let _t8 = true;
      _b9: {
        if (!__nodeOf(c, i, "Decl")) { _t8 = false; break _b9; }
        _t10_decl = c[i] as DeclNode;
        i++;
      }
      if (_t8) { _t6 = true; alt = ({ branch: "decl", decl: _t10_decl! }) as typeof alt; }
      else i = _t7;
    }
    if (!_t6) {
      let _t14_let_Kw: ("let" | "const" | "var") | undefined;
      const _t14_binding: (BindingNode)[] = [];
      const _t11 = i; let _t12 = true;
      _b13: {
        if (!(__lit(c, i, src, "let", "$keyword") || __lit(c, i, src, "const", "$keyword") || __lit(c, i, src, "var", "$keyword"))) { _t12 = false; break _b13; }
        _t14_let_Kw = src.slice(c[i].offset, c[i].end) as "let" | "const" | "var";
        i++;
        {
          const _t15 = i; let _t18 = true;
          _b19: {
            if (!__nodeOf(c, i, "Binding")) { _t18 = false; break _b19; }
            _t14_binding.push(c[i] as BindingNode);
            i++;
          }
          if (!_t18) { i = _t15; }
          else for (;;) {
            if (!__lit(c, i, src, ",", "$punct")) break;
            i++;
            const _t152 = i; let _t16 = true;
            _b17: {
              if (!__nodeOf(c, i, "Binding")) { _t16 = false; break _b17; }
              _t14_binding.push(c[i] as BindingNode);
              i++;
            }
            if (!_t16) { i = _t152; break; }
          }
        }
        {
          const _t20 = i; let _t21 = true;
          _b22: {
            if (!__lit(c, i, src, ";", "$punct")) { _t21 = false; break _b22; }
            i++;
          }
          if (!_t21) i = _t20;
        }
      }
      if (_t12) { _t6 = true; alt = ({ branch: "let_", let_Kw: _t14_let_Kw!, binding: _t14_binding }) as typeof alt; }
      else i = _t11;
    }
    if (!_t6) {
      let _t26_awaitTok: (CstLeaf) | undefined;
      let _t26_binding: (BindingNode) | undefined;
      const _t26_binding2: (BindingNode)[] = [];
      const _t23 = i; let _t24 = true;
      _b25: {
        {
          const _t27 = i; let _t28 = true;
          _b29: {
            if (!__lit(c, i, src, "await", "$keyword")) { _t28 = false; break _b29; }
            _t26_awaitTok = c[i] as CstLeaf;
            i++;
          }
          if (!_t28) i = _t27;
        }
        if (!__lit(c, i, src, "using", "$keyword")) { _t24 = false; break _b25; }
        i++;
        if (!__nodeOf(c, i, "Binding")) { _t24 = false; break _b25; }
        _t26_binding = c[i] as BindingNode;
        i++;
        for (;;) {
          const _t30 = i; let _t31 = true;
          _b32: {
            if (!__lit(c, i, src, ",", "$punct")) { _t31 = false; break _b32; }
            i++;
            if (!__nodeOf(c, i, "Binding")) { _t31 = false; break _b32; }
            _t26_binding2.push(c[i] as BindingNode);
            i++;
          }
          if (!_t31) { i = _t30; break; }
          if (i === _t30) break;
        }
        {
          const _t33 = i; let _t34 = true;
          _b35: {
            if (!__lit(c, i, src, ";", "$punct")) { _t34 = false; break _b35; }
            i++;
          }
          if (!_t34) i = _t33;
        }
      }
      if (_t24) { _t6 = true; alt = ({ branch: "await_", awaitTok: _t26_awaitTok, binding: _t26_binding!, binding2: _t26_binding2 }) as typeof alt; }
      else i = _t23;
    }
    if (!_t6) return null;
  }
  if (i !== c.length) return null;
  return { arm: "decoratorExpr3", decoratorExpr, alt: alt! };
}

function _Decl$export_2(c: readonly CstChild[], src: string): DeclMatch | null {
  const decoratorExpr: (DecoratorExprNode)[] = [];
  let alt: ({ branch: "async"; asyncTok?: CstLeaf; ident?: CstLeaf; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode; alt: { branch: "block"; block: BlockNode } | { branch: "semi" } } | { branch: "abstract"; ident: CstLeaf; typeParams?: TypeParamsNode; alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[]; classMember: (ClassMemberNode)[] } | { branch: "abstract2"; typeParams?: TypeParamsNode; alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[]; classMember: (ClassMemberNode)[] } | { branch: "expr"; expr: ExprNode }) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "export", "$keyword")) return null;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "DecoratorExpr")) { _t1 = false; break _b2; }
      decoratorExpr.push(c[i] as DecoratorExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, "default", "$keyword")) return null;
  i++;
  {
    let _t3 = false;
    if (!_t3) {
      let _t7_asyncTok: (CstLeaf) | undefined;
      let _t7_ident: (CstLeaf) | undefined;
      let _t7_typeParams: (TypeParamsNode) | undefined;
      const _t7_param: (ParamNode)[] = [];
      let _t7_type: (TypeNode) | undefined;
      let _t7_alt: ({ branch: "block"; block: BlockNode } | { branch: "semi" }) | undefined;
      const _t4 = i; let _t5 = true;
      _b6: {
        {
          const _t8 = i; let _t9 = true;
          _b10: {
            if (!__lit(c, i, src, "async", "$keyword")) { _t9 = false; break _b10; }
            _t7_asyncTok = c[i] as CstLeaf;
            i++;
          }
          if (!_t9) i = _t8;
        }
        if (!__lit(c, i, src, "function", "$keyword")) { _t5 = false; break _b6; }
        i++;
        {
          const _t11 = i; let _t12 = true;
          _b13: {
            if (!__lit(c, i, src, "*", "$punct")) { _t12 = false; break _b13; }
            i++;
          }
          if (!_t12) i = _t11;
        }
        {
          const _t14 = i; let _t15 = true;
          _b16: {
            if (!(__tok(c, i, "Ident"))) { _t15 = false; break _b16; }
            _t7_ident = c[i] as CstLeaf;
            i++;
          }
          if (!_t15) i = _t14;
        }
        {
          const _t17 = i; let _t18 = true;
          _b19: {
            if (!__nodeOf(c, i, "TypeParams")) { _t18 = false; break _b19; }
            _t7_typeParams = c[i] as TypeParamsNode;
            i++;
          }
          if (!_t18) i = _t17;
        }
        if (!__lit(c, i, src, "(", "$punct")) { _t5 = false; break _b6; }
        i++;
        {
          const _t20 = i; let _t23 = true;
          _b24: {
            if (!__nodeOf(c, i, "Param")) { _t23 = false; break _b24; }
            _t7_param.push(c[i] as ParamNode);
            i++;
          }
          if (!_t23) { i = _t20; }
          else for (;;) {
            if (!__lit(c, i, src, ",", "$punct")) break;
            i++;
            const _t202 = i; let _t21 = true;
            _b22: {
              if (!__nodeOf(c, i, "Param")) { _t21 = false; break _b22; }
              _t7_param.push(c[i] as ParamNode);
              i++;
            }
            if (!_t21) { i = _t202; break; }
          }
        }
        if (!__lit(c, i, src, ")", "$punct")) { _t5 = false; break _b6; }
        i++;
        {
          const _t25 = i; let _t26 = true;
          _b27: {
            if (!__lit(c, i, src, ":", "$punct")) { _t26 = false; break _b27; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t26 = false; break _b27; }
            _t7_type = c[i] as TypeNode;
            i++;
          }
          if (!_t26) i = _t25;
        }
        {
          let _t28 = false;
          if (!_t28) {
            let _t32__t7_block: (BlockNode) | undefined;
            const _t29 = i; let _t30 = true;
            _b31: {
              if (!__nodeOf(c, i, "Block")) { _t30 = false; break _b31; }
              _t32__t7_block = c[i] as BlockNode;
              i++;
            }
            if (_t30) { _t28 = true; _t7_alt = ({ branch: "block", block: _t32__t7_block! }) as typeof _t7_alt; }
            else i = _t29;
          }
          if (!_t28) {
            const _t33 = i; let _t34 = true;
            _b35: {
              {
                const _t37 = i; let _t38 = true;
                _b39: {
                  if (!__lit(c, i, src, ";", "$punct")) { _t38 = false; break _b39; }
                  i++;
                }
                if (!_t38) i = _t37;
              }
            }
            if (_t34) { _t28 = true; _t7_alt = ({ branch: "semi" }) as typeof _t7_alt; }
            else i = _t33;
          }
          if (!_t28) { _t5 = false; break _b6; }
        }
      }
      if (_t5) { _t3 = true; alt = ({ branch: "async", asyncTok: _t7_asyncTok, ident: _t7_ident, typeParams: _t7_typeParams, param: _t7_param, type: _t7_type, alt: _t7_alt! }) as typeof alt; }
      else i = _t4;
    }
    if (!_t3) {
      let _t43_ident: (CstLeaf) | undefined;
      let _t43_typeParams: (TypeParamsNode) | undefined;
      const _t43_alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[] = [];
      const _t43_classMember: (ClassMemberNode)[] = [];
      const _t40 = i; let _t41 = true;
      _b42: {
        if (!__lit(c, i, src, "abstract", "$keyword")) { _t41 = false; break _b42; }
        i++;
        if (!__lit(c, i, src, "class", "$keyword")) { _t41 = false; break _b42; }
        i++;
        if (!(__tok(c, i, "Ident"))) { _t41 = false; break _b42; }
        _t43_ident = c[i] as CstLeaf;
        i++;
        {
          const _t44 = i; let _t45 = true;
          _b46: {
            if (!__nodeOf(c, i, "TypeParams")) { _t45 = false; break _b46; }
            _t43_typeParams = c[i] as TypeParamsNode;
            i++;
          }
          if (!_t45) i = _t44;
        }
        for (;;) {
          const _t47 = i; let _t48 = true;
          _b49: {
            {
              let _t50 = false;
              if (!_t50) {
                const _t54__t43_alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] = [];
                const _t51 = i; let _t52 = true;
                _b53: {
                  if (!__lit(c, i, src, "extends", "$keyword")) { _t52 = false; break _b53; }
                  i++;
                  {
                    const _t55 = i; let _t58 = true;
                    _b59: {
                      {
                        let _t60 = false;
                        if (!_t60) {
                          let _t64__t54__t43_classHeritage: (ClassHeritageNode) | undefined;
                          const _t61 = i; let _t62 = true;
                          _b63: {
                            if (!__nodeOf(c, i, "ClassHeritage")) { _t62 = false; break _b63; }
                            _t64__t54__t43_classHeritage = c[i] as ClassHeritageNode;
                            i++;
                          }
                          if (_t62) { _t60 = true; _t54__t43_alt.push({ branch: "classHeritage", classHeritage: _t64__t54__t43_classHeritage! } as never); }
                          else i = _t61;
                        }
                        if (!_t60) { _t58 = false; break _b59; }
                      }
                    }
                    if (!_t58) { i = _t55; }
                    else for (;;) {
                      if (!__lit(c, i, src, ",", "$punct")) break;
                      i++;
                      const _t552 = i; let _t56 = true;
                      _b57: {
                        {
                          let _t65 = false;
                          if (!_t65) {
                            let _t69__t54__t43_classHeritage: (ClassHeritageNode) | undefined;
                            const _t66 = i; let _t67 = true;
                            _b68: {
                              if (!__nodeOf(c, i, "ClassHeritage")) { _t67 = false; break _b68; }
                              _t69__t54__t43_classHeritage = c[i] as ClassHeritageNode;
                              i++;
                            }
                            if (_t67) { _t65 = true; _t54__t43_alt.push({ branch: "classHeritage", classHeritage: _t69__t54__t43_classHeritage! } as never); }
                            else i = _t66;
                          }
                          if (!_t65) { _t56 = false; break _b57; }
                        }
                      }
                      if (!_t56) { i = _t552; break; }
                    }
                  }
                }
                if (_t52) { _t50 = true; _t43_alt.push({ branch: "extends_", alt: _t54__t43_alt } as never); }
                else i = _t51;
              }
              if (!_t50) {
                const _t73__t43_alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] = [];
                const _t70 = i; let _t71 = true;
                _b72: {
                  if (!__lit(c, i, src, "implements", "$keyword")) { _t71 = false; break _b72; }
                  i++;
                  {
                    const _t74 = i; let _t77 = true;
                    _b78: {
                      {
                        let _t79 = false;
                        if (!_t79) {
                          let _t83__t73__t43_classHeritage: (ClassHeritageNode) | undefined;
                          const _t80 = i; let _t81 = true;
                          _b82: {
                            if (!__nodeOf(c, i, "ClassHeritage")) { _t81 = false; break _b82; }
                            _t83__t73__t43_classHeritage = c[i] as ClassHeritageNode;
                            i++;
                          }
                          if (_t81) { _t79 = true; _t73__t43_alt.push({ branch: "classHeritage", classHeritage: _t83__t73__t43_classHeritage! } as never); }
                          else i = _t80;
                        }
                        if (!_t79) { _t77 = false; break _b78; }
                      }
                    }
                    if (!_t77) { i = _t74; }
                    else for (;;) {
                      if (!__lit(c, i, src, ",", "$punct")) break;
                      i++;
                      const _t742 = i; let _t75 = true;
                      _b76: {
                        {
                          let _t84 = false;
                          if (!_t84) {
                            let _t88__t73__t43_classHeritage: (ClassHeritageNode) | undefined;
                            const _t85 = i; let _t86 = true;
                            _b87: {
                              if (!__nodeOf(c, i, "ClassHeritage")) { _t86 = false; break _b87; }
                              _t88__t73__t43_classHeritage = c[i] as ClassHeritageNode;
                              i++;
                            }
                            if (_t86) { _t84 = true; _t73__t43_alt.push({ branch: "classHeritage", classHeritage: _t88__t73__t43_classHeritage! } as never); }
                            else i = _t85;
                          }
                          if (!_t84) { _t75 = false; break _b76; }
                        }
                      }
                      if (!_t75) { i = _t742; break; }
                    }
                  }
                }
                if (_t71) { _t50 = true; _t43_alt.push({ branch: "implements", alt: _t73__t43_alt } as never); }
                else i = _t70;
              }
              if (!_t50) { _t48 = false; break _b49; }
            }
          }
          if (!_t48) { i = _t47; break; }
          if (i === _t47) break;
        }
        if (!__lit(c, i, src, "{", "$punct")) { _t41 = false; break _b42; }
        i++;
        for (;;) {
          const _t89 = i; let _t90 = true;
          _b91: {
            if (!__nodeOf(c, i, "ClassMember")) { _t90 = false; break _b91; }
            _t43_classMember.push(c[i] as ClassMemberNode);
            i++;
          }
          if (!_t90) { i = _t89; break; }
          if (i === _t89) break;
        }
        if (!__lit(c, i, src, "}", "$punct")) { _t41 = false; break _b42; }
        i++;
      }
      if (_t41) { _t3 = true; alt = ({ branch: "abstract", ident: _t43_ident!, typeParams: _t43_typeParams, alt: _t43_alt, classMember: _t43_classMember }) as typeof alt; }
      else i = _t40;
    }
    if (!_t3) {
      let _t95_typeParams: (TypeParamsNode) | undefined;
      const _t95_alt: ({ branch: "extends_"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] } | { branch: "implements"; alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] })[] = [];
      const _t95_classMember: (ClassMemberNode)[] = [];
      const _t92 = i; let _t93 = true;
      _b94: {
        if (!__lit(c, i, src, "abstract", "$keyword")) { _t93 = false; break _b94; }
        i++;
        if (!__lit(c, i, src, "class", "$keyword")) { _t93 = false; break _b94; }
        i++;
        {
          const _t96 = i; let _t97 = true;
          _b98: {
            if (!__nodeOf(c, i, "TypeParams")) { _t97 = false; break _b98; }
            _t95_typeParams = c[i] as TypeParamsNode;
            i++;
          }
          if (!_t97) i = _t96;
        }
        for (;;) {
          const _t99 = i; let _t100 = true;
          _b101: {
            {
              let _t102 = false;
              if (!_t102) {
                const _t106__t95_alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] = [];
                const _t103 = i; let _t104 = true;
                _b105: {
                  if (!__lit(c, i, src, "extends", "$keyword")) { _t104 = false; break _b105; }
                  i++;
                  {
                    const _t107 = i; let _t110 = true;
                    _b111: {
                      {
                        let _t112 = false;
                        if (!_t112) {
                          let _t116__t106__t95_classHeritage: (ClassHeritageNode) | undefined;
                          const _t113 = i; let _t114 = true;
                          _b115: {
                            if (!__nodeOf(c, i, "ClassHeritage")) { _t114 = false; break _b115; }
                            _t116__t106__t95_classHeritage = c[i] as ClassHeritageNode;
                            i++;
                          }
                          if (_t114) { _t112 = true; _t106__t95_alt.push({ branch: "classHeritage", classHeritage: _t116__t106__t95_classHeritage! } as never); }
                          else i = _t113;
                        }
                        if (!_t112) { _t110 = false; break _b111; }
                      }
                    }
                    if (!_t110) { i = _t107; }
                    else for (;;) {
                      if (!__lit(c, i, src, ",", "$punct")) break;
                      i++;
                      const _t1072 = i; let _t108 = true;
                      _b109: {
                        {
                          let _t117 = false;
                          if (!_t117) {
                            let _t121__t106__t95_classHeritage: (ClassHeritageNode) | undefined;
                            const _t118 = i; let _t119 = true;
                            _b120: {
                              if (!__nodeOf(c, i, "ClassHeritage")) { _t119 = false; break _b120; }
                              _t121__t106__t95_classHeritage = c[i] as ClassHeritageNode;
                              i++;
                            }
                            if (_t119) { _t117 = true; _t106__t95_alt.push({ branch: "classHeritage", classHeritage: _t121__t106__t95_classHeritage! } as never); }
                            else i = _t118;
                          }
                          if (!_t117) { _t108 = false; break _b109; }
                        }
                      }
                      if (!_t108) { i = _t1072; break; }
                    }
                  }
                }
                if (_t104) { _t102 = true; _t95_alt.push({ branch: "extends_", alt: _t106__t95_alt } as never); }
                else i = _t103;
              }
              if (!_t102) {
                const _t125__t95_alt: ({ branch: "classHeritage"; classHeritage: ClassHeritageNode })[] = [];
                const _t122 = i; let _t123 = true;
                _b124: {
                  if (!__lit(c, i, src, "implements", "$keyword")) { _t123 = false; break _b124; }
                  i++;
                  {
                    const _t126 = i; let _t129 = true;
                    _b130: {
                      {
                        let _t131 = false;
                        if (!_t131) {
                          let _t135__t125__t95_classHeritage: (ClassHeritageNode) | undefined;
                          const _t132 = i; let _t133 = true;
                          _b134: {
                            if (!__nodeOf(c, i, "ClassHeritage")) { _t133 = false; break _b134; }
                            _t135__t125__t95_classHeritage = c[i] as ClassHeritageNode;
                            i++;
                          }
                          if (_t133) { _t131 = true; _t125__t95_alt.push({ branch: "classHeritage", classHeritage: _t135__t125__t95_classHeritage! } as never); }
                          else i = _t132;
                        }
                        if (!_t131) { _t129 = false; break _b130; }
                      }
                    }
                    if (!_t129) { i = _t126; }
                    else for (;;) {
                      if (!__lit(c, i, src, ",", "$punct")) break;
                      i++;
                      const _t1262 = i; let _t127 = true;
                      _b128: {
                        {
                          let _t136 = false;
                          if (!_t136) {
                            let _t140__t125__t95_classHeritage: (ClassHeritageNode) | undefined;
                            const _t137 = i; let _t138 = true;
                            _b139: {
                              if (!__nodeOf(c, i, "ClassHeritage")) { _t138 = false; break _b139; }
                              _t140__t125__t95_classHeritage = c[i] as ClassHeritageNode;
                              i++;
                            }
                            if (_t138) { _t136 = true; _t125__t95_alt.push({ branch: "classHeritage", classHeritage: _t140__t125__t95_classHeritage! } as never); }
                            else i = _t137;
                          }
                          if (!_t136) { _t127 = false; break _b128; }
                        }
                      }
                      if (!_t127) { i = _t1262; break; }
                    }
                  }
                }
                if (_t123) { _t102 = true; _t95_alt.push({ branch: "implements", alt: _t125__t95_alt } as never); }
                else i = _t122;
              }
              if (!_t102) { _t100 = false; break _b101; }
            }
          }
          if (!_t100) { i = _t99; break; }
          if (i === _t99) break;
        }
        if (!__lit(c, i, src, "{", "$punct")) { _t93 = false; break _b94; }
        i++;
        for (;;) {
          const _t141 = i; let _t142 = true;
          _b143: {
            if (!__nodeOf(c, i, "ClassMember")) { _t142 = false; break _b143; }
            _t95_classMember.push(c[i] as ClassMemberNode);
            i++;
          }
          if (!_t142) { i = _t141; break; }
          if (i === _t141) break;
        }
        if (!__lit(c, i, src, "}", "$punct")) { _t93 = false; break _b94; }
        i++;
      }
      if (_t93) { _t3 = true; alt = ({ branch: "abstract2", typeParams: _t95_typeParams, alt: _t95_alt, classMember: _t95_classMember }) as typeof alt; }
      else i = _t92;
    }
    if (!_t3) {
      let _t147_expr: (ExprNode) | undefined;
      const _t144 = i; let _t145 = true;
      _b146: {
        if (!__nodeOf(c, i, "Expr")) { _t145 = false; break _b146; }
        _t147_expr = c[i] as ExprNode;
        i++;
        {
          const _t148 = i; let _t149 = true;
          _b150: {
            if (!__lit(c, i, src, ";", "$punct")) { _t149 = false; break _b150; }
            i++;
          }
          if (!_t149) i = _t148;
        }
      }
      if (_t145) { _t3 = true; alt = ({ branch: "expr", expr: _t147_expr! }) as typeof alt; }
      else i = _t144;
    }
    if (!_t3) return null;
  }
  if (i !== c.length) return null;
  return { arm: "export_2", decoratorExpr, alt: alt! };
}

function _Decl$export_3(c: readonly CstChild[], src: string): DeclMatch | null {
  let alt: ({ branch: "from"; string: CstLeaf } | { branch: "as"; ident: CstLeaf; string: CstLeaf }) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "export", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "*", "$punct")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_string: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!__lit(c, i, src, "from", "$keyword")) { _t2 = false; break _b3; }
        i++;
        if (!(__tok(c, i, "String"))) { _t2 = false; break _b3; }
        _t4_string = c[i] as CstLeaf;
        i++;
        {
          const _t5 = i; let _t6 = true;
          _b7: {
            if (!__lit(c, i, src, ";", "$punct")) { _t6 = false; break _b7; }
            i++;
          }
          if (!_t6) i = _t5;
        }
      }
      if (_t2) { _t0 = true; alt = ({ branch: "from", string: _t4_string! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t11_ident: (CstLeaf) | undefined;
      let _t11_string: (CstLeaf) | undefined;
      const _t8 = i; let _t9 = true;
      _b10: {
        if (!__lit(c, i, src, "as", "$keyword")) { _t9 = false; break _b10; }
        i++;
        if (!(__tok(c, i, "Ident"))) { _t9 = false; break _b10; }
        _t11_ident = c[i] as CstLeaf;
        i++;
        if (!__lit(c, i, src, "from", "$keyword")) { _t9 = false; break _b10; }
        i++;
        if (!(__tok(c, i, "String"))) { _t9 = false; break _b10; }
        _t11_string = c[i] as CstLeaf;
        i++;
        {
          const _t12 = i; let _t13 = true;
          _b14: {
            if (!__lit(c, i, src, ";", "$punct")) { _t13 = false; break _b14; }
            i++;
          }
          if (!_t13) i = _t12;
        }
      }
      if (_t9) { _t0 = true; alt = ({ branch: "as", ident: _t11_ident!, string: _t11_string! }) as typeof alt; }
      else i = _t8;
    }
    if (!_t0) return null;
  }
  if (i !== c.length) return null;
  return { arm: "export_3", alt: alt! };
}

function _Decl$export_4(c: readonly CstChild[], src: string): DeclMatch | null {
  const exportSpecifier: (ExportSpecifierNode)[] = [];
  let string: (CstLeaf) | undefined;
  let fromTok: (CstLeaf) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "export", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "ExportSpecifier")) { _t3 = false; break _b4; }
      exportSpecifier.push(c[i] as ExportSpecifierNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "ExportSpecifier")) { _t1 = false; break _b2; }
        exportSpecifier.push(c[i] as ExportSpecifierNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  {
    const _t5 = i; let _t6 = true;
    _b7: {
      if (!__lit(c, i, src, "from", "$keyword")) { _t6 = false; break _b7; }
      fromTok = c[i] as CstLeaf;
      i++;
      if (!(__tok(c, i, "String"))) { _t6 = false; break _b7; }
      string = c[i] as CstLeaf;
      i++;
    }
    if (!_t6) i = _t5;
  }
  {
    const _t8 = i; let _t9 = true;
    _b10: {
      if (!__lit(c, i, src, ";", "$punct")) { _t9 = false; break _b10; }
      i++;
    }
    if (!_t9) i = _t8;
  }
  if (i !== c.length) return null;
  return { arm: "export_4", exportSpecifier, string, fromTok };
}

function _Decl$export_5(c: readonly CstChild[], src: string): DeclMatch | null {
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "export", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "=", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, ";", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (i !== c.length) return null;
  return { arm: "export_5", expr: expr! };
}

function _Decl$export_6(c: readonly CstChild[], src: string): DeclMatch | null {
  const exportSpecifier: (ExportSpecifierNode)[] = [];
  let string: (CstLeaf) | undefined;
  let fromTok: (CstLeaf) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "export", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "type", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "ExportSpecifier")) { _t3 = false; break _b4; }
      exportSpecifier.push(c[i] as ExportSpecifierNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "ExportSpecifier")) { _t1 = false; break _b2; }
        exportSpecifier.push(c[i] as ExportSpecifierNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  {
    const _t5 = i; let _t6 = true;
    _b7: {
      if (!__lit(c, i, src, "from", "$keyword")) { _t6 = false; break _b7; }
      fromTok = c[i] as CstLeaf;
      i++;
      if (!(__tok(c, i, "String"))) { _t6 = false; break _b7; }
      string = c[i] as CstLeaf;
      i++;
    }
    if (!_t6) i = _t5;
  }
  {
    const _t8 = i; let _t9 = true;
    _b10: {
      if (!__lit(c, i, src, ";", "$punct")) { _t9 = false; break _b10; }
      i++;
    }
    if (!_t9) i = _t8;
  }
  if (i !== c.length) return null;
  return { arm: "export_6", exportSpecifier, string, fromTok };
}

function _Decl$const_(c: readonly CstChild[], src: string): DeclMatch | null {
  let ident: (CstLeaf) | undefined;
  const enumMember: (EnumMemberNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "const", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "enum", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "EnumMember")) { _t3 = false; break _b4; }
      enumMember.push(c[i] as EnumMemberNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "EnumMember")) { _t1 = false; break _b2; }
        enumMember.push(c[i] as EnumMemberNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "const_", ident: ident!, enumMember };
}

function _Decl$import_(c: readonly CstChild[], src: string): DeclMatch | null {
  let alt: ({ branch: "importClause"; importClause: ImportClauseNode; string: CstLeaf } | { branch: "type"; importClause: ImportClauseNode; string: CstLeaf } | { branch: "ident"; ident: CstLeaf; expr: ExprNode } | { branch: "string"; string: CstLeaf }) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "import", "$keyword")) return null;
  i++;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_importClause: (ImportClauseNode) | undefined;
      let _t4_string: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!__nodeOf(c, i, "ImportClause")) { _t2 = false; break _b3; }
        _t4_importClause = c[i] as ImportClauseNode;
        i++;
        if (!__lit(c, i, src, "from", "$keyword")) { _t2 = false; break _b3; }
        i++;
        if (!(__tok(c, i, "String"))) { _t2 = false; break _b3; }
        _t4_string = c[i] as CstLeaf;
        i++;
        {
          const _t5 = i; let _t6 = true;
          _b7: {
            if (!__lit(c, i, src, ";", "$punct")) { _t6 = false; break _b7; }
            i++;
          }
          if (!_t6) i = _t5;
        }
      }
      if (_t2) { _t0 = true; alt = ({ branch: "importClause", importClause: _t4_importClause!, string: _t4_string! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t11_importClause: (ImportClauseNode) | undefined;
      let _t11_string: (CstLeaf) | undefined;
      const _t8 = i; let _t9 = true;
      _b10: {
        if (!__lit(c, i, src, "type", "$keyword")) { _t9 = false; break _b10; }
        i++;
        if (!__nodeOf(c, i, "ImportClause")) { _t9 = false; break _b10; }
        _t11_importClause = c[i] as ImportClauseNode;
        i++;
        if (!__lit(c, i, src, "from", "$keyword")) { _t9 = false; break _b10; }
        i++;
        if (!(__tok(c, i, "String"))) { _t9 = false; break _b10; }
        _t11_string = c[i] as CstLeaf;
        i++;
        {
          const _t12 = i; let _t13 = true;
          _b14: {
            if (!__lit(c, i, src, ";", "$punct")) { _t13 = false; break _b14; }
            i++;
          }
          if (!_t13) i = _t12;
        }
      }
      if (_t9) { _t0 = true; alt = ({ branch: "type", importClause: _t11_importClause!, string: _t11_string! }) as typeof alt; }
      else i = _t8;
    }
    if (!_t0) {
      let _t18_ident: (CstLeaf) | undefined;
      let _t18_expr: (ExprNode) | undefined;
      const _t15 = i; let _t16 = true;
      _b17: {
        if (!(__tok(c, i, "Ident"))) { _t16 = false; break _b17; }
        _t18_ident = c[i] as CstLeaf;
        i++;
        if (!__lit(c, i, src, "=", "$punct")) { _t16 = false; break _b17; }
        i++;
        if (!__nodeOf(c, i, "Expr")) { _t16 = false; break _b17; }
        _t18_expr = c[i] as ExprNode;
        i++;
        {
          const _t19 = i; let _t20 = true;
          _b21: {
            if (!__lit(c, i, src, ";", "$punct")) { _t20 = false; break _b21; }
            i++;
          }
          if (!_t20) i = _t19;
        }
      }
      if (_t16) { _t0 = true; alt = ({ branch: "ident", ident: _t18_ident!, expr: _t18_expr! }) as typeof alt; }
      else i = _t15;
    }
    if (!_t0) {
      let _t25_string: (CstLeaf) | undefined;
      const _t22 = i; let _t23 = true;
      _b24: {
        if (!(__tok(c, i, "String"))) { _t23 = false; break _b24; }
        _t25_string = c[i] as CstLeaf;
        i++;
        {
          const _t26 = i; let _t27 = true;
          _b28: {
            if (!__lit(c, i, src, ";", "$punct")) { _t27 = false; break _b28; }
            i++;
          }
          if (!_t27) i = _t26;
        }
      }
      if (_t23) { _t0 = true; alt = ({ branch: "string", string: _t25_string! }) as typeof alt; }
      else i = _t22;
    }
    if (!_t0) return null;
  }
  if (i !== c.length) return null;
  return { arm: "import_", alt: alt! };
}

function _Decl$decoratorExpr4(c: readonly CstChild[], src: string): DeclMatch | null {
  const decoratorExpr: (DecoratorExprNode)[] = [];
  let alt: ({ branch: "decl"; decl: DeclNode } | { branch: "stmt"; stmt: StmtNode }) | undefined;
  let i = 0;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "DecoratorExpr")) { _t1 = false; break _b2; }
      decoratorExpr.push(c[i] as DecoratorExprNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!__lit(c, i, src, "export", "$keyword")) return null;
  i++;
  {
    let _t3 = false;
    if (!_t3) {
      let _t7_decl: (DeclNode) | undefined;
      const _t4 = i; let _t5 = true;
      _b6: {
        if (!__nodeOf(c, i, "Decl")) { _t5 = false; break _b6; }
        _t7_decl = c[i] as DeclNode;
        i++;
      }
      if (_t5) { _t3 = true; alt = ({ branch: "decl", decl: _t7_decl! }) as typeof alt; }
      else i = _t4;
    }
    if (!_t3) {
      let _t11_stmt: (StmtNode) | undefined;
      const _t8 = i; let _t9 = true;
      _b10: {
        if (!__nodeOf(c, i, "Stmt")) { _t9 = false; break _b10; }
        _t11_stmt = c[i] as StmtNode;
        i++;
      }
      if (_t9) { _t3 = true; alt = ({ branch: "stmt", stmt: _t11_stmt! }) as typeof alt; }
      else i = _t8;
    }
    if (!_t3) return null;
  }
  if (i !== c.length) return null;
  return { arm: "decoratorExpr4", decoratorExpr, alt: alt! };
}

export function matchDecl(n: DeclNode, src: string): DeclMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "DecoratorExpr": {
        { const m = _Decl$decoratorExpr(c, src); if (m !== null) return m; }
        { const m = _Decl$decoratorExpr2(c, src); if (m !== null) return m; }
        { const m = _Decl$decoratorExpr3(c, src); if (m !== null) return m; }
        { const m = _Decl$decoratorExpr4(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 97: {
        { const m = _Decl$async(c, src); if (m !== null) return m; }
        { const m = _Decl$decoratorExpr(c, src); if (m !== null) return m; }
        { const m = _Decl$decoratorExpr2(c, src); if (m !== null) return m; }
        break;
      }
      case 99: {
        { const m = _Decl$decoratorExpr(c, src); if (m !== null) return m; }
        { const m = _Decl$decoratorExpr2(c, src); if (m !== null) return m; }
        { const m = _Decl$const_(c, src); if (m !== null) return m; }
        break;
      }
      case 100: {
        { const m = _Decl$declare(c, src); if (m !== null) return m; }
        { const m = _Decl$declare2(c, src); if (m !== null) return m; }
        break;
      }
      case 101: {
        { const m = _Decl$enum_(c, src); if (m !== null) return m; }
        { const m = _Decl$export_(c, src); if (m !== null) return m; }
        { const m = _Decl$export_2(c, src); if (m !== null) return m; }
        { const m = _Decl$export_3(c, src); if (m !== null) return m; }
        { const m = _Decl$export_4(c, src); if (m !== null) return m; }
        { const m = _Decl$export_5(c, src); if (m !== null) return m; }
        { const m = _Decl$export_6(c, src); if (m !== null) return m; }
        { const m = _Decl$decoratorExpr4(c, src); if (m !== null) return m; }
        break;
      }
      case 102: {
        { const m = _Decl$async(c, src); if (m !== null) return m; }
        break;
      }
      case 105: {
        { const m = _Decl$interface(c, src); if (m !== null) return m; }
        { const m = _Decl$import_(c, src); if (m !== null) return m; }
        break;
      }
      case 109: {
        { const m = _Decl$module(c, src); if (m !== null) return m; }
        break;
      }
      case 110: {
        { const m = _Decl$namespace(c, src); if (m !== null) return m; }
        break;
      }
      case 116: {
        { const m = _Decl$type(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
    }
  }
  throw new Error("matchDecl: no arm matches" + ' @' + n.offset);
}

export type InterfaceMemberMatch =
  | { arm: "new_"; newTok?: CstLeaf; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode }
  | { arm: "get"; getKw: "get" | "set"; memberName: MemberNameNode; param: (ParamNode)[]; type?: TypeNode }
  | { arm: "static_"; staticTok?: CstLeaf; _Kw?: "+" | "-"; readonlyTok?: CstLeaf; ident: CstLeaf; type: TypeNode; type2?: TypeNode; asTok?: CstLeaf; _Kw2?: "+" | "-"; type3: TypeNode }
  | { arm: "readonly"; memberName: MemberNameNode; type: TypeNode }
  | { arm: "memberName"; memberName: MemberNameNode; alt: { branch: "typeParams"; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode } | { branch: "seq"; type?: TypeNode } }
  | { arm: "static_2"; staticTok?: CstLeaf; readonlyTok?: CstLeaf; param: (ParamNode)[]; type?: TypeNode };

function _InterfaceMember$new_(c: readonly CstChild[], src: string): InterfaceMemberMatch | null {
  let newTok: (CstLeaf) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "new", "$keyword")) { _t1 = false; break _b2; }
      newTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__nodeOf(c, i, "TypeParams")) { _t4 = false; break _b5; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t6 = i; let _t9 = true;
    _b10: {
      if (!__nodeOf(c, i, "Param")) { _t9 = false; break _b10; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t9) { i = _t6; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t62 = i; let _t7 = true;
      _b8: {
        if (!__nodeOf(c, i, "Param")) { _t7 = false; break _b8; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t7) { i = _t62; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  {
    const _t11 = i; let _t12 = true;
    _b13: {
      if (!__lit(c, i, src, ":", "$punct")) { _t12 = false; break _b13; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t12 = false; break _b13; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t12) i = _t11;
  }
  if (i !== c.length) return null;
  return { arm: "new_", newTok, typeParams, param, type };
}

function _InterfaceMember$get(c: readonly CstChild[], src: string): InterfaceMemberMatch | null {
  let getKw: ("get" | "set") | undefined;
  let memberName: (MemberNameNode) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!(__lit(c, i, src, "get", "$keyword") || __lit(c, i, src, "set", "$keyword"))) return null;
  getKw = src.slice(c[i].offset, c[i].end) as "get" | "set";
  i++;
  if (!__nodeOf(c, i, "MemberName")) return null;
  memberName = c[i] as MemberNameNode;
  i++;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "Param")) { _t3 = false; break _b4; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "Param")) { _t1 = false; break _b2; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  {
    const _t5 = i; let _t6 = true;
    _b7: {
      if (!__lit(c, i, src, ":", "$punct")) { _t6 = false; break _b7; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t6 = false; break _b7; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t6) i = _t5;
  }
  if (i !== c.length) return null;
  return { arm: "get", getKw: getKw!, memberName: memberName!, param, type };
}

function _InterfaceMember$static_(c: readonly CstChild[], src: string): InterfaceMemberMatch | null {
  let staticTok: (CstLeaf) | undefined;
  let _Kw: ("+" | "-") | undefined;
  let readonlyTok: (CstLeaf) | undefined;
  let ident: (CstLeaf) | undefined;
  let type: (TypeNode) | undefined;
  let type2: (TypeNode) | undefined;
  let asTok: (CstLeaf) | undefined;
  let _Kw2: ("+" | "-") | undefined;
  let type3: (TypeNode) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "static", "$keyword")) { _t1 = false; break _b2; }
      staticTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!(__lit(c, i, src, "+", "$punct") || __lit(c, i, src, "-", "$punct"))) { _t4 = false; break _b5; }
      _Kw = src.slice(c[i].offset, c[i].end) as "+" | "-";
      i++;
    }
    if (!_t4) i = _t3;
  }
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__lit(c, i, src, "readonly", "$keyword")) { _t7 = false; break _b8; }
      readonlyTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t7) i = _t6;
  }
  if (!__lit(c, i, src, "[", "$punct")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (!__lit(c, i, src, "in", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  {
    const _t9 = i; let _t10 = true;
    _b11: {
      if (!__lit(c, i, src, "as", "$keyword")) { _t10 = false; break _b11; }
      asTok = c[i] as CstLeaf;
      i++;
      if (!__nodeOf(c, i, "Type")) { _t10 = false; break _b11; }
      type2 = c[i] as TypeNode;
      i++;
    }
    if (!_t10) i = _t9;
  }
  if (!__lit(c, i, src, "]", "$punct")) return null;
  i++;
  {
    const _t12 = i; let _t13 = true;
    _b14: {
      if (!(__lit(c, i, src, "+", "$punct") || __lit(c, i, src, "-", "$punct"))) { _t13 = false; break _b14; }
      _Kw2 = src.slice(c[i].offset, c[i].end) as "+" | "-";
      i++;
    }
    if (!_t13) i = _t12;
  }
  {
    const _t15 = i; let _t16 = true;
    _b17: {
      if (!__lit(c, i, src, "?", "$punct")) { _t16 = false; break _b17; }
      i++;
    }
    if (!_t16) i = _t15;
  }
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type3 = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "static_", staticTok, _Kw, readonlyTok, ident: ident!, type: type!, type2, asTok, _Kw2, type3: type3! };
}

function _InterfaceMember$readonly(c: readonly CstChild[], src: string): InterfaceMemberMatch | null {
  let memberName: (MemberNameNode) | undefined;
  let type: (TypeNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "readonly", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "MemberName")) return null;
  memberName = c[i] as MemberNameNode;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "?", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (!__lit(c, i, src, ":", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Type")) return null;
  type = c[i] as TypeNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "readonly", memberName: memberName!, type: type! };
}

function _InterfaceMember$memberName(c: readonly CstChild[], src: string): InterfaceMemberMatch | null {
  let memberName: (MemberNameNode) | undefined;
  let alt: ({ branch: "typeParams"; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode } | { branch: "seq"; type?: TypeNode }) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "MemberName")) return null;
  memberName = c[i] as MemberNameNode;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "?", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    let _t3 = false;
    if (!_t3) {
      let _t7_typeParams: (TypeParamsNode) | undefined;
      const _t7_param: (ParamNode)[] = [];
      let _t7_type: (TypeNode) | undefined;
      const _t4 = i; let _t5 = true;
      _b6: {
        {
          const _t8 = i; let _t9 = true;
          _b10: {
            if (!__nodeOf(c, i, "TypeParams")) { _t9 = false; break _b10; }
            _t7_typeParams = c[i] as TypeParamsNode;
            i++;
          }
          if (!_t9) i = _t8;
        }
        if (!__lit(c, i, src, "(", "$punct")) { _t5 = false; break _b6; }
        i++;
        {
          const _t11 = i; let _t14 = true;
          _b15: {
            if (!__nodeOf(c, i, "Param")) { _t14 = false; break _b15; }
            _t7_param.push(c[i] as ParamNode);
            i++;
          }
          if (!_t14) { i = _t11; }
          else for (;;) {
            if (!__lit(c, i, src, ",", "$punct")) break;
            i++;
            const _t112 = i; let _t12 = true;
            _b13: {
              if (!__nodeOf(c, i, "Param")) { _t12 = false; break _b13; }
              _t7_param.push(c[i] as ParamNode);
              i++;
            }
            if (!_t12) { i = _t112; break; }
          }
        }
        if (!__lit(c, i, src, ")", "$punct")) { _t5 = false; break _b6; }
        i++;
        {
          const _t16 = i; let _t17 = true;
          _b18: {
            if (!__lit(c, i, src, ":", "$punct")) { _t17 = false; break _b18; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t17 = false; break _b18; }
            _t7_type = c[i] as TypeNode;
            i++;
          }
          if (!_t17) i = _t16;
        }
      }
      if (_t5) { _t3 = true; alt = ({ branch: "typeParams", typeParams: _t7_typeParams, param: _t7_param, type: _t7_type }) as typeof alt; }
      else i = _t4;
    }
    if (!_t3) {
      let _t22_type: (TypeNode) | undefined;
      const _t19 = i; let _t20 = true;
      _b21: {
        {
          const _t23 = i; let _t24 = true;
          _b25: {
            if (!__lit(c, i, src, ":", "$punct")) { _t24 = false; break _b25; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t24 = false; break _b25; }
            _t22_type = c[i] as TypeNode;
            i++;
          }
          if (!_t24) i = _t23;
        }
      }
      if (_t20) { _t3 = true; alt = ({ branch: "seq", type: _t22_type }) as typeof alt; }
      else i = _t19;
    }
    if (!_t3) return null;
  }
  if (i !== c.length) return null;
  return { arm: "memberName", memberName: memberName!, alt: alt! };
}

function _InterfaceMember$static_2(c: readonly CstChild[], src: string): InterfaceMemberMatch | null {
  let staticTok: (CstLeaf) | undefined;
  let readonlyTok: (CstLeaf) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let i = 0;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "static", "$keyword")) { _t1 = false; break _b2; }
      staticTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "readonly", "$keyword")) { _t4 = false; break _b5; }
      readonlyTok = c[i] as CstLeaf;
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, "[", "$punct")) return null;
  i++;
  {
    const _t6 = i; let _t9 = true;
    _b10: {
      if (!__nodeOf(c, i, "Param")) { _t9 = false; break _b10; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t9) { i = _t6; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t62 = i; let _t7 = true;
      _b8: {
        if (!__nodeOf(c, i, "Param")) { _t7 = false; break _b8; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t7) { i = _t62; break; }
    }
  }
  if (!__lit(c, i, src, "]", "$punct")) return null;
  i++;
  {
    const _t11 = i; let _t12 = true;
    _b13: {
      if (!__lit(c, i, src, ":", "$punct")) { _t12 = false; break _b13; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t12 = false; break _b13; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t12) i = _t11;
  }
  if (i !== c.length) return null;
  return { arm: "static_2", staticTok, readonlyTok, param, type };
}

export function matchInterfaceMember(n: InterfaceMemberNode, src: string): InterfaceMemberMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "MemberName": {
        { const m = _InterfaceMember$memberName(c, src); if (m !== null) return m; }
        break;
      }
      case "TypeParams": {
        { const m = _InterfaceMember$new_(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 40: {
        { const m = _InterfaceMember$new_(c, src); if (m !== null) return m; }
        break;
      }
      case 43: {
        { const m = _InterfaceMember$static_(c, src); if (m !== null) return m; }
        break;
      }
      case 45: {
        { const m = _InterfaceMember$static_(c, src); if (m !== null) return m; }
        break;
      }
      case 91: {
        { const m = _InterfaceMember$static_(c, src); if (m !== null) return m; }
        { const m = _InterfaceMember$static_2(c, src); if (m !== null) return m; }
        break;
      }
      case 103: {
        { const m = _InterfaceMember$get(c, src); if (m !== null) return m; }
        break;
      }
      case 110: {
        { const m = _InterfaceMember$new_(c, src); if (m !== null) return m; }
        break;
      }
      case 114: {
        { const m = _InterfaceMember$static_(c, src); if (m !== null) return m; }
        { const m = _InterfaceMember$readonly(c, src); if (m !== null) return m; }
        { const m = _InterfaceMember$static_2(c, src); if (m !== null) return m; }
        break;
      }
      case 115: {
        { const m = _InterfaceMember$get(c, src); if (m !== null) return m; }
        { const m = _InterfaceMember$static_(c, src); if (m !== null) return m; }
        { const m = _InterfaceMember$static_2(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
    }
  }
  throw new Error("matchInterfaceMember: no arm matches" + ' @' + n.offset);
}

export type ClassMemberMatch =
  | { arm: "semi" }
  | { arm: "decoratorExpr"; decoratorExpr: DecoratorExprNode }
  | { arm: "constructor"; param: (ParamNode)[]; block: BlockNode }
  | { arm: "static_"; block: BlockNode }
  | { arm: "public"; publicKw: ("public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async")[]; alt: { branch: "star"; memberName: MemberNameNode; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode; block?: BlockNode } | { branch: "get"; getKw: "get" | "set"; memberName: MemberNameNode; param: (ParamNode)[]; type?: TypeNode; block?: BlockNode } | { branch: "bracket"; ident: CstLeaf; type: TypeNode; type2: TypeNode } | { branch: "memberName"; memberName: MemberNameNode; alt: { branch: "question"; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode; block?: BlockNode } | { branch: "bang"; type?: TypeNode; expr?: ExprNode } } }
  | { arm: "memberName"; memberName: MemberNameNode; type?: TypeNode; expr?: ExprNode }
  | { arm: "memberName2"; memberName: MemberNameNode; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode; block?: BlockNode };

function _ClassMember$semi(c: readonly CstChild[], src: string): ClassMemberMatch | null {
  let i = 0;
  if (!__lit(c, i, src, ";", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "semi" };
}

function _ClassMember$decoratorExpr(c: readonly CstChild[], src: string): ClassMemberMatch | null {
  let decoratorExpr: (DecoratorExprNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "DecoratorExpr")) return null;
  decoratorExpr = c[i] as DecoratorExprNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "decoratorExpr", decoratorExpr: decoratorExpr! };
}

function _ClassMember$constructor(c: readonly CstChild[], src: string): ClassMemberMatch | null {
  const param: (ParamNode)[] = [];
  let block: (BlockNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "constructor", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "Param")) { _t3 = false; break _b4; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "Param")) { _t1 = false; break _b2; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Block")) return null;
  block = c[i] as BlockNode;
  i++;
  {
    const _t5 = i; let _t6 = true;
    _b7: {
      if (!__lit(c, i, src, ";", "$punct")) { _t6 = false; break _b7; }
      i++;
    }
    if (!_t6) i = _t5;
  }
  if (i !== c.length) return null;
  return { arm: "constructor", param, block: block! };
}

function _ClassMember$static_(c: readonly CstChild[], src: string): ClassMemberMatch | null {
  let block: (BlockNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "static", "$keyword")) return null;
  i++;
  if (!__nodeOf(c, i, "Block")) return null;
  block = c[i] as BlockNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "static_", block: block! };
}

function _ClassMember$public(c: readonly CstChild[], src: string): ClassMemberMatch | null {
  const publicKw: ("public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async")[] = [];
  let alt: ({ branch: "star"; memberName: MemberNameNode; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode; block?: BlockNode } | { branch: "get"; getKw: "get" | "set"; memberName: MemberNameNode; param: (ParamNode)[]; type?: TypeNode; block?: BlockNode } | { branch: "bracket"; ident: CstLeaf; type: TypeNode; type2: TypeNode } | { branch: "memberName"; memberName: MemberNameNode; alt: { branch: "question"; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode; block?: BlockNode } | { branch: "bang"; type?: TypeNode; expr?: ExprNode } }) | undefined;
  let i = 0;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!(__lit(c, i, src, "public", "$keyword") || __lit(c, i, src, "private", "$keyword") || __lit(c, i, src, "protected", "$keyword") || __lit(c, i, src, "static", "$keyword") || __lit(c, i, src, "abstract", "$keyword") || __lit(c, i, src, "readonly", "$keyword") || __lit(c, i, src, "override", "$keyword") || __lit(c, i, src, "accessor", "$keyword") || __lit(c, i, src, "async", "$keyword"))) { _t1 = false; break _b2; }
      publicKw.push(src.slice(c[i].offset, c[i].end) as "public" | "private" | "protected" | "static" | "abstract" | "readonly" | "override" | "accessor" | "async");
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  {
    let _t3 = false;
    if (!_t3) {
      let _t7_memberName: (MemberNameNode) | undefined;
      let _t7_typeParams: (TypeParamsNode) | undefined;
      const _t7_param: (ParamNode)[] = [];
      let _t7_type: (TypeNode) | undefined;
      let _t7_block: (BlockNode) | undefined;
      const _t4 = i; let _t5 = true;
      _b6: {
        if (!__lit(c, i, src, "*", "$punct")) { _t5 = false; break _b6; }
        i++;
        if (!__nodeOf(c, i, "MemberName")) { _t5 = false; break _b6; }
        _t7_memberName = c[i] as MemberNameNode;
        i++;
        {
          const _t8 = i; let _t9 = true;
          _b10: {
            if (!__lit(c, i, src, "?", "$punct")) { _t9 = false; break _b10; }
            i++;
          }
          if (!_t9) i = _t8;
        }
        {
          const _t11 = i; let _t12 = true;
          _b13: {
            if (!__nodeOf(c, i, "TypeParams")) { _t12 = false; break _b13; }
            _t7_typeParams = c[i] as TypeParamsNode;
            i++;
          }
          if (!_t12) i = _t11;
        }
        if (!__lit(c, i, src, "(", "$punct")) { _t5 = false; break _b6; }
        i++;
        {
          const _t14 = i; let _t17 = true;
          _b18: {
            if (!__nodeOf(c, i, "Param")) { _t17 = false; break _b18; }
            _t7_param.push(c[i] as ParamNode);
            i++;
          }
          if (!_t17) { i = _t14; }
          else for (;;) {
            if (!__lit(c, i, src, ",", "$punct")) break;
            i++;
            const _t142 = i; let _t15 = true;
            _b16: {
              if (!__nodeOf(c, i, "Param")) { _t15 = false; break _b16; }
              _t7_param.push(c[i] as ParamNode);
              i++;
            }
            if (!_t15) { i = _t142; break; }
          }
        }
        if (!__lit(c, i, src, ")", "$punct")) { _t5 = false; break _b6; }
        i++;
        {
          const _t19 = i; let _t20 = true;
          _b21: {
            if (!__lit(c, i, src, ":", "$punct")) { _t20 = false; break _b21; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t20 = false; break _b21; }
            _t7_type = c[i] as TypeNode;
            i++;
          }
          if (!_t20) i = _t19;
        }
        {
          const _t22 = i; let _t23 = true;
          _b24: {
            if (!__nodeOf(c, i, "Block")) { _t23 = false; break _b24; }
            _t7_block = c[i] as BlockNode;
            i++;
          }
          if (!_t23) i = _t22;
        }
        {
          const _t25 = i; let _t26 = true;
          _b27: {
            if (!__lit(c, i, src, ";", "$punct")) { _t26 = false; break _b27; }
            i++;
          }
          if (!_t26) i = _t25;
        }
      }
      if (_t5) { _t3 = true; alt = ({ branch: "star", memberName: _t7_memberName!, typeParams: _t7_typeParams, param: _t7_param, type: _t7_type, block: _t7_block }) as typeof alt; }
      else i = _t4;
    }
    if (!_t3) {
      let _t31_getKw: ("get" | "set") | undefined;
      let _t31_memberName: (MemberNameNode) | undefined;
      const _t31_param: (ParamNode)[] = [];
      let _t31_type: (TypeNode) | undefined;
      let _t31_block: (BlockNode) | undefined;
      const _t28 = i; let _t29 = true;
      _b30: {
        if (!(__lit(c, i, src, "get", "$keyword") || __lit(c, i, src, "set", "$keyword"))) { _t29 = false; break _b30; }
        _t31_getKw = src.slice(c[i].offset, c[i].end) as "get" | "set";
        i++;
        if (!__nodeOf(c, i, "MemberName")) { _t29 = false; break _b30; }
        _t31_memberName = c[i] as MemberNameNode;
        i++;
        if (!__lit(c, i, src, "(", "$punct")) { _t29 = false; break _b30; }
        i++;
        {
          const _t32 = i; let _t33 = true;
          _b34: {
            {
              const _t35 = i; let _t38 = true;
              _b39: {
                if (!__nodeOf(c, i, "Param")) { _t38 = false; break _b39; }
                _t31_param.push(c[i] as ParamNode);
                i++;
              }
              if (!_t38) { i = _t35; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t352 = i; let _t36 = true;
                _b37: {
                  if (!__nodeOf(c, i, "Param")) { _t36 = false; break _b37; }
                  _t31_param.push(c[i] as ParamNode);
                  i++;
                }
                if (!_t36) { i = _t352; break; }
              }
            }
          }
          if (!_t33) i = _t32;
        }
        if (!__lit(c, i, src, ")", "$punct")) { _t29 = false; break _b30; }
        i++;
        {
          const _t40 = i; let _t41 = true;
          _b42: {
            if (!__lit(c, i, src, ":", "$punct")) { _t41 = false; break _b42; }
            i++;
            if (!__nodeOf(c, i, "Type")) { _t41 = false; break _b42; }
            _t31_type = c[i] as TypeNode;
            i++;
          }
          if (!_t41) i = _t40;
        }
        {
          const _t43 = i; let _t44 = true;
          _b45: {
            if (!__nodeOf(c, i, "Block")) { _t44 = false; break _b45; }
            _t31_block = c[i] as BlockNode;
            i++;
          }
          if (!_t44) i = _t43;
        }
        {
          const _t46 = i; let _t47 = true;
          _b48: {
            if (!__lit(c, i, src, ";", "$punct")) { _t47 = false; break _b48; }
            i++;
          }
          if (!_t47) i = _t46;
        }
      }
      if (_t29) { _t3 = true; alt = ({ branch: "get", getKw: _t31_getKw!, memberName: _t31_memberName!, param: _t31_param, type: _t31_type, block: _t31_block }) as typeof alt; }
      else i = _t28;
    }
    if (!_t3) {
      let _t52_ident: (CstLeaf) | undefined;
      let _t52_type: (TypeNode) | undefined;
      let _t52_type2: (TypeNode) | undefined;
      const _t49 = i; let _t50 = true;
      _b51: {
        if (!__lit(c, i, src, "[", "$punct")) { _t50 = false; break _b51; }
        i++;
        if (!(__tok(c, i, "Ident"))) { _t50 = false; break _b51; }
        _t52_ident = c[i] as CstLeaf;
        i++;
        if (!__lit(c, i, src, ":", "$punct")) { _t50 = false; break _b51; }
        i++;
        if (!__nodeOf(c, i, "Type")) { _t50 = false; break _b51; }
        _t52_type = c[i] as TypeNode;
        i++;
        if (!__lit(c, i, src, "]", "$punct")) { _t50 = false; break _b51; }
        i++;
        if (!__lit(c, i, src, ":", "$punct")) { _t50 = false; break _b51; }
        i++;
        if (!__nodeOf(c, i, "Type")) { _t50 = false; break _b51; }
        _t52_type2 = c[i] as TypeNode;
        i++;
        {
          const _t53 = i; let _t54 = true;
          _b55: {
            if (!__lit(c, i, src, ";", "$punct")) { _t54 = false; break _b55; }
            i++;
          }
          if (!_t54) i = _t53;
        }
      }
      if (_t50) { _t3 = true; alt = ({ branch: "bracket", ident: _t52_ident!, type: _t52_type!, type2: _t52_type2! }) as typeof alt; }
      else i = _t49;
    }
    if (!_t3) {
      let _t59_memberName: (MemberNameNode) | undefined;
      let _t59_alt: ({ branch: "question"; typeParams?: TypeParamsNode; param: (ParamNode)[]; type?: TypeNode; block?: BlockNode } | { branch: "bang"; type?: TypeNode; expr?: ExprNode }) | undefined;
      const _t56 = i; let _t57 = true;
      _b58: {
        if (!__nodeOf(c, i, "MemberName")) { _t57 = false; break _b58; }
        _t59_memberName = c[i] as MemberNameNode;
        i++;
        {
          let _t60 = false;
          if (!_t60) {
            let _t64__t59_typeParams: (TypeParamsNode) | undefined;
            const _t64__t59_param: (ParamNode)[] = [];
            let _t64__t59_type: (TypeNode) | undefined;
            let _t64__t59_block: (BlockNode) | undefined;
            const _t61 = i; let _t62 = true;
            _b63: {
              {
                const _t65 = i; let _t66 = true;
                _b67: {
                  if (!__lit(c, i, src, "?", "$punct")) { _t66 = false; break _b67; }
                  i++;
                }
                if (!_t66) i = _t65;
              }
              {
                const _t68 = i; let _t69 = true;
                _b70: {
                  if (!__nodeOf(c, i, "TypeParams")) { _t69 = false; break _b70; }
                  _t64__t59_typeParams = c[i] as TypeParamsNode;
                  i++;
                }
                if (!_t69) i = _t68;
              }
              if (!__lit(c, i, src, "(", "$punct")) { _t62 = false; break _b63; }
              i++;
              {
                const _t71 = i; let _t74 = true;
                _b75: {
                  if (!__nodeOf(c, i, "Param")) { _t74 = false; break _b75; }
                  _t64__t59_param.push(c[i] as ParamNode);
                  i++;
                }
                if (!_t74) { i = _t71; }
                else for (;;) {
                  if (!__lit(c, i, src, ",", "$punct")) break;
                  i++;
                  const _t712 = i; let _t72 = true;
                  _b73: {
                    if (!__nodeOf(c, i, "Param")) { _t72 = false; break _b73; }
                    _t64__t59_param.push(c[i] as ParamNode);
                    i++;
                  }
                  if (!_t72) { i = _t712; break; }
                }
              }
              if (!__lit(c, i, src, ")", "$punct")) { _t62 = false; break _b63; }
              i++;
              {
                const _t76 = i; let _t77 = true;
                _b78: {
                  if (!__lit(c, i, src, ":", "$punct")) { _t77 = false; break _b78; }
                  i++;
                  if (!__nodeOf(c, i, "Type")) { _t77 = false; break _b78; }
                  _t64__t59_type = c[i] as TypeNode;
                  i++;
                }
                if (!_t77) i = _t76;
              }
              {
                const _t79 = i; let _t80 = true;
                _b81: {
                  if (!__nodeOf(c, i, "Block")) { _t80 = false; break _b81; }
                  _t64__t59_block = c[i] as BlockNode;
                  i++;
                }
                if (!_t80) i = _t79;
              }
              {
                const _t82 = i; let _t83 = true;
                _b84: {
                  if (!__lit(c, i, src, ";", "$punct")) { _t83 = false; break _b84; }
                  i++;
                }
                if (!_t83) i = _t82;
              }
            }
            if (_t62) { _t60 = true; _t59_alt = ({ branch: "question", typeParams: _t64__t59_typeParams, param: _t64__t59_param, type: _t64__t59_type, block: _t64__t59_block }) as typeof _t59_alt; }
            else i = _t61;
          }
          if (!_t60) {
            let _t88__t59_type: (TypeNode) | undefined;
            let _t88__t59_expr: (ExprNode) | undefined;
            const _t85 = i; let _t86 = true;
            _b87: {
              {
                const _t89 = i; let _t90 = true;
                _b91: {
                  if (!__lit(c, i, src, "!", "$punct")) { _t90 = false; break _b91; }
                  i++;
                }
                if (!_t90) i = _t89;
              }
              {
                const _t92 = i; let _t93 = true;
                _b94: {
                  if (!__lit(c, i, src, "?", "$punct")) { _t93 = false; break _b94; }
                  i++;
                }
                if (!_t93) i = _t92;
              }
              {
                const _t95 = i; let _t96 = true;
                _b97: {
                  if (!__lit(c, i, src, ":", "$punct")) { _t96 = false; break _b97; }
                  i++;
                  if (!__nodeOf(c, i, "Type")) { _t96 = false; break _b97; }
                  _t88__t59_type = c[i] as TypeNode;
                  i++;
                }
                if (!_t96) i = _t95;
              }
              {
                const _t98 = i; let _t99 = true;
                _b100: {
                  if (!__lit(c, i, src, "=", "$punct")) { _t99 = false; break _b100; }
                  i++;
                  if (!__nodeOf(c, i, "Expr")) { _t99 = false; break _b100; }
                  _t88__t59_expr = c[i] as ExprNode;
                  i++;
                }
                if (!_t99) i = _t98;
              }
              {
                const _t101 = i; let _t102 = true;
                _b103: {
                  if (!__lit(c, i, src, ";", "$punct")) { _t102 = false; break _b103; }
                  i++;
                }
                if (!_t102) i = _t101;
              }
            }
            if (_t86) { _t60 = true; _t59_alt = ({ branch: "bang", type: _t88__t59_type, expr: _t88__t59_expr }) as typeof _t59_alt; }
            else i = _t85;
          }
          if (!_t60) { _t57 = false; break _b58; }
        }
      }
      if (_t57) { _t3 = true; alt = ({ branch: "memberName", memberName: _t59_memberName!, alt: _t59_alt! }) as typeof alt; }
      else i = _t56;
    }
    if (!_t3) return null;
  }
  if (i !== c.length) return null;
  return { arm: "public", publicKw, alt: alt! };
}

function _ClassMember$memberName(c: readonly CstChild[], src: string): ClassMemberMatch | null {
  let memberName: (MemberNameNode) | undefined;
  let type: (TypeNode) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "MemberName")) return null;
  memberName = c[i] as MemberNameNode;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "!", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__lit(c, i, src, "?", "$punct")) { _t4 = false; break _b5; }
      i++;
    }
    if (!_t4) i = _t3;
  }
  {
    const _t6 = i; let _t7 = true;
    _b8: {
      if (!__lit(c, i, src, ":", "$punct")) { _t7 = false; break _b8; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t7 = false; break _b8; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t7) i = _t6;
  }
  {
    const _t9 = i; let _t10 = true;
    _b11: {
      if (!__lit(c, i, src, "=", "$punct")) { _t10 = false; break _b11; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t10 = false; break _b11; }
      expr = c[i] as ExprNode;
      i++;
    }
    if (!_t10) i = _t9;
  }
  {
    const _t12 = i; let _t13 = true;
    _b14: {
      if (!__lit(c, i, src, ";", "$punct")) { _t13 = false; break _b14; }
      i++;
    }
    if (!_t13) i = _t12;
  }
  if (i !== c.length) return null;
  return { arm: "memberName", memberName: memberName!, type, expr };
}

function _ClassMember$memberName2(c: readonly CstChild[], src: string): ClassMemberMatch | null {
  let memberName: (MemberNameNode) | undefined;
  let typeParams: (TypeParamsNode) | undefined;
  const param: (ParamNode)[] = [];
  let type: (TypeNode) | undefined;
  let block: (BlockNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "MemberName")) return null;
  memberName = c[i] as MemberNameNode;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "?", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__nodeOf(c, i, "TypeParams")) { _t4 = false; break _b5; }
      typeParams = c[i] as TypeParamsNode;
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, "(", "$punct")) return null;
  i++;
  {
    const _t6 = i; let _t9 = true;
    _b10: {
      if (!__nodeOf(c, i, "Param")) { _t9 = false; break _b10; }
      param.push(c[i] as ParamNode);
      i++;
    }
    if (!_t9) { i = _t6; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t62 = i; let _t7 = true;
      _b8: {
        if (!__nodeOf(c, i, "Param")) { _t7 = false; break _b8; }
        param.push(c[i] as ParamNode);
        i++;
      }
      if (!_t7) { i = _t62; break; }
    }
  }
  if (!__lit(c, i, src, ")", "$punct")) return null;
  i++;
  {
    const _t11 = i; let _t12 = true;
    _b13: {
      if (!__lit(c, i, src, ":", "$punct")) { _t12 = false; break _b13; }
      i++;
      if (!__nodeOf(c, i, "Type")) { _t12 = false; break _b13; }
      type = c[i] as TypeNode;
      i++;
    }
    if (!_t12) i = _t11;
  }
  {
    const _t14 = i; let _t15 = true;
    _b16: {
      if (!__nodeOf(c, i, "Block")) { _t15 = false; break _b16; }
      block = c[i] as BlockNode;
      i++;
    }
    if (!_t15) i = _t14;
  }
  {
    const _t17 = i; let _t18 = true;
    _b19: {
      if (!__lit(c, i, src, ";", "$punct")) { _t18 = false; break _b19; }
      i++;
    }
    if (!_t18) i = _t17;
  }
  if (i !== c.length) return null;
  return { arm: "memberName2", memberName: memberName!, typeParams, param, type, block };
}

export function matchClassMember(n: ClassMemberNode, src: string): ClassMemberMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "DecoratorExpr": {
        { const m = _ClassMember$decoratorExpr(c, src); if (m !== null) return m; }
        break;
      }
      case "MemberName": {
        { const m = _ClassMember$public(c, src); if (m !== null) return m; }
        { const m = _ClassMember$memberName(c, src); if (m !== null) return m; }
        { const m = _ClassMember$memberName2(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 42: {
        { const m = _ClassMember$public(c, src); if (m !== null) return m; }
        break;
      }
      case 59: {
        { const m = _ClassMember$semi(c, src); if (m !== null) return m; }
        break;
      }
      case 91: {
        { const m = _ClassMember$public(c, src); if (m !== null) return m; }
        break;
      }
      case 97: {
        { const m = _ClassMember$public(c, src); if (m !== null) return m; }
        break;
      }
      case 99: {
        { const m = _ClassMember$constructor(c, src); if (m !== null) return m; }
        break;
      }
      case 103: {
        { const m = _ClassMember$public(c, src); if (m !== null) return m; }
        break;
      }
      case 111: {
        { const m = _ClassMember$public(c, src); if (m !== null) return m; }
        break;
      }
      case 112: {
        { const m = _ClassMember$public(c, src); if (m !== null) return m; }
        break;
      }
      case 114: {
        { const m = _ClassMember$public(c, src); if (m !== null) return m; }
        break;
      }
      case 115: {
        { const m = _ClassMember$static_(c, src); if (m !== null) return m; }
        { const m = _ClassMember$public(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
    }
  }
  throw new Error("matchClassMember: no arm matches" + ' @' + n.offset);
}

export type EnumMemberMatch =
  | { arm: "memberName"; memberName: MemberNameNode; expr?: ExprNode };

function _EnumMember$memberName(c: readonly CstChild[], src: string): EnumMemberMatch | null {
  let memberName: (MemberNameNode) | undefined;
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "MemberName")) return null;
  memberName = c[i] as MemberNameNode;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "=", "$punct")) { _t1 = false; break _b2; }
      i++;
      if (!__nodeOf(c, i, "Expr")) { _t1 = false; break _b2; }
      expr = c[i] as ExprNode;
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (i !== c.length) return null;
  return { arm: "memberName", memberName: memberName!, expr };
}

export function matchEnumMember(n: EnumMemberNode, src: string): EnumMemberMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "MemberName": {
        { const m = _EnumMember$memberName(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
    }
  } else {
    switch (k0.tokenType) {
    }
  }
  throw new Error("matchEnumMember: no arm matches" + ' @' + n.offset);
}

export type ImportClauseMatch =
  | { arm: "defer"; ident: CstLeaf }
  | { arm: "ident"; ident: CstLeaf; alt?: { branch: "brace"; importSpecifier: (ImportSpecifierNode)[] } | { branch: "star"; ident: CstLeaf } }
  | { arm: "brace"; importSpecifier: (ImportSpecifierNode)[] }
  | { arm: "star"; ident: CstLeaf };

function _ImportClause$defer(c: readonly CstChild[], src: string): ImportClauseMatch | null {
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "defer", "$keyword")) return null;
  i++;
  if (!__lit(c, i, src, "*", "$punct")) return null;
  i++;
  if (!__lit(c, i, src, "as", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "defer", ident: ident! };
}

function _ImportClause$ident(c: readonly CstChild[], src: string): ImportClauseMatch | null {
  let ident: (CstLeaf) | undefined;
  let alt: ({ branch: "brace"; importSpecifier: (ImportSpecifierNode)[] } | { branch: "star"; ident: CstLeaf }) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, ",", "$punct")) { _t1 = false; break _b2; }
      i++;
      {
        let _t3 = false;
        if (!_t3) {
          const _t7_importSpecifier: (ImportSpecifierNode)[] = [];
          const _t4 = i; let _t5 = true;
          _b6: {
            if (!__lit(c, i, src, "{", "$punct")) { _t5 = false; break _b6; }
            i++;
            {
              const _t8 = i; let _t11 = true;
              _b12: {
                if (!__nodeOf(c, i, "ImportSpecifier")) { _t11 = false; break _b12; }
                _t7_importSpecifier.push(c[i] as ImportSpecifierNode);
                i++;
              }
              if (!_t11) { i = _t8; }
              else for (;;) {
                if (!__lit(c, i, src, ",", "$punct")) break;
                i++;
                const _t82 = i; let _t9 = true;
                _b10: {
                  if (!__nodeOf(c, i, "ImportSpecifier")) { _t9 = false; break _b10; }
                  _t7_importSpecifier.push(c[i] as ImportSpecifierNode);
                  i++;
                }
                if (!_t9) { i = _t82; break; }
              }
            }
            if (!__lit(c, i, src, "}", "$punct")) { _t5 = false; break _b6; }
            i++;
          }
          if (_t5) { _t3 = true; alt = ({ branch: "brace", importSpecifier: _t7_importSpecifier }) as typeof alt; }
          else i = _t4;
        }
        if (!_t3) {
          let _t16_ident: (CstLeaf) | undefined;
          const _t13 = i; let _t14 = true;
          _b15: {
            if (!__lit(c, i, src, "*", "$punct")) { _t14 = false; break _b15; }
            i++;
            if (!__lit(c, i, src, "as", "$keyword")) { _t14 = false; break _b15; }
            i++;
            if (!(__tok(c, i, "Ident"))) { _t14 = false; break _b15; }
            _t16_ident = c[i] as CstLeaf;
            i++;
          }
          if (_t14) { _t3 = true; alt = ({ branch: "star", ident: _t16_ident! }) as typeof alt; }
          else i = _t13;
        }
        if (!_t3) { _t1 = false; break _b2; }
      }
    }
    if (!_t1) i = _t0;
  }
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident!, alt };
}

function _ImportClause$brace(c: readonly CstChild[], src: string): ImportClauseMatch | null {
  const importSpecifier: (ImportSpecifierNode)[] = [];
  let i = 0;
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t3 = true;
    _b4: {
      if (!__nodeOf(c, i, "ImportSpecifier")) { _t3 = false; break _b4; }
      importSpecifier.push(c[i] as ImportSpecifierNode);
      i++;
    }
    if (!_t3) { i = _t0; }
    else for (;;) {
      if (!__lit(c, i, src, ",", "$punct")) break;
      i++;
      const _t02 = i; let _t1 = true;
      _b2: {
        if (!__nodeOf(c, i, "ImportSpecifier")) { _t1 = false; break _b2; }
        importSpecifier.push(c[i] as ImportSpecifierNode);
        i++;
      }
      if (!_t1) { i = _t02; break; }
    }
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "brace", importSpecifier };
}

function _ImportClause$star(c: readonly CstChild[], src: string): ImportClauseMatch | null {
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "*", "$punct")) return null;
  i++;
  if (!__lit(c, i, src, "as", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "star", ident: ident! };
}

export function matchImportClause(n: ImportClauseNode, src: string): ImportClauseMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 42: {
        { const m = _ImportClause$star(c, src); if (m !== null) return m; }
        break;
      }
      case 100: {
        { const m = _ImportClause$defer(c, src); if (m !== null) return m; }
        break;
      }
      case 123: {
        { const m = _ImportClause$brace(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _ImportClause$ident(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchImportClause: no arm matches" + ' @' + n.offset);
}

export type ImportSpecifierMatch =
  | { arm: "ident"; ident: CstLeaf; ident2?: CstLeaf; asTok?: CstLeaf }
  | { arm: "string"; string: CstLeaf; ident: CstLeaf };

function _ImportSpecifier$ident(c: readonly CstChild[], src: string): ImportSpecifierMatch | null {
  let ident: (CstLeaf) | undefined;
  let ident2: (CstLeaf) | undefined;
  let asTok: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "as", "$keyword")) { _t1 = false; break _b2; }
      asTok = c[i] as CstLeaf;
      i++;
      if (!(__tok(c, i, "Ident"))) { _t1 = false; break _b2; }
      ident2 = c[i] as CstLeaf;
      i++;
    }
    if (!_t1) i = _t0;
  }
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident!, ident2, asTok };
}

function _ImportSpecifier$string(c: readonly CstChild[], src: string): ImportSpecifierMatch | null {
  let string: (CstLeaf) | undefined;
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "String"))) return null;
  string = c[i] as CstLeaf;
  i++;
  if (!__lit(c, i, src, "as", "$keyword")) return null;
  i++;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "string", string: string!, ident: ident! };
}

export function matchImportSpecifier(n: ImportSpecifierNode, src: string): ImportSpecifierMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _ImportSpecifier$ident(c, src); if (m !== null) return m; }
        break;
      }
      case "String": {
        { const m = _ImportSpecifier$string(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchImportSpecifier: no arm matches" + ' @' + n.offset);
}

export type ExportSpecifierMatch =
  | { arm: "ident"; alt: { branch: "ident"; ident: CstLeaf } | { branch: "string"; string: CstLeaf }; alt2?: { branch: "ident"; ident: CstLeaf } | { branch: "string"; string: CstLeaf }; asTok?: CstLeaf };

function _ExportSpecifier$ident(c: readonly CstChild[], src: string): ExportSpecifierMatch | null {
  let alt: ({ branch: "ident"; ident: CstLeaf } | { branch: "string"; string: CstLeaf }) | undefined;
  let alt2: ({ branch: "ident"; ident: CstLeaf } | { branch: "string"; string: CstLeaf }) | undefined;
  let asTok: (CstLeaf) | undefined;
  let i = 0;
  {
    let _t0 = false;
    if (!_t0) {
      let _t4_ident: (CstLeaf) | undefined;
      const _t1 = i; let _t2 = true;
      _b3: {
        if (!(__tok(c, i, "Ident"))) { _t2 = false; break _b3; }
        _t4_ident = c[i] as CstLeaf;
        i++;
      }
      if (_t2) { _t0 = true; alt = ({ branch: "ident", ident: _t4_ident! }) as typeof alt; }
      else i = _t1;
    }
    if (!_t0) {
      let _t8_string: (CstLeaf) | undefined;
      const _t5 = i; let _t6 = true;
      _b7: {
        if (!(__tok(c, i, "String"))) { _t6 = false; break _b7; }
        _t8_string = c[i] as CstLeaf;
        i++;
      }
      if (_t6) { _t0 = true; alt = ({ branch: "string", string: _t8_string! }) as typeof alt; }
      else i = _t5;
    }
    if (!_t0) return null;
  }
  {
    const _t9 = i; let _t10 = true;
    _b11: {
      if (!__lit(c, i, src, "as", "$keyword")) { _t10 = false; break _b11; }
      asTok = c[i] as CstLeaf;
      i++;
      {
        let _t12 = false;
        if (!_t12) {
          let _t16_ident: (CstLeaf) | undefined;
          const _t13 = i; let _t14 = true;
          _b15: {
            if (!(__tok(c, i, "Ident"))) { _t14 = false; break _b15; }
            _t16_ident = c[i] as CstLeaf;
            i++;
          }
          if (_t14) { _t12 = true; alt2 = ({ branch: "ident", ident: _t16_ident! }) as typeof alt2; }
          else i = _t13;
        }
        if (!_t12) {
          let _t20_string: (CstLeaf) | undefined;
          const _t17 = i; let _t18 = true;
          _b19: {
            if (!(__tok(c, i, "String"))) { _t18 = false; break _b19; }
            _t20_string = c[i] as CstLeaf;
            i++;
          }
          if (_t18) { _t12 = true; alt2 = ({ branch: "string", string: _t20_string! }) as typeof alt2; }
          else i = _t17;
        }
        if (!_t12) { _t10 = false; break _b11; }
      }
    }
    if (!_t10) i = _t9;
  }
  if (i !== c.length) return null;
  return { arm: "ident", alt: alt!, alt2, asTok };
}

export function matchExportSpecifier(n: ExportSpecifierNode, src: string): ExportSpecifierMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _ExportSpecifier$ident(c, src); if (m !== null) return m; }
        break;
      }
      case "String": {
        { const m = _ExportSpecifier$ident(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchExportSpecifier: no arm matches" + ' @' + n.offset);
}

export type JSXTagNameMatch =
  | { arm: "ident"; ident: CstLeaf; alt: ({ branch: "dot"; ident: CstLeaf } | { branch: "colon"; ident: CstLeaf } | { branch: "dash"; ident: CstLeaf })[] };

function _JSXTagName$ident(c: readonly CstChild[], src: string): JSXTagNameMatch | null {
  let ident: (CstLeaf) | undefined;
  const alt: ({ branch: "dot"; ident: CstLeaf } | { branch: "colon"; ident: CstLeaf } | { branch: "dash"; ident: CstLeaf })[] = [];
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      {
        let _t3 = false;
        if (!_t3) {
          let _t7_ident: (CstLeaf) | undefined;
          const _t4 = i; let _t5 = true;
          _b6: {
            if (!__lit(c, i, src, ".", "$punct")) { _t5 = false; break _b6; }
            i++;
            if (!(__tok(c, i, "Ident"))) { _t5 = false; break _b6; }
            _t7_ident = c[i] as CstLeaf;
            i++;
          }
          if (_t5) { _t3 = true; alt.push({ branch: "dot", ident: _t7_ident! } as never); }
          else i = _t4;
        }
        if (!_t3) {
          let _t11_ident: (CstLeaf) | undefined;
          const _t8 = i; let _t9 = true;
          _b10: {
            if (!__lit(c, i, src, ":", "$punct")) { _t9 = false; break _b10; }
            i++;
            if (!(__tok(c, i, "Ident"))) { _t9 = false; break _b10; }
            _t11_ident = c[i] as CstLeaf;
            i++;
          }
          if (_t9) { _t3 = true; alt.push({ branch: "colon", ident: _t11_ident! } as never); }
          else i = _t8;
        }
        if (!_t3) {
          let _t15_ident: (CstLeaf) | undefined;
          const _t12 = i; let _t13 = true;
          _b14: {
            if (!__lit(c, i, src, "-", "$punct")) { _t13 = false; break _b14; }
            i++;
            if (!(__tok(c, i, "Ident"))) { _t13 = false; break _b14; }
            _t15_ident = c[i] as CstLeaf;
            i++;
          }
          if (_t13) { _t3 = true; alt.push({ branch: "dash", ident: _t15_ident! } as never); }
          else i = _t12;
        }
        if (!_t3) { _t1 = false; break _b2; }
      }
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident!, alt };
}

export function matchJSXTagName(n: JSXTagNameNode, src: string): JSXTagNameMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _JSXTagName$ident(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchJSXTagName: no arm matches" + ' @' + n.offset);
}

export type JSXAttrValueMatch =
  | { arm: "string"; string: CstLeaf }
  | { arm: "brace"; expr: ExprNode };

function _JSXAttrValue$string(c: readonly CstChild[], src: string): JSXAttrValueMatch | null {
  let string: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "String"))) return null;
  string = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "string", string: string! };
}

function _JSXAttrValue$brace(c: readonly CstChild[], src: string): JSXAttrValueMatch | null {
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "brace", expr: expr! };
}

export function matchJSXAttrValue(n: JSXAttrValueNode, src: string): JSXAttrValueMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 123: {
        { const m = _JSXAttrValue$brace(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "String": {
        { const m = _JSXAttrValue$string(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchJSXAttrValue: no arm matches" + ' @' + n.offset);
}

export type JSXAttrMatch =
  | { arm: "brace"; expr: ExprNode }
  | { arm: "ident"; ident: CstLeaf; alt: ({ branch: "dot"; ident: CstLeaf } | { branch: "colon"; ident: CstLeaf } | { branch: "dash"; ident: CstLeaf })[]; jSXAttrValue?: JSXAttrValueNode };

function _JSXAttr$brace(c: readonly CstChild[], src: string): JSXAttrMatch | null {
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  if (!__lit(c, i, src, "...", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "Expr")) return null;
  expr = c[i] as ExprNode;
  i++;
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "brace", expr: expr! };
}

function _JSXAttr$ident(c: readonly CstChild[], src: string): JSXAttrMatch | null {
  let ident: (CstLeaf) | undefined;
  const alt: ({ branch: "dot"; ident: CstLeaf } | { branch: "colon"; ident: CstLeaf } | { branch: "dash"; ident: CstLeaf })[] = [];
  let jSXAttrValue: (JSXAttrValueNode) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      {
        let _t3 = false;
        if (!_t3) {
          let _t7_ident: (CstLeaf) | undefined;
          const _t4 = i; let _t5 = true;
          _b6: {
            if (!__lit(c, i, src, ".", "$punct")) { _t5 = false; break _b6; }
            i++;
            if (!(__tok(c, i, "Ident"))) { _t5 = false; break _b6; }
            _t7_ident = c[i] as CstLeaf;
            i++;
          }
          if (_t5) { _t3 = true; alt.push({ branch: "dot", ident: _t7_ident! } as never); }
          else i = _t4;
        }
        if (!_t3) {
          let _t11_ident: (CstLeaf) | undefined;
          const _t8 = i; let _t9 = true;
          _b10: {
            if (!__lit(c, i, src, ":", "$punct")) { _t9 = false; break _b10; }
            i++;
            if (!(__tok(c, i, "Ident"))) { _t9 = false; break _b10; }
            _t11_ident = c[i] as CstLeaf;
            i++;
          }
          if (_t9) { _t3 = true; alt.push({ branch: "colon", ident: _t11_ident! } as never); }
          else i = _t8;
        }
        if (!_t3) {
          let _t15_ident: (CstLeaf) | undefined;
          const _t12 = i; let _t13 = true;
          _b14: {
            if (!__lit(c, i, src, "-", "$punct")) { _t13 = false; break _b14; }
            i++;
            if (!(__tok(c, i, "Ident"))) { _t13 = false; break _b14; }
            _t15_ident = c[i] as CstLeaf;
            i++;
          }
          if (_t13) { _t3 = true; alt.push({ branch: "dash", ident: _t15_ident! } as never); }
          else i = _t12;
        }
        if (!_t3) { _t1 = false; break _b2; }
      }
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  {
    const _t16 = i; let _t17 = true;
    _b18: {
      if (!__lit(c, i, src, "=", "$punct")) { _t17 = false; break _b18; }
      i++;
      if (!__nodeOf(c, i, "JSXAttrValue")) { _t17 = false; break _b18; }
      jSXAttrValue = c[i] as JSXAttrValueNode;
      i++;
    }
    if (!_t17) i = _t16;
  }
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident!, alt, jSXAttrValue };
}

export function matchJSXAttr(n: JSXAttrNode, src: string): JSXAttrMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 123: {
        { const m = _JSXAttr$brace(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _JSXAttr$ident(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchJSXAttr: no arm matches" + ' @' + n.offset);
}

export type JSXContainerMatch =
  | { arm: "brace"; expr?: ExprNode };

function _JSXContainer$brace(c: readonly CstChild[], src: string): JSXContainerMatch | null {
  let expr: (ExprNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "{", "$punct")) return null;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "...", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  {
    const _t3 = i; let _t4 = true;
    _b5: {
      if (!__nodeOf(c, i, "Expr")) { _t4 = false; break _b5; }
      expr = c[i] as ExprNode;
      i++;
    }
    if (!_t4) i = _t3;
  }
  if (!__lit(c, i, src, "}", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "brace", expr };
}

export function matchJSXContainer(n: JSXContainerNode, src: string): JSXContainerMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 123: {
        { const m = _JSXContainer$brace(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
    }
  }
  throw new Error("matchJSXContainer: no arm matches" + ' @' + n.offset);
}

export type JSXChildMatch =
  | { arm: "jSXElement"; jSXElement: JSXElementNode }
  | { arm: "jSXContainer"; jSXContainer: JSXContainerNode }
  | { arm: "ident"; ident: CstLeaf }
  | { arm: "number"; number: CstLeaf }
  | { arm: "string"; string: CstLeaf }
  | { arm: "_"; _Kw: "," | "." | ":" | ";" | "!" | "?" };

function _JSXChild$jSXElement(c: readonly CstChild[], src: string): JSXChildMatch | null {
  let jSXElement: (JSXElementNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "JSXElement")) return null;
  jSXElement = c[i] as JSXElementNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "jSXElement", jSXElement: jSXElement! };
}

function _JSXChild$jSXContainer(c: readonly CstChild[], src: string): JSXChildMatch | null {
  let jSXContainer: (JSXContainerNode) | undefined;
  let i = 0;
  if (!__nodeOf(c, i, "JSXContainer")) return null;
  jSXContainer = c[i] as JSXContainerNode;
  i++;
  if (i !== c.length) return null;
  return { arm: "jSXContainer", jSXContainer: jSXContainer! };
}

function _JSXChild$ident(c: readonly CstChild[], src: string): JSXChildMatch | null {
  let ident: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Ident"))) return null;
  ident = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "ident", ident: ident! };
}

function _JSXChild$number(c: readonly CstChild[], src: string): JSXChildMatch | null {
  let number: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "Number"))) return null;
  number = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "number", number: number! };
}

function _JSXChild$string(c: readonly CstChild[], src: string): JSXChildMatch | null {
  let string: (CstLeaf) | undefined;
  let i = 0;
  if (!(__tok(c, i, "String"))) return null;
  string = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "string", string: string! };
}

function _JSXChild$_(c: readonly CstChild[], src: string): JSXChildMatch | null {
  let _Kw: ("," | "." | ":" | ";" | "!" | "?") | undefined;
  let i = 0;
  if (!(__lit(c, i, src, ",", "$punct") || __lit(c, i, src, ".", "$punct") || __lit(c, i, src, ":", "$punct") || __lit(c, i, src, ";", "$punct") || __lit(c, i, src, "!", "$punct") || __lit(c, i, src, "?", "$punct"))) return null;
  _Kw = src.slice(c[i].offset, c[i].end) as "," | "." | ":" | ";" | "!" | "?";
  i++;
  if (i !== c.length) return null;
  return { arm: "_", _Kw: _Kw! };
}

export function matchJSXChild(n: JSXChildNode, src: string): JSXChildMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "JSXContainer": {
        { const m = _JSXChild$jSXContainer(c, src); if (m !== null) return m; }
        break;
      }
      case "JSXElement": {
        { const m = _JSXChild$jSXElement(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 33: {
        { const m = _JSXChild$_(c, src); if (m !== null) return m; }
        break;
      }
      case 44: {
        { const m = _JSXChild$_(c, src); if (m !== null) return m; }
        break;
      }
      case 46: {
        { const m = _JSXChild$_(c, src); if (m !== null) return m; }
        break;
      }
      case 58: {
        { const m = _JSXChild$_(c, src); if (m !== null) return m; }
        break;
      }
      case 59: {
        { const m = _JSXChild$_(c, src); if (m !== null) return m; }
        break;
      }
      case 63: {
        { const m = _JSXChild$_(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
      case "Ident": {
        { const m = _JSXChild$ident(c, src); if (m !== null) return m; }
        break;
      }
      case "Number": {
        { const m = _JSXChild$number(c, src); if (m !== null) return m; }
        break;
      }
      case "String": {
        { const m = _JSXChild$string(c, src); if (m !== null) return m; }
        break;
      }
    }
  }
  throw new Error("matchJSXChild: no arm matches" + ' @' + n.offset);
}

export type JSXElementMatch =
  | { arm: "lt"; jSXTagName: JSXTagNameNode; type: (TypeNode)[]; jSXAttr: (JSXAttrNode)[]; jSXSelfClose: CstLeaf }
  | { arm: "lt2"; jSXTagName: JSXTagNameNode; type: (TypeNode)[]; jSXAttr: (JSXAttrNode)[]; jSXChild: (JSXChildNode)[]; jSXClose: CstLeaf; jSXTagName2?: JSXTagNameNode }
  | { arm: "lt3"; jSXChild: (JSXChildNode)[]; jSXClose: CstLeaf };

function _JSXElement$lt(c: readonly CstChild[], src: string): JSXElementMatch | null {
  let jSXTagName: (JSXTagNameNode) | undefined;
  const type: (TypeNode)[] = [];
  const jSXAttr: (JSXAttrNode)[] = [];
  let jSXSelfClose: (CstLeaf) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "<", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "JSXTagName")) return null;
  jSXTagName = c[i] as JSXTagNameNode;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "<", "$punct")) { _t1 = false; break _b2; }
      i++;
      {
        const _t3 = i; let _t6 = true;
        _b7: {
          if (!__nodeOf(c, i, "Type")) { _t6 = false; break _b7; }
          type.push(c[i] as TypeNode);
          i++;
        }
        if (!_t6) { i = _t3; }
        else for (;;) {
          if (!__lit(c, i, src, ",", "$punct")) break;
          i++;
          const _t32 = i; let _t4 = true;
          _b5: {
            if (!__nodeOf(c, i, "Type")) { _t4 = false; break _b5; }
            type.push(c[i] as TypeNode);
            i++;
          }
          if (!_t4) { i = _t32; break; }
        }
      }
      if (!__lit(c, i, src, ">", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  for (;;) {
    const _t8 = i; let _t9 = true;
    _b10: {
      if (!__nodeOf(c, i, "JSXAttr")) { _t9 = false; break _b10; }
      jSXAttr.push(c[i] as JSXAttrNode);
      i++;
    }
    if (!_t9) { i = _t8; break; }
    if (i === _t8) break;
  }
  if (!(__tok(c, i, "JSXSelfClose"))) return null;
  jSXSelfClose = c[i] as CstLeaf;
  i++;
  if (i !== c.length) return null;
  return { arm: "lt", jSXTagName: jSXTagName!, type, jSXAttr, jSXSelfClose: jSXSelfClose! };
}

function _JSXElement$lt2(c: readonly CstChild[], src: string): JSXElementMatch | null {
  let jSXTagName: (JSXTagNameNode) | undefined;
  const type: (TypeNode)[] = [];
  const jSXAttr: (JSXAttrNode)[] = [];
  const jSXChild: (JSXChildNode)[] = [];
  let jSXClose: (CstLeaf) | undefined;
  let jSXTagName2: (JSXTagNameNode) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "<", "$punct")) return null;
  i++;
  if (!__nodeOf(c, i, "JSXTagName")) return null;
  jSXTagName = c[i] as JSXTagNameNode;
  i++;
  {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__lit(c, i, src, "<", "$punct")) { _t1 = false; break _b2; }
      i++;
      {
        const _t3 = i; let _t6 = true;
        _b7: {
          if (!__nodeOf(c, i, "Type")) { _t6 = false; break _b7; }
          type.push(c[i] as TypeNode);
          i++;
        }
        if (!_t6) { i = _t3; }
        else for (;;) {
          if (!__lit(c, i, src, ",", "$punct")) break;
          i++;
          const _t32 = i; let _t4 = true;
          _b5: {
            if (!__nodeOf(c, i, "Type")) { _t4 = false; break _b5; }
            type.push(c[i] as TypeNode);
            i++;
          }
          if (!_t4) { i = _t32; break; }
        }
      }
      if (!__lit(c, i, src, ">", "$punct")) { _t1 = false; break _b2; }
      i++;
    }
    if (!_t1) i = _t0;
  }
  for (;;) {
    const _t8 = i; let _t9 = true;
    _b10: {
      if (!__nodeOf(c, i, "JSXAttr")) { _t9 = false; break _b10; }
      jSXAttr.push(c[i] as JSXAttrNode);
      i++;
    }
    if (!_t9) { i = _t8; break; }
    if (i === _t8) break;
  }
  if (!__lit(c, i, src, ">", "$punct")) return null;
  i++;
  for (;;) {
    const _t11 = i; let _t12 = true;
    _b13: {
      if (!__nodeOf(c, i, "JSXChild")) { _t12 = false; break _b13; }
      jSXChild.push(c[i] as JSXChildNode);
      i++;
    }
    if (!_t12) { i = _t11; break; }
    if (i === _t11) break;
  }
  if (!(__tok(c, i, "JSXClose"))) return null;
  jSXClose = c[i] as CstLeaf;
  i++;
  {
    const _t14 = i; let _t15 = true;
    _b16: {
      if (!__nodeOf(c, i, "JSXTagName")) { _t15 = false; break _b16; }
      jSXTagName2 = c[i] as JSXTagNameNode;
      i++;
    }
    if (!_t15) i = _t14;
  }
  if (!__lit(c, i, src, ">", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "lt2", jSXTagName: jSXTagName!, type, jSXAttr, jSXChild, jSXClose: jSXClose!, jSXTagName2 };
}

function _JSXElement$lt3(c: readonly CstChild[], src: string): JSXElementMatch | null {
  const jSXChild: (JSXChildNode)[] = [];
  let jSXClose: (CstLeaf) | undefined;
  let i = 0;
  if (!__lit(c, i, src, "<", "$punct")) return null;
  i++;
  if (!__lit(c, i, src, ">", "$punct")) return null;
  i++;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      if (!__nodeOf(c, i, "JSXChild")) { _t1 = false; break _b2; }
      jSXChild.push(c[i] as JSXChildNode);
      i++;
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (!(__tok(c, i, "JSXClose"))) return null;
  jSXClose = c[i] as CstLeaf;
  i++;
  if (!__lit(c, i, src, ">", "$punct")) return null;
  i++;
  if (i !== c.length) return null;
  return { arm: "lt3", jSXChild, jSXClose: jSXClose! };
}

export function matchJSXElement(n: JSXElementNode, src: string): JSXElementMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
      case 60: {
        { const m = _JSXElement$lt(c, src); if (m !== null) return m; }
        { const m = _JSXElement$lt2(c, src); if (m !== null) return m; }
        { const m = _JSXElement$lt3(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else {
    switch (k0.tokenType) {
    }
  }
  throw new Error("matchJSXElement: no arm matches" + ' @' + n.offset);
}

export type ProgramMatch =
  | { arm: "decl"; alt: ({ branch: "decl"; decl: DeclNode } | { branch: "stmt"; stmt: StmtNode })[] };

function _Program$decl(c: readonly CstChild[], src: string): ProgramMatch | null {
  const alt: ({ branch: "decl"; decl: DeclNode } | { branch: "stmt"; stmt: StmtNode })[] = [];
  let i = 0;
  for (;;) {
    const _t0 = i; let _t1 = true;
    _b2: {
      {
        let _t3 = false;
        if (!_t3) {
          let _t7_decl: (DeclNode) | undefined;
          const _t4 = i; let _t5 = true;
          _b6: {
            if (!__nodeOf(c, i, "Decl")) { _t5 = false; break _b6; }
            _t7_decl = c[i] as DeclNode;
            i++;
          }
          if (_t5) { _t3 = true; alt.push({ branch: "decl", decl: _t7_decl! } as never); }
          else i = _t4;
        }
        if (!_t3) {
          let _t11_stmt: (StmtNode) | undefined;
          const _t8 = i; let _t9 = true;
          _b10: {
            if (!__nodeOf(c, i, "Stmt")) { _t9 = false; break _b10; }
            _t11_stmt = c[i] as StmtNode;
            i++;
          }
          if (_t9) { _t3 = true; alt.push({ branch: "stmt", stmt: _t11_stmt! } as never); }
          else i = _t8;
        }
        if (!_t3) { _t1 = false; break _b2; }
      }
    }
    if (!_t1) { i = _t0; break; }
    if (i === _t0) break;
  }
  if (i !== c.length) return null;
  return { arm: "decl", alt };
}

export function matchProgram(n: ProgramNode, src: string): ProgramMatch {
  const c = n.children;
  const k0 = c[0] as (CstChild & { tokenType?: string; rule?: string }) | undefined;
  if (k0 === undefined) {
    { const m = _Program$decl(c, src); if (m !== null) return m; }
  } else if (k0.tokenType === undefined) {
    switch (k0.rule) {
      case "Decl": {
        { const m = _Program$decl(c, src); if (m !== null) return m; }
        break;
      }
      case "Stmt": {
        { const m = _Program$decl(c, src); if (m !== null) return m; }
        break;
      }
    }
  } else if (k0.tokenType === '$keyword' || k0.tokenType === '$punct') {
    switch (src.charCodeAt(k0.offset)) {
    }
  } else {
    switch (k0.tokenType) {
    }
  }
  throw new Error("matchProgram: no arm matches" + ' @' + n.offset);
}

/** rule name → its matcher (generic walking; the totality gate uses this). */
export const MATCHERS: Record<string, (n: never, src: string) => { arm: string }> = {
  "Type": matchType,
  "TypeMember": matchTypeMember,
  "DecoratorExpr": matchDecoratorExpr,
  "TypeofRef": matchTypeofRef,
  "Expr": matchExpr,
  "Prop": matchProp,
  "MemberName": matchMemberName,
  "NewTarget": matchNewTarget,
  "ClassHeritage": matchClassHeritage,
  "Stmt": matchStmt,
  "Block": matchBlock,
  "BindingProperty": matchBindingProperty,
  "BindingElement": matchBindingElement,
  "ArrayBindingElement": matchArrayBindingElement,
  "BindingPattern": matchBindingPattern,
  "Binding": matchBinding,
  "ForBinding": matchForBinding,
  "Param": matchParam,
  "ForHead": matchForHead,
  "SwitchCase": matchSwitchCase,
  "TypeParams": matchTypeParams,
  "TypeParam": matchTypeParam,
  "Decl": matchDecl,
  "InterfaceMember": matchInterfaceMember,
  "ClassMember": matchClassMember,
  "EnumMember": matchEnumMember,
  "ImportClause": matchImportClause,
  "ImportSpecifier": matchImportSpecifier,
  "ExportSpecifier": matchExportSpecifier,
  "JSXTagName": matchJSXTagName,
  "JSXAttrValue": matchJSXAttrValue,
  "JSXAttr": matchJSXAttr,
  "JSXContainer": matchJSXContainer,
  "JSXChild": matchJSXChild,
  "JSXElement": matchJSXElement,
  "Program": matchProgram,
};
